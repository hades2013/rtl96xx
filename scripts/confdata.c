...
int conf_write(const char *name)
{
	...
	/* busybox`s code */
	if (out_h) {
		fprintf(out_h, "#ifndef LW_CONFIG_H\n#define LW_CONFIG_H\n");
		fprintf(out_h, "/*\n"
		...
		if (!sym) {
			...
			case S_TRISTATE:
				...
				case no:
					...
					if (out_h)
                    {
                        /* begin added by liujiaquan for compile subdir */
                        pcSubdir = NULL;
                        pcSubdir = strstr(sym->name, "_subdir");
                        if(NULL != pcSubdir)
                            *pcSubdir = 0;
                        /* end added by liujiaquan for compile subdir */
			...
			if (out_h) {
				...
				fprintf(out_h, "#define SKIP_%s(...)%s\n", opt_name,
					(use_flg ? "" : "  __VA_ARGS__"));
			    if((0 == strncmp(sym->name,"PRODUCT_", 8)) && use_flg)
			    {
			        char *ptemp = NULL;
			        fprintf(out_h, "#define PRODUCT_NAME \"%s\"\n", opt_name);
                    #if 0
                    for(ptemp = opt_name; *ptemp != '\0'; ptemp++)
                    {                        
                        fprintf( out_h, "%c", (char)toupper(*ptemp));
                    }
                    fprintf(out_h, "\"\n");
                    #endif
                }
...
next:
	...
	fclose(out);
	if (out_h) {
		fprintf(out_h, "#endif /* MW_CONFIG_H */\n");   /* busybox`s code */
		fclose(out_h);
		//rename(".tmpconfig.h", "./public/config.h");  /* product config name */
		file_write_dep(NULL);
	}
