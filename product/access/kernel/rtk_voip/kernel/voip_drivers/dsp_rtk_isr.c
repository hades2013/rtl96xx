			...
			} else
			{
				...
				if(iphone_handfree==0 && (pcm_ch_for_DAA != 1))
				{
					...
					if( RtkLecObj.EC_G168Process == NULL ) 
						...
						RtkLecObj. EC_G168Process( chid, lec_ref/*(Word16*)&LEC_Rin_CircBuf[chid][80*PCM_PERIOD*LEC_buf_rindex[chid] +i*PCM_PERIOD_10MS_SIZE/2+sync_sample_offset_daa[chid]]*/,
						          (Word16*)(rxBuf+(i*PCM_PERIOD_10MS_SIZE>>2)),
						          (Word16*)(rxBuf+(i*PCM_PERIOD_10MS_SIZE>>2)) );
#endif
                                }
...
//#endif
		   ...
		   )
		{
			...
			{	
...
#endif
#else
				RtkLecObj. EC_G168Process( chid, lec_ref, 
#endif		
				   		(Word16*)(rxBuf+(i*PCM_PERIOD_10MS_SIZE>>2)),
                                                (Word16*)(rx_fifo[chid][rx_fifo_cnt_w[chid]]));
#else
				...
				RtkLecObj. EC_G168Process( chid, lec_ref/*(Word16*)&LEC_RinBuf[chid][80*PCM_PERIOD*(FSIZE-sync_point[chid])+i*PCM_PERIOD_10MS_SIZE/2-sync_sample[chid]]*/, 
#else
	#if 1
...
#ifdef DTMF_DET_DURATION_HIGH_ACCURACY
			unsigned int duration;
			if (( 1 == dtmf_on_flag[chid][0] ) && ( 1 == dtmf_det_duration_get(chid, 0, &duration)))
			{
				voip_event_dtmf_in( chid, 'E', 0, get_dtmf_dBFS(chid, 0, 1), duration );
				get_dtmf_dBFS(chid, 0, 1);
				dtmf_on_cnt_10ms[chid][0]=0;
