		...
		// port 0
		{
			// proxies
			{
				// proxies[0]
				{
					...
					0,			//TLS_enable
					60,			//e8c: option ping expire , HEARTBEAT_CYCLE
...
void rcm_voip_flash_if_name(char *if_name_dst, int if_name_max_len)
{
...
//		voip_if_link_status = get_net_link_status(VOIP_WAN_IF);
		...
		printf("MIB_ATM_VC_TBL Entry.dgw  is 0\n");
		goto l_err_end;
	}
...
l_err_end:
	printf("Error: No valid IF for VoIP!!!\n");
	//Fill NULL string
	*if_name_dst = NULL;
...
#else
	rcm_voip_flash_if_name(interface, DNS_LEN); /*copy default gateway to pVoIPShare->net_cfg.gateway_interface */
#endif
