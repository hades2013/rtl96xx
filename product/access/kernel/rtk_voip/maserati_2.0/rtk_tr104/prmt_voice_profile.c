...
struct CWMP_PRMT tVoiceProfileEntityLeafInfo[] =
{
	...
	{ "SignalingProtocols",                eCWMP_tSTRING,  CWMP_READ|CWMP_WRITE, &tVoiceProfileEntityLeafOP},
	...
	if( strcmp( pszLastname, "Enable" ) == 0 ) {
		...
		if( str2id_DTMFMethod( data, &s.DTMFMethod ) ){
			...
			return ERR_9007;
   			...
   			res = mib_set_type1( MIB_VOICE_PROFILE__DIGIT_MAP_ENABLE, nVoiceProfileInstNum, &s.DigitMapEnable );
    } else if( strcmp( pszLastname, "DigitMap" ) == 0 ) {
	...
	*data = NULL;
	...
	if( strcmp( pszLastname, "ProxyServer" ) == 0 ) {
		...
		*data = intdup( s.XCT_HeartbeatCount );
		...
		mib_get_type1( MIB_VOICE_PROFILE__SIP__XCT_SESSIONUPDATETIMER, nVoiceProfileInstNum, &s.XCT_SessionUpdateTimer );
...
int setVPSipEntity(char *name, struct CWMP_LEAF *entity, int type, void *data)
{
	...
	if( strcmp( pszLastname, "ProxyServer" ) == 0 ) {
		...
		res = mib_set_type1( MIB_VOICE_PROFILE__SIP__XCT_SESSIONUPDATETIMER, nVoiceProfileInstNum, data );
	} else {
		return ERR_9005;
	}
	return res;	//1->self-reboot, 0->apply without reboot
...
{
	/* VoiceService.{i}.VoiceProfile.{i}.X_CT-COM_G711FAX */
	unsigned int nVoiceProfileInstNum;
    union{
	    int Enable;
        char ControlType[16];
