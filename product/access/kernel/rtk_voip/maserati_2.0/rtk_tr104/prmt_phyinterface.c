...
//******* VoiceService.{i}.PhyInterface.{i}.Tests.X_CT-COM_SimulateTest. EntityLEAF*******
...
//******* VoiceService.{i}.PhyInterface.{i}.X_ CT-COM_Stats.PoorQualityList.{i}. EntityLEAF*******
...
//******* VoiceService.{i}.PhyInterface.{i}.X_ CT-COM_Stats EntityObject*******
...
{
    /*(name,                  type,           flag,       op)*/
    {"X_CT-COM_SimulateTest", eCWMP_tOBJECT,  CWMP_READ,  NULL}
};
...
{
    /*(name,          type,           flag,      op)*/
	{ "InterfaceID",  eCWMP_tSTRING,  CWMP_READ, &tPhyInterfaceEntityLeafOP},
};
...
struct CWMP_PRMT tPhyInterfaceEntityObjectInfo[] =
{
    ...
    {"X_CT-COM_Stats", eCWMP_tOBJECT,  CWMP_READ,  NULL}
};
...
int objPhyInterface(char *name, struct CWMP_LEAF *e, int type, void *data)
{
	...
	case eCWMP_tINITOBJ:
	{
		...
		num = MAX_PHYINTERFACE_COUNT; /* Now, we has one voice profile only. */
        
		for( i=1; i<=num;i++ )
		{
			if( create_Object( c, tPhyInterfaceObject, sizeof(tPhyInterfaceObject), 1, MAX_PROFILE_COUNT) < 0 )
				return -1;
		}
            ...
            for( i=1; i<=num;i++ ) {
                ...
                if( remove_entity!=NULL )
                {
                    ...
                    add_Object( name, (struct CWMP_LINKNODE **)&entity->next,
                        tPhyInterfaceObject, sizeof(tPhyInterfaceObject), (unsigned int *)&num );
                }
           }
