...
static int udp_tl_open(void)
{
	...
	if (eXtl_udp.proto_family == AF_INET)
	{
		...
		res = setsockopt(udp_socket, SOL_SOCKET, SO_PRIORITY, 	(SOCKET_OPTION_VALUE)&pri, sizeof(pri));
	} else {
		int tos = (eXosip.dscp << 2) & 0xFC;
#ifdef IPV6_TCLASS
...
static int eXtl_update_local_target(osip_message_t * req)
{
...
#ifdef REDUCE_MALLOC
	int proxy_size = hostSize;
	char proxy[proxy_size];
#else
	char *proxy = NULL;
#endif	//REDUCE_MALLOC
	int i;
	...
	if (MSG_IS_REQUEST(req)) {
		if (req->from != NULL
		    && &req->from->url.host[0] != '\0')
		    osip_strncpy(proxy, req->from->url.host, proxy_size-1);
    } else {
		if (req->to != NULL && &req->to->url != NULL && &req->to->url.host != NULL)
			osip_strncpy(proxy, req->to->url.host, proxy_size-1);
    }
#else
	if (MSG_IS_REQUEST(req)) {
		if (req->from != NULL
	...
	if (udp_firewall_ip[0] != '\0') {
		...
		while (!osip_list_eol(&req->contacts, pos)) {
			...
			pos++;
#ifdef REDUCE_MALLOC
			if (co != NULL && &co->url.host[0] != '\0'
#if 0
				&& 0 == osip_strcasecmp(co->url->host, udp_firewall_ip)
#endif
				) {
			    if (ainfo == NULL) {
                    int port = osip_atoi(udp_firewall_port);
			        if (co->url.port > 0&&
			            5060 != port) {
			            co->url.port = osip_atoi(udp_firewall_port);
   					} else if (co->url.port > 0 &&	
							   co->url.port != port) {
						co->url.port = osip_atoi(udp_firewall_port);
                    }
                } else {
                    if (co->url.port > 0 && ainfo->nat_port != 5060) {
                        co->url.port = ainfo->nat_port;
                    } else if (co->url.port > 0 &&
                               ainfo->nat_port != co->url.port) {
						co->url.port = ainfo->nat_port;
					}
#if 1
					if (ainfo->nat_ip[0] != '\0')
					{
						osip_strncpy(co->url.host, ainfo->nat_ip, hostSize-1);
					}
				}
			}
#endif
					...
					int port, int out_socket)
{
...
#ifdef REDUCE_MALLOC
	if (host == NULL) {
	    host = osip_strdup(sip->req_uri.host);
	    if (sip->req_uri.port > 0)
	        port = sip->req_uri.port;
	    else
	        port = 5060;
	}
#else
	...
	if (tr != NULL) {
		...
		if (tr->ict_context != NULL)
		    osip_ict_set_destination(tr->ict_context, ipbuf, port);
		if (tr->nict_context != NULL)
		    osip_nict_set_destination(tr->nict_context, ipbuf, port);
#else
		if (tr->ict_context != NULL)
