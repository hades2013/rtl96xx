...
#include <unistd.h>
#endif
//[SD6, ericchung, support get interface ipaddress
#include <sys/ioctl.h>
#include <net/if.h>
...
//[SD6, bohungwu, exosip 3.5 integration
//static jauthinfo_t *eXosip_find_authentication_info(const char *username,
jauthinfo_t *eXosip_find_authentication_info(const char *username,
//]
													const char *realm)
{
...
//[SD6, ericchung, mark for multi account register sip ,the auth must not find other username.
#if 0 
	for (authinfo = eXosip.authinfos; authinfo != NULL; authinfo = authinfo->next) {
		if (realm != NULL && authinfo->realm != NULL)
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_INFO3, NULL,
						"INFO: no match username authinfo: %s %s\n", realm, authinfo->realm));
		if ((authinfo->realm == NULL || authinfo->realm[0] == '\0')
			&& fallback == NULL) {
			fallback = authinfo;
		} else if (realm == NULL
									  ...
									  osip_message_t * last_response)
{
	...
	while (proxyauth != NULL) {
		char *uri;
//[SD6, ericchung, Support anonymous call
#ifdef REDUCE_MALLOC
		if(strcmp(req->from->url.username,"anonymous")==0){
			authinfo = eXosip_find_authentication_info(req->orig_number,
														   proxyauth->realm);
		}else{
		authinfo = eXosip_find_authentication_info(req->from->url.username,
												   proxyauth->realm);
...
int eXosip_get_localip_for(char *address_to_reach,char **loc){
	...
	*loc=osip_malloc(MAXHOSTNAMELEN);
	if (eXosip.eXtl->proto_family==AF_INET)
		strcpy(*loc,"127.0.0.1");  /* always fallback to local loopback */
	else strcpy(*loc,"::1"); 

	memset(&hints,0,sizeof(hints));
	hints.ai_family=(eXosip.eXtl->proto_family==AF_INET) ? PF_INET:PF_INET6;
...
//		eXosip_trace(OSIP_ERROR,("Error in getaddrinfo for %s: %s\n",address_to_reach,gai_strerror(err)));
		printf("Error in getaddrinfo for %s: %d\n",address_to_reach,err);
...
int rcm_get_localip_for_voip(char *interfacename,char **loc){
	...
	*loc=osip_malloc(MAXHOSTNAMELEN);
	...
	if((interfacename==NULL)||(strlen(interfacename)==0)){
		g_warning("rcm_get_localip_for_voip , interfacename is null or length is 0 , use 127.0.0.1\n");
		sprintf(*loc,"%s","127.0.0.1");
		return 0;
	}
	...
	g_message("rcm_get_localip_for_voip , interface is %s\n",interfacename);

	if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
			g_warning("Error openning socket when getting IP address info\n");
			return -1;
	}
	...
	if (ioctl(fd,SIOCGIFADDR,&ifr) < 0) {
		close(fd);
		g_warning("\r\n can't get interface %s ip address \n",interfacename);
		sprintf(*loc,"%s","127.0.0.1");
		return 0;
//		return -1;
	}
	...
	/* display result */
	g_message("rcm_get_localip_for_voip get %s\n", inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr));

	

	sprintf(*loc,"%s",inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr));
...
//[SD6, bohungwu, exosip 3.5 integration 
//]
#if 0 //ericchung: for exosip3.5 not need old api
