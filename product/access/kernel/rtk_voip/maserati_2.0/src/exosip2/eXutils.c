...
_eXosip_srv_lookup(osip_message_t * sip, osip_naptr_t **naptr_record) 
{
	...
	if (MSG_IS_REQUEST(sip)) {
		...
		if (route != NULL) {
			...
			osip_strncpy(host, route->url.host, host_size-1);
#else
			port = 5060;
			...
			if (maddr_param != NULL && maddr_param->gvalue != NULL)
				osip_strncpy(host, maddr_param->gvalue, host_size-1);
#else
			osip_uri_uparam_get_byname(sip->req_uri, "maddr", &maddr_param);
			host = NULL;
			...
			if (host[0] == '\0')
                osip_strncpy(host, sip->req_uri.host, host_size-1);
#else
...
#ifdef REDUCE_MALLOC
		if (maddr != NULL)
			osip_strncpy(host, maddr->gvalue, host_size-1);
		else if (received != NULL)
			osip_strncpy(host, received->gvalue, host_size-1);
		else
			osip_strncpy(host, via->host, host_size-1);
#else
		if (maddr != NULL)
			host = maddr->gvalue;
		else if (received != NULL)
...
#if 0
int
rcm_eXosip_get_ip_by_name(unsigned long *ip, const char *hostname, int service)
{
  struct addrinfo *addrinfo;
  int ret;
  struct sockaddr_in addr;	/* in order to eliminate compiler warning */
  
  if( ( ret = eXosip_get_addrinfo(&addrinfo, ( char * )hostname, service, IPPROTO_UDP) ) )
