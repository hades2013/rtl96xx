	...
	*dest = NULL;
	...
	if (doing_register) {
		...
		if (proxy != NULL && proxy[0] != 0) {	/* equal to a pre-existing route set */
			...
			{
#ifdef REDUCE_MALLOC
				memcpy(&request->req_uri, &o_proxy->url, sizeof(osip_uri_t));
#else
...
#ifdef REDUCE_MALLOC
	if (doing_register && request->from != NULL)
#else
...
#ifdef REDUCE_MALLOC
		memcpy(&new_contact->url, new_contact_url, sizeof(osip_uri_t));	

		if (i == 0 && (*reg)->from != NULL
			&& &(*reg)->from->url != NULL && &(*reg)->from->url.username != NULL) {
			osip_strncpy(new_contact_url->username, (*reg)->from->url.username, strlen(new_contact_url->username)-1);
		}
#else
...
#ifdef REDUCE_MALLOC
			if (firewall_ip[0] != '\0' && (*reg)->req_uri.host[0] != '\0') 
#else
...
#else
#ifdef REDUCE_MALLOC
				osip_strncpy(new_contact_url->host, firewall_ip, strlen(new_contact_url->host)-1);
				osip_strncpy(new_contact_url->port, firewall_port, strlen(new_contact_url->port)-1);
#else
				new_contact_url->host = osip_strdup(firewall_ip);
				new_contact_url->port = osip_strdup(firewall_port);
#endif	//REDUCE_MALLOC
...
#ifdef REDUCE_MALLOC
                osip_strncpy(new_contact_url->host, locip, strlen(new_contact_url->host)-1);
                osip_strncpy(new_contact_url->port, firewall_port, strlen(new_contact_url->port)-1);
#else
				new_contact_url->host = osip_strdup(locip);
				new_contact_url->port = osip_strdup(firewall_port);
