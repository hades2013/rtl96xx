...
eXosip_set_user_agent(const char *user_agent)
{
  ...
  /* append rtk_voip version */
  length = strlen (user_agent) + SIP_VERSION + 3; // space and ()

  ua = (char *) osip_malloc (length + 1);
  sprintf(ua, "%s (%s)",user_agent,SIP_VERSION);
  ua[length] = '\0';

  osip_free(eXosip.user_agent);
  eXosip.user_agent = ua;
}
#endif
...
int eXosip_init(void)
{
	...
	memset(&eXosip, 0, sizeof(eXosip));
	eXosip.pri = 0; 
	eXosip.dscp = 0x1a; /* default dscp must check */

//[SD6, ericchung, exosip 3.5 integration
	//snprintf(eXosip.ipv4_for_gateway, 256, "%s", "217.12.3.11");
	snprintf(eXosip.ipv4_for_gateway, 256, "%s", "127.0.0.1");
	//]
...
//ericchung: default disable keep alive
	eXosip.keep_alive = 0;
