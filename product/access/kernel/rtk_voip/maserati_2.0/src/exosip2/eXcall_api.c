...
int eXosip_call_send_ack(int did, osip_message_t * ack)
{
	...
	if (host==NULL)
	{
		...
		if (route != NULL) {
			...
			port = 5060;
#ifdef REDUCE_MALLOC
			if (ack->req_uri.port > 0)
			    port = ack->req_uri.port;
...
//[SD6, bohungwu. exosip 3.5 integration
//Ported from sal_call_accept_refer() in Linphone 3.3.4
int rcm_eXosip_transfer_send_notify(int jid, int subscription_status, char *body)
{
	osip_message_t *msg=NULL;
	int err=0;
	eXosip_lock();

	err = eXosip_call_build_notify(jid, subscription_status, &msg);
	assert(msg != NULL);
	...
	eXosip_call_send_request(jid, msg);
 ...
 */
int   rcm_eXosip_transfer_call(int jid, const char *refer_to)
{
	//Modification based on sal_call_transfer() in Linphone 3.4.3
	osip_message_t *msg=NULL;
	int err=0;

	eXosip_lock();
	eXosip_call_build_refer(jid,refer_to, &msg);
	if (msg) err=eXosip_call_send_request(jid, msg);
	else err=-1;
	eXosip_unlock();
	return err;
}
 ...
 */
int rcm_eXosip_info_call(int jid, char *content_type, char *body)
{
	osip_message_t *info_msg=NULL;
	int err=0;

	eXosip_lock();
	eXosip_call_build_info(jid,&info_msg);
	if (info_msg){
		osip_message_set_body(info_msg, body, strlen(body));
		osip_message_set_content_type(info_msg, content_type);
		rcm_osip_message_set_content_length(info_msg, strlen(body));
		err = eXosip_call_send_request(jid, info_msg);
	}
	else {
		err=-1;
	}
...
int rcm_eXosip_on_update_call(int jid, int session_version)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *transaction;
	...
	eXosip_call_build_request(jid, "INVITE", &invite);
...
//	eXosip_get_localip_for(jd->d_dialog->remote_contact_uri->url->host, &locip);
#endif
	rcm_get_localip_for_voip(g_pVoIPCfg->voip_interface,&locip);
	...
	if (body)
	{
		rcm_osip_message_set_content_length(invite, strlen(body));
	...
	if(eXosip_call_send_request(jid, invite) != OSIP_SUCCESS)
	{
