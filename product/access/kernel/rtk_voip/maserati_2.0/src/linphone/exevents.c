...
//extern LinphoneTR104Test_t e8c_autotest;
#ifdef CONFIG_RTK_VOIP_TR104
...
int linphone_inc_new_call(LinphoneCore *lc, guint32 ssid, eXosip_event_t *ev)
{
	...
	lc->call[ssid] = linphone_call_new_incoming(lc, ssid, from, to, cid, did);
	//]
	if (lc->call[ssid] == NULL)
	{
		...
		case 200 :
			...
			if (GetSysState(chid) == SYS_STATE_IDLE)
			{
				...
				else
				{
					...
					rcm_tapi_Gen_CID_And_FXS_Ring(chid, 1/*fsk*/, caller_id, NULL, caller_id_name, 0/*type1*/, 0);
				}
...
//			regstatus = REGSTATUS_REG_FAIL_RETRY_L;
		...
		default:
			...
			if(reason==NULL){
				...
				if(cfg->index==0){
					//main proxy register fail.  change to second proxy for register.
...
//		rcm_timerLaunch(lc->register_retry_timer,60*1000);	//60sec
	...
	rcm_linphone_message_to_proxy(lc);
...
//#ifdef MULTI_PROXY
	...
	// For LED
		...
		linphone_update_and_display_registration_status( lc, cfg, REGSTATUS_REG_FAIL );
		rcm_tapi_sip_register(lc->chid, 0);
		...
		case EXOSIP_CALL_MESSAGE_NEW:
			...
			if (ev->request){
				if (MSG_IS_INFO(ev->request)){
					...
					eXosip_call_build_answer(ev->tid,200,&ans);
					if (ans){
						osip_message_set_allow(ans,"INVITE, ACK, BYE, CANCEL, OPTIONS, MESSAGE, SUBSCRIBE, NOTIFY, INFO");
