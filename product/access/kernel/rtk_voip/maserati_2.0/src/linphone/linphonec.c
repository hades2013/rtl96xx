...
//LinphoneTR104Test_t e8c_autotest;
#ifdef CONFIG_RTK_VOIP_TR104
...
//[SD6 ericchung , shold read from FLASH  . TBD
	...
	if(strlen(g_pVoIPCfg->voip_interface)!=0){
		if(rcm_get_localip_for_voip(g_pVoIPCfg->voip_interface,&voip_bound_ipaddress)==-1){
...
int rcm_exosip_init(TAPP *pApp)
{
	...
	if(strlen(g_pVoIPCfg->voip_interface)==0)
		rcm_retreive_voip_if_name(g_pVoIPCfg->voip_interface, DNS_LEN);
	//rcm_voip_flash_if_name(g_pVoIPCfg->voip_interface, DNS_LEN);
	
//	g_message("\r\n g_pVoIPCfg->voip_interface is %s\n",g_pVoIPCfg->voip_interface);
...
void rcm_netlink_handler(int netlink_desc)
{
	...
	for (; NLMSG_OK(nlh, err); nlh = NLMSG_NEXT(nlh, err))
	{
		...
		case RTM_NEWADDR:
			...
			g_message("voip_interface is (%d): %s\n",strlen(g_pVoIPCfg->voip_interface),g_pVoIPCfg->voip_interface);

			if(strlen(g_pVoIPCfg->voip_interface)==0)
				rcm_retreive_voip_if_name(g_pVoIPCfg->voip_interface, DNS_LEN);
	
			rcm_get_localip_for_voip(g_pVoIPCfg->voip_interface,&voip_new_ip);
		...
		case RTM_NEWLINK:
			...
			printf("Maserati: Receive link status change event :RTM_NEWLINK/RTM_DELLINK\n");

				rcm_retreive_voip_if_name(voip_if_name_new, DNS_LEN);

				g_message("VoIP WAN interface, original=%s, new=%s\n", g_pVoIPCfg->voip_interface, voip_if_name_new);
				if(strcmp(g_pVoIPCfg->voip_interface, voip_if_name_new) !=0)
				{
...
#endif
#if 1	
	//for BaudTec project, only support G711U G711A G729 codec 
	g_mapSupportedCodec[_CODEC_G711U] = idx++;
	g_mapSupportedCodec[_CODEC_G711A] = idx++;
	g_mapSupportedCodec[_CODEC_G729] = (g_VoIP_Feature & CODEC_G729_SUPPORT) ? idx++ : _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G723] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G726_16] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G726_24] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G726_32] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G726_40] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_GSMFR] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_ILBC] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G722] =  _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_SPEEX_NB] = _CODEC_MAX;
#endif
#if 0
	//for E8C project, only support 4 codec 
	g_mapSupportedCodec[_CODEC_G711U] = idx++;
	g_mapSupportedCodec[_CODEC_G711A] = idx++;
	g_mapSupportedCodec[_CODEC_G729] = (g_VoIP_Feature & CODEC_G729_SUPPORT) ? idx++ : _CODEC_MAX;
	g_mapSupportedCodec[_CODEC_G723] =  _CODEC_MAX;
...
void rcm_configure_restart(void *arg){
	...
	printf("rcm_configure_restart \n");
	for (i=0; i<g_MaxVoIPPorts; i++)
	{
		if(GetSysState(i)!=SYS_STATE_IDLE){
			printf("\r\n rcm_configure_restart 002, delay 3 sec\n");
...
/* for ctype wan, always get interface again */
	if(strlen(g_pVoIPCfg->voip_interface)==0)
#endif		
		rcm_retreive_voip_if_name(g_pVoIPCfg->voip_interface, DNS_LEN);
...
//		rcm_voip_flash_if_name(g_pVoIPCfg->voip_interface, DNS_LEN);
	...
	//printf("\r\n %s(%d) g_pVoIPCfg->voip_interface is %s\n",__FUNCTION__,__LINE__,g_pVoIPCfg->voip_interface);
}
...
/* for test */
	...
	strcpy(g_pVoIPCfg->voip_interface , sim_iface);
	//rcm_dp("sim_ifp=%s sim_iface=%s\n" , sim_ifp , g_pVoIPCfg->voip_interface);
