...
unsigned long g_SystemDebug = DEBUG_B_ERROR | DEBUG_B_WARNING | DEBUG_B_MESSAGE;
...
#if 0
void g_print(int type, const gchar *fmt, ...)
#else
void g_print(char *file, int line, char *func, int type, const gchar *fmt, ...)
#endif
{
	va_list args;
	char msg[DBG_PRINT_MAX];
	char full_msg[DBG_PRINT_MAX];
	int level;
	int size;
	...
	size = snprintf(full_msg, DBG_PRINT_MAX, "<%s:%d> %s", file, line, msg);
	rcm_voiplog_write(LOG_CALL_MGNT, full_msg);
