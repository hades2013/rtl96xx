...
void linphone_proxy_config_register(LinphoneProxyConfig *obj){
	...
	//ToDo: contact parameter
	obj->rid=eXosip_register_build_initial_register((gchar*)obj->reg_identity,obj->reg_proxy,obj->contact,obj->expires,&reg_msg);
	//]
...
//re-register again , if already send register before
void linphone_proxy_send_register_again(LinphoneProxyConfig *obj){
	obj->frozen=TRUE;
	if ((obj->reg_sendregister)&&(obj->registered)){
		eXosip_lock();
		eXosip_register_send_register(obj->rid, NULL);
		eXosip_unlock();
	}
}
...
void linphone_proxy_config_done(LinphoneProxyConfig *obj)
{
	...
	gchar *ct=NULL;
	int err=0;
	voipCfgPortParam_t *voip_ptr = &g_pVoIPCfg->ports[obj->lc->chid];
	voipCfgProxy_t *proxy;

	proxy = &voip_ptr->proxies[obj->index];
	//]
	...
	obj->frozen=FALSE;
...
#ifdef REDUCE_MALLOC
			osip_strncpy(identity->url.host, obj->lc->net_conf.nat_address, hostSize-1);

			/* Mandy add for stun support */
			identity->url.port = obj->lc->net_conf.nat_port;
#else
		...
		strcpy(obj->contact,ct);
g_message(" obj->contact  %s\n",obj->contact);
		if (eXosip_add_authentication_info
			(proxy->number, proxy->login_id, proxy->password, NULL, NULL)) {
			g_warning("eXosip_add_authentication_info failed");
			
		}

#if 0
		eXosip_lock ();
		obj->rid=eXosip_register_build_initial_register((gchar*)id_str,obj->reg_proxy,ct,obj->expires,&msg);
		if (msg){
			g_message("[eXosip_register_build_initial_register ok.] File %s ,API %s , Line %d\n",__FILE__,__FUNCTION__,__LINE__);
				

		}else{
			g_error("Could not build initial register.\r\n");
			g_error("[Could not build initial register.] File %s ,API %s , Line %d\n",__FILE__,__FUNCTION__,__LINE__);
			eXosip_unlock();
			return;
		}

//		osip_message_set_supported (msg, "100rel");
		err=eXosip_register_send_register(obj->rid,msg);
...
#ifdef OLD_API //eric not need send stun
...
LinphoneProxyConfig *linphone_core_get_proxy_config_from_rid(LinphoneCore *lc, int rid){
	...
	for(elem=linphone_core_get_proxy_config_list(lc);elem!=NULL;elem=g_list_next(elem)){
		LinphoneProxyConfig *cfg=(LinphoneProxyConfig*) elem->data;
		if (cfg->reg_sendregister && cfg->rid==rid)
		{//match
