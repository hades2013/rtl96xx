...
#ifdef SUPPORT_G7111
PayloadType payload_type_g711u_wb={
	TYPE(PAYLOAD_AUDIO_CONTINUOUS),
	CLOCK_RATE(16000),
	NORMAL_BITRATE(64000),
	MIME_TYPE("PCMU-WB")
...
PayloadType payload_type_g711a_wb={
	TYPE(PAYLOAD_AUDIO_CONTINUOUS),
	CLOCK_RATE(16000),
	NORMAL_BITRATE(64000),
	MIME_TYPE("PCMA-WB")
...
const profile_list_t profileList[] = {
	...
	{	/* G711U_WB */
		_CODEC_G711U_WB,
	    rtpPayloadPCMU_WB,
		&payload_type_g711u_wb,
		1
	},
	{	/* G711A_WB */
		_CODEC_G711A_WB,
	    rtpPayloadPCMA_WB,
		&payload_type_g711a_wb,
		1
	},
#endif
	{	/* telephone-event */
...
#ifdef SUPPORT_CUSTOMIZE_FRAME_SIZE	//Alex, 20111019
int get_list_number_with_pt(int payload){
	int i, nMax, result=_CODEC_MAX;
    nMax = (sizeof(profileList)/sizeof(profileList[0]));

    for(i=0; i<nMax; i++){
        //printf("checking with payload %d profileList[%d]=%d\n", payload, i, profileList[i].pt);
		if(profileList[i].pt==payload)
        {
        	//printf("Query with payload number %d result is %d \n", payload, profileList[i].nSupportedCodec);
            result = profileList[i].nSupportedCodec;
            break;
        }
        else
            continue;
