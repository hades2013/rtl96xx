	      ...
	      if (payload != NULL)
		{
		  ...
		  if (my_payload != NULL)	/* payload is supported */
		      ...
		      if (0 == ret)
			{
			  ...
			  // rock add:
			  // 1. confirm 1 codec only
			  // 2. add telephone event
			  if (context->dtmf_mode != 2)
			  {
			  	sdp_message_m_payload_add(*dest, i, osip_strdup("101"));
			  	sdp_message_a_attribute_add(*dest, i, osip_strdup("rtpmap"), osip_strdup("101 telephone-event/8000"));
			  	sdp_message_a_attribute_add(*dest, i, osip_strdup("fmtp"), osip_strdup("101 0-11"));
			  }
			  break;
			}
		    }
...
int
osip_negotiation_sdp_message_put_on_fax (sdp_message_t * sdp, int port)
{
	sdp_media_t *media;
	int pos;
	...
	if( 0 == port ){
		...
		// remove all payloads
		payload = (char *) osip_list_get_first(&media->m_payloads, &iterator);
		while (osip_list_iterator_has_elem(iterator))
		{
			osip_free(payload);
			payload = (char *) osip_list_iterator_remove(&iterator);
		}

		// remove all attributes
		attr = (sdp_attribute_t *) osip_list_get_first(&media->a_attributes, &iterator);
		while (osip_list_iterator_has_elem(iterator))
		{
			sdp_attribute_free(attr);
			attr = (sdp_attribute_t *) osip_list_iterator_remove(&iterator);
		}
	
		// add G.711-ulaw codec only
		/*++added by Jack Chan 16/02/07 for T.38++*/
		if( NULL != media->m_proto ){
			if( 0 == osip_strcasecmp(media->m_media,"image") ){ /* re-INVITE G.711 for T.38 fail*/
				osip_list_remove(&sdp->m_medias, 0);
				sdp_message_m_media_add(sdp,osip_strdup("audio"),osip_strdup(media->m_port),NULL,osip_strdup("RTP/AVP"));
		...
		/*++SDP message for T.38 add by Jack Chan 22/01/07++*/
		/* remove the media */
		osip_list_remove(&sdp->m_medias, 0);
		sdp_media_free(media);
		/* add the T.38 attribute */
		sdp_message_m_media_add(sdp, osip_strdup("image"), int_2char(port), NULL, osip_strdup("udptl"));
		sdp_message_m_payload_add(sdp, 0, osip_strdup("t38"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxVersion"), osip_strdup("0"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38maxBitRate"), osip_strdup("14400"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxFillBitRemoval"), osip_strdup("0"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxTranscodingMMR"), osip_strdup("0"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxTranscodingJBIG"), osip_strdup("0"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxRateManagement"), osip_strdup("transferredTCF"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxMaxBuffer"), osip_strdup("72"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxMaxDatagram"), osip_strdup("316"));
		sdp_message_a_attribute_add(sdp, 0, osip_strdup("T38FaxUdpEC"), osip_strdup("t38UDPRedundancy"));
	}
	/*--end--*/
	return 0;
}
...
#if 1
      // move global attribute to local attribute
      if (recv_send == -1)
...
int
osip_negotiation_sdp_message_put_media_mode(sdp_message_t *sdp, int media_mode)
{
	int pos_media;
	sdp_connection_t *conn;

	pos_media = -1; // session layer check first
	while (!sdp_message_endof_media(sdp, pos_media))
	{
		char *rcvsnd;
		int i, pos, found;
		char *sz_media_modes[MEDIA_MODES] = {
			"sendrecv",
			"sendonly",
			"recvonly",
			"inactive"
		...
		while (rcvsnd != NULL)
		{
			for (i=0; i<MEDIA_MODES && !found; i++)
			{
				if (strcmp(rcvsnd, sz_media_modes[i]) != 0)
					continue;
				...
				{
					found = 1;
					sprintf(rcvsnd, sz_media_modes[media_mode]);
				}
			}
			pos++;
			rcvsnd = sdp_message_a_att_field_get(sdp, pos_media, pos);
		...
		// force add sendrecv attribute in media layer if not found
		if (pos_media >= 0 && !found)
		{
        	sdp_message_a_attribute_add(sdp, pos_media, 
				osip_strdup(sz_media_modes[media_mode]), NULL);
		}

		// rock: mix 2543 & 3264 hold
		if (media_mode != MEDIA_SENDRECV)
		{
			conn = sdp_message_connection_get(sdp, pos_media, 0);
			if (conn)
			{
				osip_free(conn->c_addr);
				conn->c_addr = osip_strdup("0.0.0.0");
      	...
      	pos_media++; // pos_media >=0 is media layer
	}
