...
int __osip_ict_init(osip_ict_t ** ict, osip_t * osip, osip_message_t * invite)
{
	...
	*ict = (osip_ict_t *) osip_malloc(sizeof(osip_ict_t));
...
#endif	//REDUCE_MALLOC
	{
		...
		if (route->url.port > 0)
		    port = route->url.port;
		osip_ict_set_destination((*ict), route->url.host, port);
#else
...
#ifdef REDUCE_MALLOC
		if (invite->req_uri.port > 0)
	    	port = invite->req_uri.port; 
   		osip_uri_uparam_get_byname(&invite->req_uri, "maddr", &maddr_param);
		if (maddr_param != NULL && maddr_param->gvalue != NULL)
			osip_ict_set_destination((*ict), maddr_param->gvalue,
									 port);
		else
			osip_ict_set_destination((*ict), invite->req_uri.host,
									 port);
#else
		port = 5060;
...
int osip_ict_set_destination(osip_ict_t * ict, char *destination, int port)
{
    ...
    if (ict->destination != NULL)
		osip_strncpy(ict->destination, destination, sizeof(ict->destination)-1);
#else
	if (ict->destination != NULL)
		osip_free(ict->destination);
