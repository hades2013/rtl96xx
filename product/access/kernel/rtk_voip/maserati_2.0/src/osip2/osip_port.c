...
//[SD6, bohungwu, debugging log unification
...
osip_trace(char *fi, int li, osip_trace_level_t level, FILE * f, char *chfr, ...)
{
	...
	va_list ap;
	int relative_time = 0;
	char console_msg[6000];//for console debug usage
...
//printf("g_SystemDebug is %x\n",g_SystemDebug);
	if( ( g_SystemDebug & DEBUG_B_CONSOLE ) ){
	
		va_start(ap, chfr);
		vsprintf(console_msg, chfr, ap);
		va_end(ap);
	...
	default:		lev = ANDROID_LOG_DEFAULT;	break;
		...
		memset(buffer, 0, sizeof(buffer));
		if (level == OSIP_FATAL)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "| FATAL | %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_BUG)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "|  BUG  | %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_ERROR)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "| ERROR | %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_WARNING)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "|WARNING| %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_INFO1)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "| INFO1 | %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_INFO2)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "| INFO2 | %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_INFO3)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "| INFO3 | %i <%s: %i> ", relative_time,
						   fi, li);
		else if (level == OSIP_INFO4)
			in = snprintf(buffer, MAX_LENGTH_TR-1, "| INFO4 | %i <%s: %i> ", relative_time,
						   fi, li);
		...
		rcm_voiplog_write(LOG_OSIP_EVENT, buffer);
#endif
	}
#endif
