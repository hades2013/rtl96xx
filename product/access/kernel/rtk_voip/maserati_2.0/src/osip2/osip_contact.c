...
/* returns -1 on error. */
int osip_contact_parse(osip_contact_t * contact, const char *hvalue)
{
	if (contact == NULL)
		return OSIP_BADPARAMETER;
	if (strncmp(hvalue, "*", 1) == 0) {
#ifdef REDUCE_MALLOC
		osip_strncpy(contact->displayname, hvalue, strlen(contact->displayname)-1);
#else
...
#ifndef MINISIZE
int osip_contact_init(osip_contact_t ** contact)
{
...
int osip_contact_clone(const osip_contact_t * contact, osip_contact_t ** dest)
{
