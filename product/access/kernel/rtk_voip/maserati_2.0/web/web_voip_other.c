...
#endif
{
	...
	else if (strcmp(argv[0], "sip_disable_prack") == 0)
	{
		printf("pVoIPCfg->rfc_flags is %x\n",pVoIPCfg->rfc_flags);
			boaWrite(wp, "%s", pVoIPCfg->rfc_flags & SIP_DISABLE_PRACK ? "checked" : "");
	}
	...
	else if (strcmp(argv[0], "hookflash_type")==0)
	{
		for (i=0; i < 2 ;i++)
		{
			if (i == (pVoIPCfg->rfc_flags & SIP_FLASHHOOK_INFO))
				boaWrite(wp, "<option selected>%s</option>", hookflash_type[i]);
			else
				boaWrite(wp, "<option>%s</option>", hookflash_type[i]);
		}
	}	
...
//eric merge
	...
	if (gstrcmp(boaGetVar(wp, "sip_disable_prack", ""), "on") == 0){
...
//support flash hook by SIP INFO
	ptr = boaGetVar(wp, "hookflash_type", "");
