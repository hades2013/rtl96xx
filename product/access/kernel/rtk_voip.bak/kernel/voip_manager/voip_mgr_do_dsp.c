...
int do_mgr_VOIP_MGR_ON_HOOK_RE_INIT( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern unsigned char lec_g168_cng_flag_bk[];
	extern short Attack_Stepsize_bk[];
	extern short Release_Stepsize_bk[];
	RtkLecObj.EC_G168ReInit(stVoipCfg.ch_id);
	RtkLecObj.EC_G168NlpInit( chid, Attack_Stepsize_bk[chid], Release_Stepsize_bk[chid], lec_g168_cng_flag_bk[chid] );
	NR_re_init(stVoipCfg.ch_id);
#endif
...
//#ifdef EXPER_AEC
	if ( (stVoipValue.value5 < 32) && (RtkLecObj.ec_select) )	{	// tail length of 128ms lec min. value is 32
		PRINT_Y("Warning: ec128 tail length support min. 32ms tail length, ch%d\n", stVoipValue.ch_id);
		RtkLecObj.EC_G168SetTailLength(stVoipValue.ch_id, 32);
	#ifdef CONFIG_RTK_VOIP_DRIVERS_IP_PHONE
		RtkLecObj.EC_G168SetTailLength(3, 32);// for handset.
	#endif
	} else {
		if ( stVoipValue.value5 <= 0 )
			stVoipValue.value5 = 2;
		RtkLecObj.EC_G168SetTailLength(stVoipValue.ch_id, stVoipValue.value5);
	#ifdef CONFIG_RTK_VOIP_DRIVERS_IP_PHONE
		RtkLecObj.EC_G168SetTailLength(3, stVoipValue.value5);// for handset.
	#endif
	}
	RtkLecObj.EC_G168NlpInit(stVoipValue.ch_id, stVoipValue.value2, stVoipValue.value3, stVoipValue.value4);
	
//#endif

#ifdef SUPPORT_LEC_G168_ISR
	#ifdef CONFIG_RTK_VOIP_DRIVERS_IP_PHONE
	RtkLecObj.EC_G168SetTailLength(stVoipValue.ch_id, 16);
		#ifdef T_TYPE_ECHO_CAN
	RtkLecObj.EC_G168Init(stVoipValue.ch_id, LEC|nlp|nlp_mode);	// for handfree channel
	RtkLecObj.EC_G168SetTailLength(3, 16);	// for handset's echo path channel
	RtkLecObj.EC_G168Init(3, LEC|nlp|nlp_mode);	// for handset
		#else
	RtkLecObj.EC_G168Init(stVoipValue.ch_id, LEC|nlp|LEC_NLP_CNG);
		#endif
	#else
	RtkLecObj.EC_G168Init(stVoipValue.ch_id, LEC|nlp|nlp_mode);
	#endif
	...
	Attack_Stepsize_bk[ stVoipValue.ch_id ] = stVoipValue.value2;
	Release_Stepsize_bk[ stVoipValue.ch_id ] = stVoipValue.value3;
	lec_g168_cng_flag_bk[ stVoipValue.ch_id ] = stVoipValue.value4;	
#endif
 	restore_flags(flags);
#endif
...
#ifdef SUPPORT_LEC_G168_ISR
	{
		RtkLecObj.EC_G168Enable(stVoipCfg.ch_id);
#ifdef CONFIG_RTK_VOIP_DRIVERS_IP_PHONE
		#ifdef T_TYPE_ECHO_CAN
		RtkLecObj.EC_G168Enable(3);// for handset.
		#endif
#endif
	}
	else if (stVoipCfg.enable == 0)
	{
		RtkLecObj.EC_G168Disable(stVoipCfg.ch_id);
#ifdef CONFIG_RTK_VOIP_DRIVERS_IP_PHONE
		#ifdef T_TYPE_ECHO_CAN
		RtkLecObj.EC_G168Disable(3);// for handset.
		#endif
#endif
	}
...
#else
        extern LecObj_t RtkLecObj;
	RtkLecObj.EC_G168VbdAuto(stVoipCfg.ch_id, stVoipCfg.cfg, stVoipCfg.cfg2, stVoipCfg.cfg3);
#endif
	return 0;
}
#else
...
int do_mgr_VOIP_MGR_GET_EC_DEBUG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	if (stVoipEcDebug.mode&0x1) {//stVoipLecDebug.mode = 1, do ec
		...
		if (ec_test_mode==2) {
			...
			RtkLecObj.EC_G168Process( 3, input, stVoipEcDebug.buf2, stVoipEcDebug.buf1)
			...
			if( RtkLecObj.EC_G168Process == NULL ) 
				PRINT_R("Error !!! %s not supported !!!\n", RtkLecObj.EC_G168Process);
			else
				RtkLecObj. EC_G168Process( 3, input, stVoipEcDebug.buf2, stVoipEcDebug.buf1 );			
		//
		}
		...
		if (stVoipEcDebug.mode&INIT_MODE_CLEAN_H_REGISTER)
			...
			RtkLecObj.EC_G168SetTailLength(3, 128);
			RtkLecObj.EC_G168Init(3, 1);
			RtkLecObj.EC_G168ReInit(3);
		}
    #endif
		ec_test_mode = 2;
	} else {//stVoipEcDebug.mode = 0, do ec init
		RtkLecObj.EC_G168SetTailLength(3, 16);
		RtkLecObj.EC_G168Init( 3, LEC|LEC_NLP|LEC_NLP_MUTE);
		RtkLecObj.EC_G168Init = 0;
	}
#endif
	return 0;
}
...
int do_mgr_VOIP_MGR_SET_COUNTRY( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	if (stVoipValue.value == DSPCODEC_COUNTRY_JP) // Japan
	{
		for (chid=0; chid<CON_CH_NUM; chid++) {
			...
			RtkLecObj.EC_G168NlpConfig(chid, 5);
	...
	else
	{
		for (chid=0; chid<CON_CH_NUM; chid++) {
			...
			RtkLecObj.EC_G168NlpConfig(chid, 18);
#else
			RtkLecObj.EC_G168NlpConfig(chid, 21);
#endif
		}
	}
#endif
