...
#ifdef AIPC_DEBUG
#define DPRINT(fmt, args...) printf("%s(%d) "fmt, __FUNCTION__, __LINE__ ,## args)
#else
...
int aipc_write_length(int dev_fd , void *dst_addr , void *src_addr , unsigned long length , int padding)
{
	...
	if (!padding)
		ioctl(dev_fd , IOCTL_COPY);
	else
		ioctl(dev_fd , IOCTL_COPY_PADDING);
...
int aipc_write_input(int dev_fd , int ifd , void *addr)
{
	...
	rc = read(ifd , read_buf , sb.st_size);
	wc = write(dev_fd , read_buf , sb.st_size);

	DPRINT("size=%lu rc=%lu wc=%lu\n" , sb.st_size , rc , wc);
	return 0;
}
...
#if 1	//word display
	unsigned long *ulp = (unsigned long *)&read_buf;
	i=0;
	do{
		printf("%08lx ", ulp[i]);
		if((i+1)%4==0)
...
{
	ioctl(dev_fd , ioctl_num , ioctl_flag);
	return OK;
}
...
int main( int argc, char *argv[])
{
	...
	if (ioctl_num){
		aipc_send_ioctl(dev_fd , ioctl_num , ioctl_flag );
	}
	else if (dst_addr){
		if (wr){
			if (ifd>0){
				aipc_write_input(dev_fd , ifd , dst_addr);
				sync();
			}
			else if (length && src_addr){
				aipc_write_length(dev_fd , dst_addr , src_addr , length , padding);
				sync();
			}
			else if (op_word){	
				aipc_write_word(dev_fd , word , dst_addr);
				sync();
			}
			else if (op_and || op_or || op_xor || op_not){
				if (op_and)
					aipc_write_bitop(dev_fd , and , dst_addr , BOP_AND);
				if (op_or)
					aipc_write_bitop(dev_fd , or  , dst_addr , BOP_OR);
				if (op_xor)
					aipc_write_bitop(dev_fd , xor , dst_addr , BOP_XOR);
				if (op_not)
					aipc_write_bitop(dev_fd , not , dst_addr , BOP_NOT);
			...
			if ((ofd>0) && length){
				aipc_read_ram(ofd , dev_fd , dst_addr , length);
				sync();
			}
			else if (length){
				aipc_read_put_cnsl(dev_fd , dst_addr , length);
				sync();
			}
			else{
				aipc_read_put_cnsl(dev_fd , dst_addr , sizeof(unsigned long));
				sync();
			}
		}else{
			display_help();
