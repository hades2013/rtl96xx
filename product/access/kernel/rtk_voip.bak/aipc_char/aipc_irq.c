...
//static DEFINE_SPINLOCK(aipc_lock);
	...
	if( add_hiq ){
		while(1){
			...
			if( aipc_int_2dsp_hiq_full() ){
					...
					//spin_unlock_irqrestore( &aipc_lock , state );
					#endif
					continue;			
				}
				else{
...
#else // for initiall uboot test version
...
aipc_cpu_ISR( int irq , void *data )
{
	...
	//printk("aipc: %s %d irq=%d data=%p\n" , __FUNCTION__ , __LINE__ , irq , data);

	aipc_int_mask( irq );
	aipc_int_deassert( T_CPU );

#if 0	//burst write 4MB to uncahced memory
	int i ;
	//for(i=0 ; i< 0xffff ; i++)
	for(i=0 ; i< 1 ; i++)
	{
		memset(0xa3000000 , 0xa5a5a5a5 , 0x400000);
		memset(0xa3000000 , 0x5a5a5a5a , 0x400000);
	}
#endif

	status = aipc_inl(R_GISR0_0);
...
aipc_cpu_ISR( int irq , void *data )
{
	u32_t fid = AIPC_MGR_2CPU_MBOX_RECV;
	u32_t status = 0;
...
{
#ifndef DSP_DSR_SUPPORT		// or receive in DSR aipc_dsp_DSR()
	void *mp = NULL;
	
	mp = aipc_data_2dsp_recv();

	if(mp){
		ADEBUG(DBG_CONT , "mbox receive from ISR\n");
