...
{
	struct net_device *ndev = OPL_NULL;

	if (OPL_NULL == name || OPL_NULL == mac)
	{
		return OPL_ERROR;
	}
	ndev = dev_get_by_name(name);
	...
	OPL_MEMCPY(mac, ndev->dev_addr, ndev->addr_len);
