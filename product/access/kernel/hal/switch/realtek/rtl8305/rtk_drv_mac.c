...
{        
    ...
    {
        /*Enable mac learning*/
        rtkApiRet = rtk_l2_limitLearningCnt_set(PHY_CPU_PORTID, RTK_MAX_NUM_OF_LEARN_LIMIT);
        if (RT_ERR_OK != rtkApiRet)
        {
            return DRV_SDK_GEN_ERROR;
        }
        /*Forward packet when learning mac num exceeds the limited num.*/
        rtkApiRet = rtk_l2_limitLearningCntAction_set(PHY_CPU_PORTID, LIMIT_LEARN_CNT_ACTION_FORWARD);
        if (RT_ERR_OK != rtkApiRet)
        {
            return DRV_SDK_GEN_ERROR;
        }
    ...
    else if(MAC_LEARN_FWD_DISABLED == ulEnable)
    {
        ...
        rtkApiRet = rtk_l2_limitLearningCnt_set(PHY_CPU_PORTID, 0);
        if (RT_ERR_OK != rtkApiRet)
        {
            return DRV_SDK_GEN_ERROR;
        }
...
{        
    ...
    LgcPortFor(lport)
    {
        if(TstLgcMaskBit(lport, &stPortMask))
        {
            ...
            {
                /*Enable mac learning*/
                rtkApiRet = rtk_l2_limitLearningCnt_set(PortLogic2PhyPortId(lport), RTK_MAX_NUM_OF_LEARN_LIMIT);
                if (RT_ERR_OK != rtkApiRet)
                {
                    return DRV_SDK_GEN_ERROR;
                }
                /*Forward packet when learning mac num exceeds the limited num.*/
                rtkApiRet = rtk_l2_limitLearningCntAction_set(PortLogic2PhyPortId(lport), LIMIT_LEARN_CNT_ACTION_FORWARD);
                if (RT_ERR_OK != rtkApiRet)
                {
                    return DRV_SDK_GEN_ERROR;
                }
            ...
            {
                /*Disable mac learning*/
                rtkApiRet = rtk_l2_limitLearningCnt_set(PortLogic2PhyPortId(lport), 0);
                if (RT_ERR_OK != rtkApiRet)
                {
                    return DRV_SDK_GEN_ERROR;
                }
                /*Drop packet when learning mac num exceeds the limited num.*/
                rtkApiRet = rtk_l2_limitLearningCntAction_set(PortLogic2PhyPortId(lport), LIMIT_LEARN_CNT_ACTION_DROP);
                if (RT_ERR_OK != rtkApiRet)
                {
                    return DRV_SDK_GEN_ERROR;
                }
...
{    
    ...
    rtkApiRet = rtk_l2_limitLearningCnt_get(PortLogci2ChipId(ulLgcPort), &rtkMacCnt);
    if (RT_ERR_OK != rtkApiRet)
    {
        return DRV_SDK_GEN_ERROR;
    }
...
DRV_RET_E Hal_SetMacCpuMacAdd(vlan_id_t tdVid, mac_address_t mac_address)
{
    ...
    tdVlanId = tdVid;
    memcpy(stMacAdd.octet, mac_address, sizeof(mac_address_t));

    memset(&stUcastMacAdd, 0, sizeof(stUcastMacAdd));
    stUcastMacAdd.cvid = tdVlanId;
    stUcastMacAdd.fid = 0;
    stUcastMacAdd.efid = 0;
    stUcastMacAdd.ivl = DISABLED;
    stUcastMacAdd.is_static = ENABLED;
    stUcastMacAdd.port = PHY_CPU_PORTID;

    /*The following opration will renew the existed LUT entry.*/
    if(RT_ERR_OK != rtk_l2_addr_add(&stMacAdd, &stUcastMacAdd))
    {
...
/* 014045 */
    ...
    #else
        if(!VALID_PORT(stMacDelete.port))
    #endif
/* 014045 */
        {
            return DRV_ERR_PARA;
...
DRV_RET_E Hal_SetMacAddUcastMac(mac_ucast_t stMacUcast)
{
    ...
    memcpy(stMacAdd.octet, stMacUcast.mac_addr, sizeof(stMacAdd));
    
    /*set struct*/
    memset(&ucast_addr, 0, sizeof(rtk_l2_ucastAddr_t));
    ucast_addr.cvid = (rtk_uint32)stMacUcast.tdVid;
    ucast_addr.fid = 0;
    ucast_addr.efid = 0;
    ucast_addr.ivl = DISABLED;
    memcpy(ucast_addr.mac.octet, stMacUcast.mac_addr, sizeof(rtk_mac_t));

    if(stMacUcast.ucStatic)
    {
        ucast_addr.is_static = ENABLED;
    }
    ...
    /*The following opration will renew the existed LUT entry.*/
    if(RT_ERR_OK != rtk_l2_addr_add(&stMacAdd, &ucast_addr))
    {
        return DRV_ERR_UNKNOW;
    }
...
DRV_RET_E Hal_SetMacDelUcastMac(mac_ucast_t stMacUcast)
{
    ...
    stUniLutEntry.cvid = stMacUcast.tdVid;
    stUniLutEntry.efid = 0;
    stUniLutEntry.fid  = 0;
    stUniLutEntry.ivl  = DISABLED;
    
    memcpy(&(stUniLutEntry.mac), &stMacAdd, sizeof(rtk_mac_t));
    if(RT_ERR_OK != rtk_l2_addr_del(&stMacAdd, &stUniLutEntry))
    {
        return DRV_SDK_GEN_ERROR;
    }
...
#else
        if(0 == _mac_mcast_entry_dump[i].tdVid)
#endif
        {
...
DRV_RET_E Hal_SetMacAddMcastMac(mac_mcast_t stMacMcast)
{
    ...
    rtk_api_ret_t rtkApiRet;
    rtk_mac_t stMacAddr;
    rtk_portmask_t stPortMask;

    MaskLogic2Phy(&(stMacMcast.port_mask), &phy_mask);

    memset(&stPortMask, 0, sizeof(stPortMask));
    memcpy(stMacAddr.octet, stMacMcast.mac_addr, ETHER_ADDR_LEN);

    UnitFor(unit)
    {
        memcpy(&stPortMask, &phy_mask.pmask[unit], sizeof(stPortMask));
		/* 014002 */		
		#if defined(CHIPSET_RTL8305)
		/*use svl fid 0 */
        rtkApiRet = rtk_l2_mcastAddr_add(&stMacAddr, DISABLE, 0, stPortMask);
		#else
        rtkApiRet = rtk_l2_mcastAddr_add(&stMacAddr, ENABLE, stMacMcast.tdVid, stPortMask);
		#endif
        if (RT_ERR_OK != rtkApiRet)
        {
            return DRV_SDK_GEN_ERROR;
...
/*end modified by liaohongjun 2012/7/30 of QID0001*/
    ...
    memcpy(stMacAddr.octet, stMacMcast.mac_addr, sizeof(rtk_mac_t));
...
#if defined(CHIPSET_RTL8305)
        /*use svl fid 0 */
        if(RT_ERR_OK != rtk_l2_mcastAddr_del(&stMacAddr, DISABLE, 0))
#else
        if(RT_ERR_OK != rtk_l2_mcastAddr_del(&stMacAddr, ENABLE, stMacMcast.tdVid))
#endif
/* 014010 */
        {
            diag_printf("\nfunc:%s,line:%d\n",__FUNCTION__,__LINE__);
...
{
    if(RT_ERR_OK != rtk_l2_aging_set((rtk_l2_age_time_t)ulSecond))
    {
        return DRV_ERR_PARA;
    }
...
DRV_RET_E Hal_SetMacLearnLimit(UINT32 ulLgcPort, UINT32 uLlimit, UINT32 ulDisForward)
{
    ...
    if(DIS_ARL_LEARN_LIMIT == uiMacCnt)
    {
        ...
        uiMacCnt = RTK_MAX_NUM_OF_LEARN_LIMIT;
    }
    
    if(RT_ERR_OK != rtk_l2_limitLearningCnt_set(port, (rtk_mac_cnt_t)uiMacCnt))
    {
        return DRV_ERR_UNKNOW;
    }
    ...
    if(RT_ERR_OK != rtk_l2_limitLearningCntAction_set(port, action))
    {
        return DRV_ERR_UNKNOW;
    }
...
DRV_RET_E Hal_GetMacUcastMac(vlan_id_t tdVid, mac_address_t mac_addr, mac_ucast_t *pstMacUcast)
{
    ...
    stUcastAddr.cvid = tdVid;
    stUcastAddr.ivl = DISABLED;
    stUcastAddr.fid = 0;
    stUcastAddr.efid = 0;
    ...
    memcpy(stRtkMac.octet, mac_addr, ETHER_ADDR_LEN);
    
    UnitFor(unit)
    {
        if(RT_ERR_OK == rtk_l2_addr_get(&stRtkMac,&stUcastAddr))
        {
...
DRV_RET_E Hal_GetMacMcastMac(vlan_id_t tdVid, mac_address_t mac_addr, mac_mcast_t *pstMacMcast)
{
    ...
    memcpy(stRtkMacAddr.octet, mac_addr, ETHER_ADDR_LEN);
    memcpy(pstMacMcast->mac_addr,mac_addr,sizeof(mac_address_t));
    UnitFor(unit)
    {
    /* 014010 */
    #if defined(CHIPSET_RTL8305)
        if(RT_ERR_OK == rtk_l2_mcastAddr_get(&stRtkMacAddr, DISABLE, 0, &stRtkPortMask))
    #else
        if(RT_ERR_OK == rtk_l2_mcastAddr_get(&stRtkMacAddr, ENABLED, tdVid, &stRtkPortMask))
    #endif
    /* 014010 */
        {
            memcpy(&(stPhyMask.pmask[unit]), &(stRtkPortMask), sizeof(rtk_portmask_t));
            ucGetMac = 1;
        }
    }
...
/* 014533 */
        ...
        rtkApiRet = rtk_l2_mcastAddr_add(&stMacAddr, DISABLE, 0, stPortMask);
#else
        rtkApiRet = rtk_l2_mcastAddr_add(&stMacAddr, ENABLED, stMacMcast.tdVid, stPortMask);
#endif
/* 014533*/
        if (RT_ERR_OK != rtkApiRet)
        {
...
/* */
VLAN_FID_S m_astVlanFid[RTL8367B_FIDMAX+1];
/*****************************************************************************
...
DRV_RET_E Hal_SetFidByVid(UINT32 uiVid, UINT32 uiFid, logic_pmask_t stLPortMsk, logic_pmask_t stUntagLPortMsk)
{
    ...
    if (!VALID_VLAN_ID(uiVid) || (RTL8367B_FIDMAX < uiFid)) 
    {
        return DRV_ERR_PARA;
    }
...
DRV_RET_E Hal_GetValideFid(UINT32 uiVid, UINT32 *puiFid)
{
    ...
    if (RTL8367B_FIDMAX >= *puiFid)
    {
        return DRV_OK;
    }
        
    SPIN_HAL_MAC_LOCK;
    for (uiFidIdx = 1; uiFidIdx <= RTL8367B_FIDMAX; uiFidIdx++)
    {
        for (i = 0; i <= RTL8367B_FIDMAX; i++)
        {
            if (TRUE == m_astVlanFid[i].bValid)
...
DRV_RET_E Hal_GetMcMacByMacAndFid(UINT32 uiVid, mac_address_t mac_addr, mac_mcast_t *pstMacMcast)
{
    ...
    UnitFor(unit)
    {
        if(RT_ERR_OK != rtk_l2_mcastAddr_get(&stRtkMacAddr, DISABLED, uiFid, &stRtkPortMask))
        {
            //printk("\nf:%s,l:%d,fid:%d\n",__func__,__LINE__,uiFid);
            return DRV_ERR_MAC_ADDR_NOT_FOUND;
        }
...
DRV_RET_E Hal_SetMcMacByMacAndFid(mac_mcast_t stMacMcast)
{
   ...
   // printk("\n*****enRet=%d\n",enRet);
    ...
    {
        memcpy(&stPortMask, &phy_mask.pmask[unit], sizeof(stPortMask));
        rtkApiRet = rtk_l2_mcastAddr_add(&stMacAddr, DISABLED, uiFid, stPortMask);
        if (RT_ERR_OK != rtkApiRet)
        {
            return DRV_SDK_GEN_ERROR;
        }
...
DRV_RET_E Hal_DelMcMacByMacAndFid(mac_mcast_t stMacMcast)
{
...
#if defined(CONFIG_PRODUCT_ET704LA)
    if((0 != stMacMcast.tdVid) && (!VALID_VLAN_ID(stMacMcast.tdVid)))
#else
    if(!VALID_VLAN_ID(stMacMcast.tdVid))
#endif
    ...
    if (DRV_OK == enRet)
    {
        ...
        if (TRUE == bSamePort)
        {
            if(RT_ERR_OK != rtk_l2_mcastAddr_del(&stMacAddr, DISABLED, uiFid))
            {
                diag_printf("\nfunc:%s,line:%d\n",__FUNCTION__,__LINE__);
                return DRV_SDK_GEN_ERROR;
            }
        ...
        else
        {
            ...
            UnitFor(unit)
            {
                memcpy(&stPortMask, &phy_mask_tmp.pmask[unit], sizeof(stPortMask));
                rtkApiRet = rtk_l2_mcastAddr_add(&stMacAddr, DISABLED, uiFid, stPortMask);
                if (RT_ERR_OK != rtkApiRet)
                {
                    diag_printf("\nfunc:%s,line:%d\n",__FUNCTION__,__LINE__);
...
{    
    ...
    (void)rtk_l2_learningCnt_get(PortLogic2PhyPortId(ulLgcPort), pulCount);
...
static UINT32 _Hal_UcastCompare(UINT32 ulUnit, rtk_l2_ucastAddr_t *pstRtkUcast, mac_dump_ucast_tab_t *pstDumpTab, UINT32 *pulRefreshOnly)
{
    ...
    *pulRefreshOnly = 1;
    /*compare vid*/
    tdVid = (vlan_id_t)pstRtkUcast->cvid;
...
static DRV_RET_E _Hal_DumpUCastMacEntry(UINT32 ulUnit, UINT32 ulStartIndex, UINT32 ulEndIndex)
{
    ...
    {        
        stRtkMacEntry.index = (rtk_int32)ulIndex; 
        if(RT_ERR_OK == rtk_l2_entry_get(&stRtkMacEntry))
        {                            
            /*unicast*/
            if (0 == (stRtkMacEntry.mac.octet[0] & 0x01))
            {
                SPIN_HAL_MAC_LOCK;
                ulRefreshOnly = 0;
                pstMacDump = &(_mac_ucast_entry_dump[ulUnit][ulIndex]);
                ...
                if(pstMacDump->flag & DRV_MAC_DUMP_FLAG_VALID)
                {
                    ...
                    memcpy(&stRtkMac.mac, &stRtkMacEntry.mac, sizeof(stRtkMac.mac));
                    stRtkMac.cvid      = stRtkMacEntry.cvid;
                    stRtkMac.ivl       = stRtkMacEntry.ivl;
                    stRtkMac.fid       = stRtkMacEntry.fid;
                    stRtkMac.is_static = stRtkMac.is_static;
                ...
                memcpy(pstMacDump->mac_addr, stRtkMacEntry.mac.octet, sizeof(mac_address_t));
                pstMacDump->vid = (vlan_id_t)stRtkMacEntry.cvid;
                if(!ulRefreshOnly)
                {
                    /*insert if need*/
        ...
        for(ulIndex = 0; ulIndex < MAX_ARL_TABLE; ulIndex++)
        {
            ...
            {       
                if(0 == (stMacEntry.mac.octet[0] & 0x01))
                {
                    ...
                    #else
                        if ((INVALID_PORT != stUcastDumpTmp.lport) && (LOGIC_CPU_PORT != stUcastDumpTmp.lport))
                    #endif
                    /* 014045 */
                        {
                            stUcastDumpTmp.flag = DRV_MAC_DUMP_FLAG_VALID;
                    ...
                    memcpy(stUcastDumpTmp.mac_addr, stMacEntry.mac.octet, sizeof(mac_address_t));
                    stUcastDumpTmp.vid = (UINT16)stMacEntry.cvid;
                
                    /*Copy mac to user space*/
                    if(ADDRESS_IN_USER_SPACE == ulPDataType)
                    {
                        (void)copy_to_user(pstUserUcastDump, &stUcastDumpTmp, sizeof(mac_dump_ucast_tab_t));
                    }
...
DRV_RET_E Hal_SetMacTrapToCpu(port_num_t lport, mac_trap_set_t stTrapSet, UINT32 priority)
{
    ...
    rtk_igmp_type_t enRtkFrameType;
    rtk_trap_igmp_action_t enRtkAction;
    ...
    switch(stTrapSet.frameType)
    {
        ...
        case FRAME_TRAP_TYPE_MLD:
            enRtkFrameType = IGMP_MLD;
            break;
        case FRAME_TRAP_TYPE_IGMP:
            enRtkFrameType = IGMP_IPV4;
            break;
    ...
    switch(stTrapSet.trapAction)
    {
        case FRAME_TRAP_ACTION_FORWARD:
            enRtkAction = IGMP_ACTION_FORWARD;
            break;
        case FRAME_TRAP_ACTION_DROP:
            enRtkAction = IGMP_ACTION_DROP;
            break;
        case FRAME_TRAP_ACTION_TRAP2CPU:
            enRtkAction = IGMP_ACTION_TRAP2CPU;
            break;
    ...
    if(0 == lport)
    {
        LgcPortFor(lgcPort)
        {
            tdRtkRet = rtk_trap_igmpCtrlPktAction_set(enRtkFrameType, enRtkAction);
...
DRV_RET_E Hal_GetMacExact(vlan_id_t tdVid, mac_address_t mac_addr, mac_common_t *pstTmpMac)
{
    ...
    /*find multicast*/
    memset(&stRtkPortMask, 0, sizeof(stRtkPortMask));

    UnitFor(unit)
    {
        
        if(RT_ERR_OK == rtk_l2_mcastAddr_get(&stRtkMac, ENABLED, tdVid, &stRtkPortMask))
        {
            memcpy(&(stPhyMask.pmask[unit]), &stRtkPortMask, sizeof(rtk_portmask_t));
...
{        
    ...
    UINT32 uiLPort;
    UINT32 uiPPort;
    rtk_trap_mcast_action_t stAction;
    ...
    if(DISABLE == ulEnable)
    {
        ...
        stAction = MCAST_ACTION_FORWARD;

    }
    else
    {
        /*Drop unknow multicast-MAC packet*/
        stAction = MCAST_ACTION_DROP;
    }
    ...
    UnitFor(unit)
    {
        LgcPortFor(uiLPort)
        {
            ...
            if(RT_ERR_OK != rtk_trap_unknownMcastPktAction_set(uiPPort, MCAST_L2, stAction))
            {
                return DRV_SDK_GEN_ERROR;
            }
        }
    ...
    if(RT_ERR_OK != rtk_trap_unknownMcastPktAction_set(SWITCH_UPLINK_PHY_PORT, MCAST_L2, stAction))
    {
        return DRV_SDK_GEN_ERROR;
    }
    return DRV_OK;
...
{        
    ...
    switch (enActMode)
    {
        ...
        case MCAST_ACT_TRAP2CPU:
            stAction = MCAST_ACTION_TRAP2CPU;
            break;
        case MCAST_ACT_ROUTER_PORT:
            stAction = MCAST_ACTION_ROUTER_PORT;
            break;
        default :
            return DRV_ERR_PARA;
    }
    ...
    UnitFor(unit)
    {
        LgcPortFor(uiLPort)
        {
            ...
            if(RT_ERR_OK != rtk_trap_unknownMcastPktAction_set(uiPPort, MCAST_IPV4, stAction))
            {
                return DRV_SDK_GEN_ERROR;
            }
        }
    ...
    /*Drop unknow mcpkt ingress from uplink port.*/
    if(RT_ERR_OK != rtk_trap_unknownMcastPktAction_set(SWITCH_UPLINK_PHY_PORT, MCAST_IPV4, stAction))
    {
        return DRV_SDK_GEN_ERROR;
    }
    return DRV_OK;
...
DRV_RET_E Hal_SetIgmpPktAction(UINT32 ulAction)
{
    ...
    UINT32 uiLPort;
    UINT32 uiPPort;
    rtk_trap_igmp_action_t stAction;

    switch (ulAction)
	{
	    case PASS_TYPE_CPU_ONLY:
			 stAction = IGMP_ACTION_TRAP2CPU;
			 break;
		case PASS_TYPE_ALLPORTS:
			 stAction = IGMP_ACTION_FORWARD;
			 break;
		case PASS_TYPE_DROP:
			 stAction = IGMP_ACTION_DROP;
			 break;
		case PASS_TYPE_CPU_EXCLUDE:
			 stAction = IGMP_ACTION_FORWARD_EXCLUDE_CPU;
			 break;
		default:
			 return DRV_ERR_PARA;
	}
	...
	UnitFor(unit)
    {
        LgcPortFor(uiLPort)
        {
            ...
            if(RT_ERR_OK != rtk_igmp_protocol_set(uiPPort, PROTOCOL_IGMPv1, stAction) ||
  			   RT_ERR_OK != rtk_igmp_protocol_set(uiPPort, PROTOCOL_IGMPv2, stAction))
            {
                return DRV_SDK_GEN_ERROR;
            }
			diag_printf("Hal_SetIgmpPktAction port %d action %d\n", uiPPort, stAction);
        }
    }
    return DRV_OK;
}
