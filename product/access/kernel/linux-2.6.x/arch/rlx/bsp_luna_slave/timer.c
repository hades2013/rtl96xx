...
#error "CONFIG_RTL_TIMER_ADJUSTMENT"
#include <rtl_types.h>
...
{
	REG32(BSP_TCIR) = (REG32(BSP_TCIR)|BSP_TC4IE|BSP_TC4IP) & (~(BSP_TC0IP|BSP_TC1IP|BSP_TC2IP|BSP_TC3IP|BSP_TC5IP));
}

void new_bsp_timer_ack(void)
{
	REG32(BSP_TC4INT) |= (BSP_TCIP);
}

void __init bsp_timer_init(void) {
	if(SOC_ID == 0x0371){
		/* Clear Timer IP status */
		if (REG32(BSP_TCIR) & BSP_TC4IP)
			REG32(BSP_TCIR) |= BSP_TC4IP;
		...
		REG32(BSP_TC4DATA) = ( (BSP_MHZ)*((BSP_DIVISOR)/HZ)) << 4;
	} else {
		/* Clear Timer IP status */
		if (REG32(BSP_TC4INT) & BSP_TCIP) {
			REG32(BSP_TC4INT) |= (BSP_TCIP);
		}
	
		/* disable timer */
		REG32(BSP_TC4CTL) = 0x0; /* disable timer before setting CDBR */
	
		/* initialize timer registers */
		REG32(BSP_TC4CTL) |= (BSP_DIVISOR) << 0;
		REG32(BSP_TC4DATA) = ( (BSP_MHZ)*((BSP_DIVISOR)/HZ)) << 0;
	}
		
	/* hook up timer interrupt handler */
	rlx_clockevent_init(BSP_TC4_IRQ);
	
	if(SOC_ID == 0x0371) {
		bsp_timer_ack = old_bsp_timer_ack;
		/* enable timer */
		REG32(BSP_TCCNR) = BSP_TC4EN | BSP_TC4MODE_TIMER;
		REG32(BSP_TCIR) = BSP_TC4IE;
	} else {
		bsp_timer_ack = new_bsp_timer_ack;
		/* enable timer */
		REG32(BSP_TC4CTL) |= BSP_TCEN | BSP_TCMODE_TIMER;
		REG32(BSP_TC4INT) |= BSP_TCIE;
	}
}
