...
#endif
#ifdef CONFIG_RTL8672
extern void wirelessnet_shutdown_set(void (*hook) (void));
extern int wirelessnet_shutdown_loaded(void);
#endif
		...
		{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
			...
			{(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, PCI_SLOT1_CONFIG_ADDR, 0xb8C00000, 3, NULL},
							...
							{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, BSP_PCIE0_D_CFG0, BSP_PCIE0_D_MEM, BSP_PCIE_IRQ, NULL},
								...
								{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, BSP_PCIE1_D_CFG0, BSP_PCIE1_D_MEM, SLOT1_IRQ, NULL},
							#endif
						#elif (RTL_USED_PCIE_SLOT==1)
							{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, BSP_PCIE1_D_CFG0, BSP_PCIE1_D_MEM, SLOT1_IRQ, NULL},
							#ifdef CONFIG_RTL_92D_DMDP
								{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, BSP_PCIE1_D_CFG0, BSP_PCIE1_F1_D_MEM, SLOT1_IRQ, NULL},
							#endif
						...
						#if defined(CONFIG_RTL_88E_SUPPORT)||defined(CONFIG_RTL_92E_SUPPORT)||defined(CONFIG_RTL_8812_SUPPORT)
							#if defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1)
...
//	Remove temporary, hang on bootup
//	if ((priv->drv_state & DRV_STATE_OPEN)==0)	// set driver as has been closed, david
//		goto int_exit;
...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
...
#endif
	{
...
#ifndef __KERNEL__
		if ((unsigned long)page_ptr & (~(PAGE_SIZE - 1)))
			page_ptr = (unsigned char *)(((unsigned long)page_ptr & (~(PAGE_SIZE - 1))) + PAGE_SIZE);
#endif
#ifdef CONFIG_NET_PCI
		if (!IS_PCIBIOS_TYPE)
#endif
		...
		DEBUG_INFO("rx_descL=%08lx tx_desc0=%08lx, tx_desc1=%08lx, tx_desc2=%08lx, tx_desc3=%08lx, tx_desc4=%08lx, "
			"tx_desc5=%08lx, rxcmd_desc=%08lx, txcmd_desc=%08lx, tx_descB=%08lx\n",
			(unsigned long)phw->rx_descL, (unsigned long)phw->tx_desc0, (unsigned long)phw->tx_desc1, (unsigned long)phw->tx_desc2,
			(unsigned long)phw->tx_desc3, (unsigned long)phw->tx_desc4, (unsigned long)phw->tx_desc5,
			(unsigned long)phw->rxcmd_desc, (unsigned long)phw->txcmd_desc, (unsigned long)phw->tx_descB);
#ifdef RTK_QUE
	...
	*/
	...
	INIT_LIST_HEAD(&(priv->addps_list));
...
#endif
		{
			...
			if (priv->pshare->is_40m_bw == 0)
				...
				if ((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)
					&&((GET_CHIP_VER(priv)==VERSION_8192D) || (GET_CHIP_VER(priv)==VERSION_8812E))) {
...
#ifdef CONFIG_RTL_8812_SUPPORT  //80M todo, for Band 1 only now
//	if(priv->pshare->is_40m_bw == 2)
//		get_txsc_8812(priv, priv->pmib->dot11RFEntry.dot11channel);
#endif
...
#if defined(CONFIG_AUTO_PCIE_PHY_SCAN) && (defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8196D))
#ifdef CONFIG_RTL_88E_SUPPORT
		if(GET_CHIP_VER(priv)==VERSION_8188E)
		{
...
#endif
#endif
#if !defined(CONFIG_PHY_EAT_40MHZ) && (defined(CONFIG_RTL_8197DL) || defined(CONFIG_RTL8672))
#ifdef CONFIG_RTL_88E_SUPPORT
		if (GET_CHIP_VER(priv)==VERSION_8188E) {
			//25MHz
			printk("\n\n 88E 25M TEST 0x11=0x5b, 0x2c[0:3]=0x1 \n\n");
...
#else
		while (NULL != (skb = skb_dequeue(&priv->pshare->skb_queue))) {
...
#endif
#ifdef CONFIG_RTL_8812_SUPPORT
    if (priv->pshare->version_id == VERSION_8812E) {
        if (priv->pshare->rf_ft_var.use_ext_lna)
            priv->pshare->rf_ft_var.use_ext_lna = 0;
		//if (priv->pshare->rf_ft_var.use_ext_pa)
...
#if defined (CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT)
#ifdef CONFIG_RTL_92D_SUPPORT
				if (vendor_deivce_id == ((unsigned long)((0x8193<<16)|PCI_VENDOR_ID_REALTEK)))
					priv->pshare->version_id = VERSION_8192D;
				else
...
#ifdef MBSSID
		if (vap_idx >= 0) // create for vap
			GET_ROOT_PRIV(priv)->pvap_priv[vap_idx] = NULL;
#endif
	} else
#endif
	wdev->priv = NULL;
...
{
	struct net_device *dev;
#if defined(WDS) || defined(MBSSID) || defined(CONFIG_RTK_MESH)
	int i;
#endif
...
#endif
#ifdef CONFIG_RTL8672
	wirelessnet_shutdown_set(force_stop_wlan_hw);
#endif
#endif // CONFIG_WIRELESS_LAN_MODULE && !NOT_RTK_BSP
#endif // __KERNEL__
...
// Called by root interface.
void execute_tx_pwr_tracking(struct rtl8192cd_priv *priv)
{
#if !defined(USE_OUT_SRC) || defined(_OUTSRC_COEXIST)
	#ifdef _OUTSRC_COEXIST
	if(!IS_OUTSRC_CHIP(priv))
...
#elif !defined(CALIBRATE_BY_ODM) && defined(CONFIG_RTL_88E_SUPPORT) //for 88e tx power tracking
	if (GET_CHIP_VER(priv)==VERSION_8188E){
		odm_TXPowerTrackingCallback_ThermalMeter_8188E(priv);
	}

#elif defined(USE_OUT_SRC)
	// Call ODM Function.
	ODM_DoTxPowerTrackingAP(ODMPTR);
#endif
}
...
int kTPT_task_init(struct rtl8192cd_priv *priv)
{
	...
	#endif
	sprintf( thread_name, "kTPTd_%d", idx );

	rtl_atomic_set(&priv->pshare->do_tpt, 0);
	if (NULL == priv->pshare->tpt_task[idx]) {
		priv->pshare->tpt_task[idx] = kthread_run(kTPT_thread, (void *)priv, thread_name);
		if (IS_ERR(priv->pshare->tpt_task[idx])) {
			printk("%s Thread create failed!\n", thread_name);
			priv->pshare->tpt_task[idx] = NULL;
			return -1;
		}
	}
...
void kTPT_task_stop(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pshare->tpt_task[idx]
#ifdef CHECK_HANGUP
			&& !priv->reset_hangup
#endif
			)
	{
		kthread_stop(priv->pshare->tpt_task[idx]);
		priv->pshare->tpt_task[idx] = NULL;
	}
}
#endif /* TPT_THREAD */
