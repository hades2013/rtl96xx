	...
	)
{
	...
	else if (Method == BBSWING)
	{
		...
		if (RFPath == RF_PATH_A)
		{
			setIqkMatrix_8812A(pDM_Odm, pDM_Odm->BbSwingIdxOfdm[RF_PATH_A], RF_PATH_A, 
						 pDM_Odm->RFCalibrateInfo.IQKMatrixRegSetting[ChannelMappedIndex].Value[0][0],
						 pDM_Odm->RFCalibrateInfo.IQKMatrixRegSetting[ChannelMappedIndex].Value[0][1]);						
		}
		else if (RFPath == RF_PATH_B)
		{
			setIqkMatrix_8812A(pDM_Odm, pDM_Odm->BbSwingIdxOfdm[RF_PATH_B], RF_PATH_B,
...
void _IQK_BackupMacBB_8812A(
	IN PDM_ODM_T		pDM_Odm,
	IN unsigned int*		MACBB_backup,
	IN unsigned int*		Backup_MACBB_REG, 
	IN unsigned int		MACBB_NUM
	)
{
	unsigned int i;
	ODM_SetBBReg(pDM_Odm, 0x82c, BIT(31), 0x0); // [31] = 0 --> Page C
...
void _IQK_BackupRF_8812A(
	IN PDM_ODM_T		pDM_Odm,
	IN unsigned int*		RFA_backup,
	IN unsigned int*		RFB_backup, 
	IN unsigned int*		Backup_RF_REG, 
	IN unsigned int		RF_NUM
	)	
{
...
void _IQK_BackupAFE_8812A(
	IN PDM_ODM_T		pDM_Odm,
	IN unsigned int*		AFE_backup,
	IN unsigned int*		Backup_AFE_REG, 
	IN unsigned int		AFE_NUM
	)
{
	unsigned int i;
	ODM_SetBBReg(pDM_Odm, 0x82c, BIT(31), 0x0); // [31] = 0 --> Page C
...
void _IQK_RestoreRF_8812A(
	...
	IN ODM_RF_RADIO_PATH_E 	Path,
	IN unsigned int*			Backup_RF_REG,
	IN unsigned int* 			RF_backup,
	IN unsigned int 			RF_REG_NUM
	)
{	
	unsigned int i;
	...
	)
{
	...
	ODM_Write4Byte(pDM_Odm, 0xc8c, 0x0);
	...
	ODM_Write4Byte(pDM_Odm, 0xe8c, 0x0);
...
{
    unsigned int 	TX_fail,RX_fail, delay_count, IQK_ready, cal_retry, cal = 0, temp_reg65;
    int 			TX_X = 0, TX_Y = 0, RX_X = 0, RX_Y = 0, TX_Average = 0, RX_Average = 0;
    int 			TX_X0[cal_num], TX_Y0[cal_num], TX_X0_RXK[cal_num], TX_Y0_RXK[cal_num], RX_X0[cal_num], RX_Y0[cal_num];
    BOOLEAN 	    TX0IQKOK, RX0IQKOK;
	int 			TX_X1[cal_num], TX_Y1[cal_num], TX_X1_RXK[cal_num], TX_Y1_RXK[cal_num], RX_X1[cal_num], RX_Y1[cal_num];
	BOOLEAN  	    TX1IQKOK = FALSE, RX1IQKOK = FALSE;
	int 			i;


	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("EXPA = %d\n", pDM_Odm->ExtPA));
	while (cal < cal_num){
		switch (Path) {
		case ODM_RF_PATH_A:
			{
                ...
                {
					ODM_SetBBReg(pDM_Odm, 0x82c, BIT(31), 0x0); // [31] = 0 --> Page C
					ODM_SetRFReg(pDM_Odm, Path, 0x65, bRFRegOffsetMask, 0x931d7);
				}
				...
				ODM_SetRFReg(pDM_Odm, Path, 0x31, bRFRegOffsetMask, 0x3f4f5);
				ODM_SetRFReg(pDM_Odm, Path, 0x32, bRFRegOffsetMask, 0xfefbf);
				...
				ODM_Write4Byte(pDM_Odm, 0xc8c, 0x28161480);
				ODM_Write4Byte(pDM_Odm, 0xcb8, 0x00100000);// cb8[20] 將 SI/PI 使用權切給 iqk_dpk module
		...
		case ODM_RF_PATH_B:
            {
                ...
                ODM_SetRFReg(pDM_Odm, Path, 0x8f, bRFRegOffsetMask, 0x8a001);
                ODM_SetRFReg(pDM_Odm, Path, 0xef, bRFRegOffsetMask, 0x00000);
                ODM_SetBBReg(pDM_Odm, 0xeb8, 0xf, 0xd);
                ODM_Write4Byte(pDM_Odm, 0x90c, 0x00008000);
                ODM_Write4Byte(pDM_Odm, 0xb00, 0x03000100);
                ...
                if (pDM_Odm->ExtPA == 0){
                    ...
                    ODM_SetBBReg(pDM_Odm, 0x82c, BIT(31), 0x1); // [31] = 1 --> Page C1
                    ODM_Write4Byte(pDM_Odm, 0xe88, 0x821603e0);
                    ODM_Write4Byte(pDM_Odm, 0xeb8, 0x00100000);// cb8[20] 將 SI/PI 使用權切給 iqk_dpk module
                    cal_retry = 0;
                    while(1){
                        // one shot
                        ...
                        if (delay_count < 20){							// If 20ms No Result, then cal_retry++
                            ...
                            if (~TX_fail){
                                ODM_Write4Byte(pDM_Odm, 0xeb8, 0x02000000);
                                TX_X1_RXK[cal] = ODM_GetBBReg(pDM_Odm, 0xd40, 0x07ff0000)<<21;
                                ODM_Write4Byte(pDM_Odm, 0xeb8, 0x04000000);
                                TX_Y1_RXK[cal] = ODM_GetBBReg(pDM_Odm, 0xd40, 0x07ff0000)<<21;
                                TX1IQKOK = TRUE;
                                break;
                            }
                            else
                    ...
                    if (TX1IQKOK == FALSE){				//If RX mode TXK fail, then take TXK Result
                        TX_X1_RXK[cal] = 	TX_X1[cal];
                        TX_Y1_RXK[cal] = 	TX_Y1[cal];
                        TX1IQKOK = TRUE;
                        ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("1"));
                    }
                    ...
                    ODM_SetBBReg(pDM_Odm, 0x978, 0x03FF8000, (TX_X1_RXK[cal])>>21&0x000007ff);
                    ODM_SetBBReg(pDM_Odm, 0x978, 0x000007FF, (TX_Y1_RXK[cal])>>21&0x000007ff);
                    ODM_SetBBReg(pDM_Odm, 0x978, BIT(31), 0x1);
                    ODM_SetBBReg(pDM_Odm, 0x97c, BIT(31), 0x0);
                    ODM_SetBBReg(pDM_Odm, 0xeb8, 0xF, 0xe);
                    ODM_Write4Byte(pDM_Odm, 0x90c, 0x00008000);
                    ODM_Write4Byte(pDM_Odm, 0x984, 0x0046a911);
                    ...
                    ODM_Write4Byte(pDM_Odm, 0xe8c, 0x28161480);
                    ODM_Write4Byte(pDM_Odm, 0xeb8, 0x00100000);// cb8[20] 將 SI/PI 使用權切給 iqk_dpk module
	...
	switch (Path){
		case ODM_RF_PATH_A:
            {
                ...
                for (i = 0; i < TX_Average; i++){
                    TX_X = TX_X+(TX_X0[i]>>21);
                    TX_Y = TX_Y+(TX_Y0[i]>>21);
                ...
                TX_X = (TX_X/TX_Average)&0x000007ff;
                TX_Y = (TX_Y/TX_Average)&0x000007ff;
                if (RX_Average == 0)
                    break;
                for (i = 0; i < RX_Average; i++){
                    RX_X = RX_X+(RX_X0[i]>>21);
                    RX_Y = RX_Y+(RX_Y0[i]>>21);
                ...
                RX_X = (RX_X/RX_Average)&0x000007ff;
                RX_Y = (RX_Y/RX_Average)&0x000007ff;
                ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("TX_X = %x;;TX_Y = %x\n", TX_X, TX_Y));
                ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RX_X = %x;;RX_Y = %x\n", RX_X, RX_Y));
		...
		case ODM_RF_PATH_B:
            {
                ...
                if (TX_Average == 0)
                    break;
                for (i = 0; i < TX_Average; i++){
                    TX_X = TX_X+(TX_X1[i]>>21);
                    TX_Y = TX_Y+(TX_Y1[i]>>21);
                    ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, (" TX_X1_RXK[%d] = %x ;; TX_Y1_RXK[%d] = %x\n", i, (TX_X1_RXK[i])>>21&0x000007ff, i, (TX_Y1_RXK[i])>>21&0x000007ff));
		...
		_IQK_TX_FillIQC_8812A(pDM_Odm, Path, TX_X, TX_Y);
                ...
                for (i = 0; i < RX_Average; i++){
                    RX_X = RX_X+(RX_X1[i]>>21);
                    RX_Y = RX_Y+(RX_Y1[i]>>21);
...
{
	unsigned int MACBB_backup[MACBB_REG_NUM], AFE_backup[AFE_REG_NUM], RFA_backup[RF_REG_NUM], RFB_backup[RF_REG_NUM];
	unsigned int Backup_MACBB_REG[MACBB_REG_NUM] = {0x520, 0x550, 0x554, 0x808, 0x838, 0x90c, 0xb00, 0xc00, 0xc50, 0xe00, 0xe50 , 0xb2c}; 
	unsigned int Backup_AFE_REG[AFE_REG_NUM] = {0xc5c, 0xc60, 0xc64, 0xc68, 0xc6c, 0xc70, 0xc74, 0xc78, 0xc7c, 0xc80, 0xc84, 0xcb8,
		                                                                     0xe5c, 0xe60, 0xe64, 0xe68, 0xe6c, 0xce0, 0xe74, 0xe78, 0xe7c, 0xe80, 0xe84, 0xeb8}; 
	unsigned int Backup_RF_REG[RF_REG_NUM] = {0xef, 0x30, 0x31, 0x32, 0x65, 0x8f}; 
...
#ifdef APK_BB_REG_NUM
#undef APK_BB_REG_NUM
#endif
