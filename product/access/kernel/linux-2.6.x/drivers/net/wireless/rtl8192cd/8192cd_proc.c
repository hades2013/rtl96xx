...
#ifdef CONFIG_RTL_8812_SUPPORT  //vht rate 
	PRINT_ARRAY_ARG("    dot11OperationalRateSet_AC: ",
			priv->pshare->rf_ft_var.dot11OperationalRateSet_AC, "%02x",
			priv->pshare->rf_ft_var.dot11OperationalRateSetLen_AC);
	PRINT_SINGL_ARG("    dot11OperationalRateSetLen_AC: ",
			priv->pshare->rf_ft_var.dot11OperationalRateSetLen_AC, "%d");
#endif
...
#ifdef CONFIG_RTL_8812_SUPPORT  //vht rate 
	PRINT_SINGL_ARG("	 dot11SupportedRates_AC: ",
			priv->pshare->rf_ft_var.dot11SupportedRates_AC, "%d");
	PRINT_SINGL_ARG("	 dot11BasicRates_AC: ",
			priv->pshare->rf_ft_var.dot11BasicRates_AC, "%d");
...
#ifdef CONFIG_RTL_8812_SUPPORT //FOR_8812_MP
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) //FOR_8812_MP
...
#ifdef CONFIG_RTL_8812_SUPPORT  //vht rate , todo, dump vht rates in Mbps
	if(pstat->current_tx_rate >= 0x90){
		PRINT_ONE("    current_tx_rate: VHT ", "%s", 0);
		PRINT_ONE(pstat->current_tx_rate, "0x%x", 1);
		CHECK_LEN_B;
	}
	else
#endif
...
//		rate = (unsigned char *)MCS_DATA_RATEStr[pstat->rx_bw][pstat->rx_splcp][pstat->rx_rate&0x7f];
	...
	//PRINT_SINGL_ARG("    tx_bw: ", (pstat->tx_bw ? (pstat->tx_bw*40) : 20), "%d");
	PRINT_SINGL_ARG("    tx_bw: ", (0x1<<(pstat->tx_bw))*20, "%d");	
	CHECK_LEN_B;
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
	...
	if ( create_proc_read_entry ("mib_dfs", 0644, rtl8192cd_proc_root,
		...
		if ( create_proc_read_entry ("mib_rf_AC", 0644, rtl8192cd_proc_root,
			...
			printk("create proc mib_AC failed!\n");
			return;
		}
#endif
...
void MDL_DEVEXIT rtl8192cd_proc_remove (struct net_device *dev)
{
#ifdef NETDEV_NO_PRIV
	struct rtl8192cd_priv *priv = ((struct rtl8192cd_priv *)netdev_priv(dev))->wlan_priv;
#else
...
#endif
#ifdef CONFIG_RTL_8812_SUPPORT //FOR_8812_MP
	{"mib_AC", 				rtl8192cd_proc_mib_rf_AC},
#endif
