...
#ifdef CONFIG_RTL_8812_SUPPORT  //80M , todo, limlt in Band 1 
...
#ifdef CONFIG_RTL_8812_SUPPORT  //80M
	if(GET_CHIP_VER(priv) == VERSION_8812E)
	{
...
#endif
			{
				if (priv->assoc_num > 1 && pstat->rssi <= 32)
		...
		if (priv->pmib->wapiInfo.wapiType != wapiDisable)
		{
			...
			if (retVal) {
				...
				if (retVal) {
					...
					retVal = CamDeleteOneEntry(priv, MULTICAST_ADD, 1, 0);
					if (retVal)
