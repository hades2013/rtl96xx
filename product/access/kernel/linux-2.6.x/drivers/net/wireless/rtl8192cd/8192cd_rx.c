...
{
	struct ipv6hdr *iph = (struct ipv6hdr *)SKB_IP_HEADER(pskb);
...
void rtl8192cd_rx_isr(struct rtl8192cd_priv *priv)
{
	...
	while (1)
	{
		...
		#ifdef NOT_RTK_BSP
		rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(phw->rx_infoL[tail].paddr), (RX_BUF_LEN - sizeof(struct rx_frinfo)), PCI_DMA_FROMDEVICE);
		#endif
