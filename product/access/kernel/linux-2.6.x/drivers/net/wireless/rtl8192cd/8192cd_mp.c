...
#include <bsp/bspchip.h>
	...
	#define _UART_LSR_				_UART_LSR
		...
		#define _GIMR_				BSP_GIMR
		#endif
		#define _ICU_UART_MSK_		BSP_UART0_IE
		#define _UART_RBR_			BSP_UART0_RBR
		#define _UART_LSR_			BSP_UART0_LSR
	#else
		#define _GIMR_				GIMR
		#define _ICU_UART_MSK_		UART0_IE
		#define _UART_RBR_			UART0_RBR
		#define _UART_LSR_			UART0_LSR
	#endif // __LINUX_2_6__
...
#define DISABLE_UART_INT() \
	do { \
		RTL_UART_W16(_GIMR_, RTL_UART_R16(_GIMR_) & ~_ICU_UART_MSK_); \
		RTL_UART_R32(_UART_RBR_); \
		RTL_UART_R32(_UART_RBR_); \
	} while (0)

#define RESTORE_UART_INT() \
	do { \
		RTL_UART_W16(_GIMR_, RTL_UART_R16(_GIMR_) | _ICU_UART_MSK_); \
	} while (0)
...
//_TXPWR_REDEFINE ?? shall also apply to Normal Driver ??
	if ((priv->pshare->rf_ft_var.use_ext_pa) && (GET_CHIP_VER(priv)==VERSION_8192C) ) {
		RTL_W8(0xc50, 0x2e);
		RTL_W8(0xc58, 0x2e);
	} else
#endif
	if (priv->pshare->rf_ft_var.use_ext_lna &&
	  ((GET_CHIP_VER(priv)==VERSION_8192C) || (GET_CHIP_VER(priv)==VERSION_8188C) ||(GET_CHIP_VER(priv)==VERSION_8192D))) {
		RTL_W8(0xc50, 0x2a);
		RTL_W8(0xc58, 0x2a);
	}
	else {
...
#endif
					PHY_SetRFReg(priv, (RF92CD_RADIO_PATH_E)eRFPath, rRfChannel, (BIT(10) | BIT(11)), 0x00);
...
#ifdef CONFIG_RTL_8812_SUPPORT //for 88e tx power tracking
	if (GET_CHIP_VER(priv)==VERSION_8812E)
