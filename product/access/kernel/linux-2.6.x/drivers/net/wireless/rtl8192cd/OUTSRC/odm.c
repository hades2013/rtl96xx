	...
	)
{
	...
	for (i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++)
	{
		...
		if (IS_STA_VALID(pstat))
		{
			...
			struct aid_obj *aidobj = container_of(pstat, struct aid_obj, station);
			priv = aidobj->priv;
#endif
...
#if ((RTL8192C_SUPPORT==1) || (RTL8192D_SUPPORT==1) || (RTL8188E_SUPPORT==1))
			...
			{			
...
#endif
				{
					...
					if(pDM_Odm->SupportICType == ODM_RTL8812) {
						...
						H2C_Parameter[1] = REMAP_AID(pstat);
	...
	)
{
	...
	if ( (priv->pmib->dot11RFEntry.ther) && ((priv->up_time % priv->pshare->rf_ft_var.tpt_period) == 0)){
		#ifndef TPT_THREAD
		ODM_DoTxPowerTrackingAP(pDM_Odm);
		#else
...
#if ((DM_ODM_SUPPORT_TYPE==ODM_AP)&& ( defined(CONFIG_RTL_8198B) || defined CONFIG_RTL_819XD))
    else if( (priv->assoc_num == 1) && (AMPDU_ENABLE || AMSDU_ENABLE)) {	
        if (pstat) {
            ...
            if ((priv->swq_en == 0) && (pstat->current_tx_bytes > en_thd) && (pstat->current_rx_bytes > en_thd) )  { //50Mbps
      	 	...
      	 	else if ((priv->swq_en == 1) && ((pstat->tx_avarage < en_thd2) || (pstat->rx_avarage < en_thd2))) { //35Mbps
                		priv->swq_en = 0;
