...
#include "data_PHY_REG_PG_92C_FCC.c"
#include "data_PHY_REG_PG_92C_CE.c"
...
VAR_MAPPING(PHY_REG_PG_92C_FCC, PHY_REG_PG_92C_FCC);
VAR_MAPPING(PHY_REG_PG_92C_CE, PHY_REG_PG_92C_CE);
...
int PHY_ConfigTXLmtWithParaFile(struct rtl8192cd_priv *priv)
{
	...
	if (GET_CHIP_VER(priv)!=VERSION_8192D && GET_CHIP_VER(priv)!=VERSION_8192C) {
		printk("[%s]NOT support! TXPWR_LMT is for RTL8192C/D ONLY!\n",__FUNCTION__);
		return -1;
	}
	...
	if (reg_file == PHYREG_PG) {
		...
		if ((GET_CHIP_VER(priv) == VERSION_8192C)||(GET_CHIP_VER(priv) == VERSION_8188C)){
...
#endif
			{
				if(priv->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_FCC){
					//printk("\nFCC PG!!!\n");
					next_head = data_PHY_REG_PG_92C_FCC_start;
					read_bytes = (int)(data_PHY_REG_PG_92C_FCC_end - data_PHY_REG_PG_92C_FCC_start);
				}
				else if(priv->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_ETSI){
					//printk("\nCE PG!!!\n");
					next_head = data_PHY_REG_PG_92C_CE_start;
					read_bytes = (int)(data_PHY_REG_PG_92C_CE_end - data_PHY_REG_PG_92C_CE_start);
				}
				else{
...
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) //_eric_8812 ?? u4bRegOffset
	...
	while (1) {
		...
		if (file_format == THREE_COLUMN){
			...
			if (reg_file == PHYREG_PG && (
				...
				|| 
#endif
				(GET_CHIP_VER(priv)==VERSION_8188E) || (GET_CHIP_VER(priv)==VERSION_8812E)
#endif
				)) {
				if (u4bRegOffset==0xe00){
					if (idx == pg_tbl_idx)
				...
				RF92CD_RADIO_PATH_E eRFPath)
{
	...
	while (1) {
		...
		num = get_offset_val(line_head, &u4bRegOffset, &u4bRegValue);
		if (num > 0) {
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) //_eric_8812 ?? rf paras ??
			if ((GET_CHIP_VER(priv)==VERSION_8188E) || (GET_CHIP_VER(priv)==VERSION_8812E)) {
				if (u4bRegOffset == 0xffff) {
					break;
...

	{
		...
		while (1) {
			...
			if (num > 0) {
				...
				len++;
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT)//eric_8812 ??
				if ((GET_CHIP_VER(priv)==VERSION_8188E) || (GET_CHIP_VER(priv)==VERSION_8812E)) {
					if (u4bRegOffset == 0xffff)
						break;
				} else
...
#ifdef CONFIG_RTL_8812_SUPPORT //todo, find working channel for 80M
...
INT8 PwrPercent2PwrLevel(int percentage)
{
	#define ARRAYSIZE(x)	(sizeof(x)/sizeof((x)[0]))
	
	const INT16 percent_threshold[] = {95, 85, 75, 67, 60, 54, 48, 43, 38, 34, 30, 27, 24, 22, 19, 17, 15, 14, 12, 11, 10};
	const INT8 pwrlevel_diff[9] = {-40, -34, -30, -28, -26, -24, -23, -22, -21};	// for < 10% case
	int i;
...
void PHY_RF6052SetOFDMTxPower(struct rtl8192cd_priv *priv, unsigned int channel)
{
	...
	INT8 base, byte0, byte1, byte2, byte3;
...
#endif
#ifdef POWER_PERCENT_ADJUSTMENT
	INT8 pwrdiff_percent = PwrPercent2PwrLevel(priv->pmib->dot11RFEntry.power_percent);
#endif
#ifdef CONFIG_RTL_92D_SUPPORT
...
#ifdef MP_TEST
		|| (priv->pshare->rf_ft_var.mp_specific && priv->pshare->mp_txpwr_patha)
#endif
		) 
	{
...
void PHY_RF6052SetCCKTxPower(struct rtl8192cd_priv *priv, unsigned int channel)
{
	...
	if (priv->pshare->rf_ft_var.cck_pwr_max) {
		byte = POWER_RANGE_CHECK((char)priv->pshare->rf_ft_var.cck_pwr_max);
		writeVal = byte;
...
// Set reserved page for each queue
	...
	RTL_W8(TXPKTBUF_WMAC_LBK_BF_HD, txpktbufSz);
}
...
//	RTL_W32(TCR, RTL_R32(TCR)|CRC|CFE_FORM);
	...
	//RTL_W8(PBP, (PBP_256B&PSTX_Mask)<<PSTX_SHIFT|(PBP_256B&PSRX_Mask)<<PSRX_SHIFT);
		...
		printk("BEQ_DESA = 0x%08x 0x%08x \n", RTL_R32(BEQ_DESA),priv->pshare->phw->tx_ring2_addr);
...
{	
	...
	ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_MP_TEST_CHIP, !IS_TEST_CHIP(priv));
...
//	priv->pmib->dot11RFEntry.trswitch = 1;
	...
	if (GET_CHIP_VER(priv) == VERSION_8812E) {
		...
		RTL_W8(0x44b, 0x3c);		// 2SS MCS7 enable
//		RTL_W32(0x2c, 0x28a3e200);
		RTL_W32(0x2c, 0x4103e200);

		RTL_W32(0x864, 0x796a6c32);

#ifdef FOR_VHT5G_PF2 //8812 cca
...
//	Path A only
		if(get_rf_mimo_mode(priv) == MIMO_1T1R) {
			...
			RTL_W8(0xa07, (RTL_R8(0xa07) & 0xf3)| 0x04 );	// [3:2] = 2'b 01
	...
	//RTL_W8(0x44b, 0x3c); //Disble MCS7 of RA
#endif
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv)==VERSION_8192E) {
			HalPwrSeqCmdParsing(priv, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_PCI_MSK, rtl8821A_card_disable_flow);
		} else
#endif
#ifdef CONFIG_RTL_8812_SUPPORT
		if (GET_CHIP_VER(priv)==VERSION_8812E) {
...
//========== <2> Set primary channel 0x8ac & 0xa00, txsc 0x483
...
#ifdef RTL8676_WAKE_GPIO
	return request_irq(irq_num, gpio_wakeup_isr, IRQF_DISABLED, "rtk_gpio", dev);
#else
	#if defined(__LINUX_2_6__)
...
#ifdef CONFIG_RTL_92D_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8192D) {
		efuse_crystalCap = EEPROM_92D_XTAL_K;
	}
#endif

#ifdef CONFIG_RTL_88E_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8188E) {
		efuse_crystalCap = EEPROM_XTAL_88E;
	}
#endif

	priv->pmib->dot11RFEntry.xcap = (hwinfo[efuse_crystalCap]);	//[7:0]
	DEBUG_INFO("CrystalCap: 0x%2x\n", priv->pmib->dot11RFEntry.xcap);
}
			...
			if (efuse_data!=0xFF)
			{
				...
				if(target_pkt.offset >= EFUSE_MAX_SECTION_BASE)
				{
					...
					if(tmp_header == pg_header)
					{
						...
						DEBUG_INFO("efuse_PgPacketWrite extended pg_header[6:3] | word_en 0x%x\n", pg_header);
