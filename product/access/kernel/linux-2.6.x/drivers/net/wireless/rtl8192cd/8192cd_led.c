...
static void set_sw_LED0(struct rtl8192cd_priv *priv, int flag)
{
	...
	(defined(CONFIG_RTL_88E_SUPPORT) && (defined(CONFIG_RTL8676) || defined(NOT_RTK_BSP)))
	if (flag)
		RTL_W32(LEDCFG, (RTL_R32(LEDCFG) & 0xfffffff0) | LED0SV);
	else
		RTL_W32(LEDCFG, RTL_R32(LEDCFG) & 0xfffffff0);
#elif defined(CONFIG_RTL_88E_SUPPORT) && defined(RTLWIFINIC_GPIO_CONTROL)
	if (flag)
...
#if defined(CONFIG_RTL8676) || defined(NOT_RTK_BSP)
	RTL_W32(LEDCFG, ((RTL_R32(LEDCFG)&0xFFF0FFFF)) | LED2EN);
#elif defined(RTLWIFINIC_GPIO_CONTROL)
