...
#endif
	if(pDM_Odm->SupportICType & (ODM_RTL8192C|ODM_RTL8192D|ODM_RTL8192E))
	{
	...
	)
{
	...
	pDM_DigTable->RSSI_pre = 0;
	memset(pDM_DigTable->DIG_record, 0, sizeof(pDM_DigTable->DIG_record));
	memset(pDM_DigTable->FA_record, 0, sizeof(pDM_DigTable->FA_record));
...
//	pRXHP_T						pRX_HP_Table  = &pDM_Odm->DM_RXHP_Table;
	...
	else
	{
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))
#ifdef DFS
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): pDM_Odm->ControlChannel=%d , pDM_Odm->pChannel =%d\n", 
				pDM_Odm->ControlChannel, *pDM_Odm->pChannel ));
			if (((*pDM_Odm->pChannel>= 52) &&(*pDM_Odm->pChannel <= 64)) ||
				((*pDM_Odm->pChannel >= 100) &&	(*pDM_Odm->pChannel <= 140)))
			{
				bDFSBand = TRUE;
				ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): bDFSBand = %d \n", bDFSBand));
			}
#endif
			if (priv->pmib->dot11RFEntry.tx2path && !bDFSBand) {
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG() : initial upper bound = 0x%x, lower bound = 0x%x\n",dm_dig_max,dm_dig_min));
	}
	...
	if(pDM_Odm->bLinked)
	{
		...
		else
		{
			...
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG() : modified upper bound = 0x%x\n",pDM_DigTable->rx_gain_range_max));
	...
	*/
	...
	else
	{
		...
		else
		{
			...
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): Upper bound for DFS band is fixed to 0x28 when blinked is false. \n"));
		}
		DIG_Dynamic_MIN = dm_dig_min;
	...
	if(bDFSBand)
	{
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): DFS band Abnormally false alarm case: lower bound = 0x%x \n", pDM_DigTable->rx_gain_range_min));
	}
	else
	{
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): Abnormally false alarm case: lower bound = 0x%x \n", pDM_DigTable->rx_gain_range_min));
	}
	...
	if(pDM_Odm->bLinked)
	{
		...
		if(FirstConnect)
		{
			...
			if(!bDFSBand)
			{
				...
				ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("DIG: First Connect, IGI does on-shot to RSSI value\n"));
			}
			else
			{
				if(pDM_Odm->RSSI_Min > 0x28)
					...
					CurrentIGI = pDM_Odm->RSSI_Min;
				ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("DIG: First Connect, DFS band one-shot to 0x28 upmost\n"));
			}

			memset(pDM_DigTable->DIG_record, 0, sizeof(pDM_DigTable->DIG_record));
			memset(pDM_DigTable->FA_record, 0, sizeof(pDM_DigTable->FA_record));
			pDM_DigTable->RSSI_pre = pDM_Odm->RSSI_Min;
			pDM_DigTable->DIG_record[0] = CurrentIGI;
			pDM_DigTable->FA_record[0] = pFalseAlmCnt->Cnt_all;
			ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): initial record \n"));
		}
		else
		{	
			// if((pDM_Odm->RSSI_Min + 5) < pDM_DigTable->RSSI_pre || (pDM_DigTable->RSSI_pre + 5) < pDM_Odm->RSSI_Min)
			if((pDM_Odm->RSSI_Min + 5) < pDM_DigTable->RSSI_pre)
			{
				memset(pDM_DigTable->DIG_record, 0, sizeof(pDM_DigTable->DIG_record));
				memset(pDM_DigTable->FA_record, 0, sizeof(pDM_DigTable->FA_record));
				ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): reset DIG_record and FA_record\n"));
			}
			else
			{
				for(index = 19; index > 0; index--)
				{
					pDM_DigTable->DIG_record[index] = pDM_DigTable->DIG_record[index - 1] ;
					pDM_DigTable->FA_record[index] = pDM_DigTable->FA_record[index - 1];
				}
			}
			pDM_DigTable->RSSI_pre = pDM_Odm->RSSI_Min;
			pDM_DigTable->DIG_record[0] = CurrentIGI;
...
#endif
				{
					if((pDM_DigTable->CurIGValue >= 0x32) && (pFalseAlmCnt->Cnt_all > 1000))
					{
						CurrentIGI = pDM_DigTable->rx_gain_range_max;
						ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): DIG one-shot to upperbound = 0x%x\n", CurrentIGI));
					}
					else
					{
						...
						{
							ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): adjust IGI for DFS band\n"));
							...
							if(!bDFSBand)
							{
								...
								{
									if(pDM_DigTable->DIG_record[index] == CurrentIGI - 2)
									{
										DIG_count++;
										FA_ave = FA_ave + pDM_DigTable->FA_record[index];
									}
								}

                                if (DIG_count != 0) {
								    FA_ave = FA_ave / DIG_count;
                                }

								if(FA_ave <= 300)
								{
									CurrentIGI = CurrentIGI - 2;//pDM_DigTable->CurIGValue =pDM_DigTable->PreIGValue-1;
									ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): current IGI = current IGI - 2\n"));
								}
								ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): FA_ave = %d, DIG_count = %d \n", FA_ave, DIG_count)); 
							}
							else
								CurrentIGI = CurrentIGI - 2;
						}
	...
	else
	{
		...
		if(!bDFSBand)
		{
			if(pFalseAlmCnt->Cnt_all > 5000)
				CurrentIGI = CurrentIGI + 4;//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+2;
			else if (pFalseAlmCnt->Cnt_all > 4000)
				CurrentIGI = CurrentIGI + 2;//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+1;
			else if(pFalseAlmCnt->Cnt_all < 250)
				CurrentIGI = CurrentIGI - 2;//pDM_DigTable->CurIGValue =pDM_DigTable->PreIGValue-1;	
		}
		else
		{
			if(pFalseAlmCnt->Cnt_all > 2000)
				CurrentIGI = CurrentIGI + 4;//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+2;
			else if (pFalseAlmCnt->Cnt_all > 1000)
				CurrentIGI = CurrentIGI + 2;//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+1;
			else if(pFalseAlmCnt->Cnt_all < 250)
				CurrentIGI = CurrentIGI - 2;//pDM_DigTable->CurIGValue =pDM_DigTable->PreIGValue-1;	
		}
	...
	)
{
	...
	if ( (priv->pmib->dot11RFEntry.ther) && ((priv->up_time % priv->pshare->rf_ft_var.tpt_period) == 0)){
		...
		if (pDM_Odm->SupportICType==ODM_RTL8881A) {			
			if (!pDM_Odm->RFCalibrateInfo.TM_Trigger) {		//at least delay 1 sec
				ODM_SetRFReg(pDM_Odm, RF_PATH_A, RF_T_METER_8821A, BIT17 | BIT16, 0x3);
				pDM_Odm->RFCalibrateInfo.TM_Trigger = 1;
				return;				
			}
	...
	)
{
	...
	if(pDM_Odm->SupportICType & (ODM_RTL8192C | ODM_RTL8192D | ODM_RTL8192E))
	{
 	...
 	ODM_AntennaDiversityInit_8192E(pDM_Odm);
#endif
	}
	else if(pDM_Odm->SupportICType == ODM_RTL8188E)
	{
	...
	int initial_gain)
{
	...
	// scan in-band PSD
	//ODM_SetRFReg(pDM_Odm, RF_PATH_A, RF_CHNLBW, 0x3FF, priv->pmib->dot11RFEntry.dot11channel);
	...
	//ODM_SetRFReg(pDM_Odm, RF_PATH_A, RF_CHNLBW, bRFRegOffsetMask, priv->pmib->dot11RFEntry.dot11channel); //請自行找切channel相關的code
	...
	)
{
	...
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_MP, ODM_DBG_LOUD, ("===> ODM_MPT_DIG, pBandType = %d\n", *pDM_Odm->pBandType));
	...
	if (*pDM_Odm->pBandType == 1) {   // for 5G
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_MP, ODM_DBG_LOUD, ("RX OK = %d\n", RXOK_cal));
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_MP, ODM_DBG_LOUD, ("RSSI = %d\n", pDM_Odm->RxPWDBAve));
		...
		else
		{
			...
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_MP, ODM_DBG_LOUD, ("DIG = (%x, %x), Cnt_all = %d, Cnt_Ofdm_fail = %d, Cnt_Cck_fail = %d\n", C50, E50, pFalseAlmCnt->Cnt_all, pFalseAlmCnt->Cnt_Ofdm_fail, pFalseAlmCnt->Cnt_Cck_fail));
		}
	}
	else {
		if (pDM_Odm->MPDIG_2G == FALSE){
			ODM_Write1Byte( pDM_Odm, ODM_REG(IGI_A,pDM_Odm), 0x20);
			if (pDM_Odm->SupportICType != ODM_RTL8188E)
				ODM_Write1Byte( pDM_Odm, ODM_REG(IGI_B,pDM_Odm), 0x20);
		}
