...
static const iw_handler rtl_iwhandler[] =
{
	...
	(iw_handler) NULL,						/* SIOCGIWRANGE  *//*move rtl_giwrange to rtl8192cd_ioctl by shipeng*/
...
#endif // SIOCSIWMLME //
	(iw_handler) NULL,						/* SIOCGIWAPLIST *//*move rtl_iwaplist to rtl8192cd_ioctl by shipeng*/
#ifdef SIOCGIWSCAN
...
int rtl8192cd_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCGIWNAME:
		...
		ret = 0;
		break;
	/*begin add for iwlist by shipeng*/
	case SIOCGIWAPLIST:
	{
		ret = rtl_iwaplist(dev, NULL, &wrq->u.data, NULL);
		break;
	}
	/*end add for iwlist by shipeng*/
#ifdef CONFIG_RTL_COMAPI_WLTOOLS
	case SIOCGIFHWADDR:
