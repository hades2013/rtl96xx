				...
				union iwreq_data *wrqu, char *extra)
{
	...
	/*begin add by shipeng*/
	if(extra == NULL)
		range = (struct iw_range *)wrqu->data.pointer;
	else
			...
			struct iw_point *data, char *extra)
{
	...
	{
		data->length = i;
		copy_to_user((char *)(data->pointer), &addr, i*sizeof(addr[0]));
		data->flags = 1;		/* signal quality present (sort of) */
		copy_to_user((char *)(data->pointer) + i*sizeof(addr[0]), &qual, i*sizeof(qual[i]));
	}
	/*begin modify by shipeng*/
