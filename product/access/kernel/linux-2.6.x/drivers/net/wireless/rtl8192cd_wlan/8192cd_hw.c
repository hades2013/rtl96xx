...
#if defined (TXPWR_LMT_NEWFILE) || defined(RTK_AC_SUPPORT)

#if defined(TXPWR_LMT_88E) || defined(TXPWR_LMT_92EE)
void assign_target_value_88e_new(struct rtl8192cd_priv *priv, unsigned int *u4bRegOffset, unsigned char *tmp_byte)
{	
	//CCK-11M
	if(*u4bRegOffset == rTxAGC_B_CCK11_A_CCK2_11)
	if(tmp_byte[1] != 0)
		priv->pshare->tgpwr_CCK_new[RF_PATH_A] = tmp_byte[1];
...
unsigned int generate_u4bRegValue_88e_new(struct rtl8192cd_priv *priv, unsigned int *u4bRegOffset, unsigned char *tmp_byte)
{
	...
	else if(*u4bRegOffset == rTxAGC_B_CCK1_55_Mcs32) // {1M 2M 5.5M NA}
	{
		reg_value = RTL_R32(rTxAGC_B_CCK1_55_Mcs32); 
		tmp_reg[0]= (reg_value & (0xff));
	...
	else if(*u4bRegOffset == rTxAGC_B_CCK11_A_CCK2_11) //{2M 5.5M 11M NA} {NA NA NA NA 11M}
	{
		reg_value = RTL_R32(rTxAGC_B_CCK11_A_CCK2_11); 
		tmp_reg[0]= 0;
		tmp_reg[1]= 0;
		...
		reg_value = gen_reg_value(tmp_reg);
		RTL_W32(rTxAGC_B_CCK11_A_CCK2_11, reg_value);
		return reg_value;
	}
...
unsigned int generate_u4bRegValue_8812_new(struct rtl8192cd_priv *priv, unsigned int *u4bRegOffset, unsigned char *tmp_byte)
{
	...
	{
		tmp_reg[0]= priv->pshare->tgpwr_VHT1S_new[RF_PATH_A] - tmp_byte[0];
		tmp_reg[1]= priv->pshare->tgpwr_VHT1S_new[RF_PATH_A] - tmp_byte[1];
	...
	{
		tmp_reg[0]= priv->pshare->tgpwr_VHT1S_new[RF_PATH_B] - tmp_byte[0];
		tmp_reg[1]= priv->pshare->tgpwr_VHT1S_new[RF_PATH_B] - tmp_byte[1];
...
#if 0
	printk("[0x%x] tmp_byte = %02d-%02d-%02d-%02d ", *u4bRegOffset, 
		tmp_byte[3], tmp_byte[2], tmp_byte[1], tmp_byte[0]);
#endif

#if defined(TXPWR_LMT_88E) || defined(TXPWR_LMT_92EE)//CONFIG_RTL_88E_SUPPORT
...
#ifdef TXPWR_LMT_NEWFILE
void find_pwr_limit_new(struct rtl8192cd_priv *priv, int offset)
{
	int tmp = ch2idx(priv->pshare->working_channel);
	int tmp2 = ch2idx(priv->pmib->dot11RFEntry.dot11channel); //for CCK, OFDM & 20M bw
	int tmp3 = 0;

	int working_channel_40m = priv->pmib->dot11RFEntry.dot11channel;

	if (priv->pshare->is_40m_bw == 2)
	{
		if (offset == 1)
			working_channel_40m -= 2;
		else
			working_channel_40m += 2;

		tmp3 = ch2idx(working_channel_40m);
	}

#if 0
	printk("[BW%dM]:tmp %d %d %d:channel %d %d %d\n", 
		20*(1 << priv->pshare->is_40m_bw),
		tmp, tmp2, tmp3, 
		priv->pshare->working_channel, priv->pmib->dot11RFEntry.dot11channel, working_channel_40m);
#endif
	...
	if ((tmp>=0) && (tmp2>=0)) {
		...
		if (priv->pshare->is_40m_bw == 2){ //BW=80M
			priv->pshare->txpwr_lmt_HT1S = priv->pshare->ch_pwr_lmtHT40_1S[tmp3];
			priv->pshare->txpwr_lmt_HT2S = priv->pshare->ch_pwr_lmtHT40_2S[tmp3];
			priv->pshare->txpwr_lmt_VHT1S = priv->pshare->ch_pwr_lmtVHT80_1S[tmp];
			priv->pshare->txpwr_lmt_VHT2S = priv->pshare->ch_pwr_lmtVHT80_2S[tmp];
		} else if (priv->pshare->is_40m_bw == 1){ //BW=40M
			priv->pshare->txpwr_lmt_HT1S = priv->pshare->ch_pwr_lmtHT40_1S[tmp];
			priv->pshare->txpwr_lmt_HT2S = priv->pshare->ch_pwr_lmtHT40_2S[tmp];
			priv->pshare->txpwr_lmt_VHT1S = priv->pshare->ch_pwr_lmtHT40_1S[tmp];
			priv->pshare->txpwr_lmt_VHT2S = priv->pshare->ch_pwr_lmtHT40_2S[tmp];
		} else { //BW=20M
			...
			priv->pshare->txpwr_lmt_VHT1S = priv->pshare->ch_pwr_lmtHT20_1S[tmp2];
			priv->pshare->txpwr_lmt_VHT2S = priv->pshare->ch_pwr_lmtHT20_2S[tmp2];
		}
...
int PHY_ConfigTXLmtWithParaFile_new(struct rtl8192cd_priv *priv)
{
	...
	if (GET_CHIP_VER(priv) == VERSION_8881A) {
		printk("[%s][TXPWR_LMT_8881A_new]\n",__FUNCTION__);
		next_head = data_TXPWR_LMT_8881A_new_start;
		read_bytes = (int)(data_TXPWR_LMT_8881A_new_end - data_TXPWR_LMT_8881A_new_start);
	}
	else
#endif			
#ifdef CONFIG_WLAN_HAL_8192EE
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		printk("[%s][TXPWR_LMT_8881A_new]\n",__FUNCTION__);
		next_head = data_TXPWR_LMT_92EE_new_start;
		read_bytes = (int)(data_TXPWR_LMT_92EE_new_end - data_TXPWR_LMT_92EE_new_start);
	}
	else
#endif	
...
int PHY_ConfigBBWithParaFile(struct rtl8192cd_priv *priv, int reg_file)
{
	...
	int 			   file_format = TWO_COLUMN;
	unsigned char      *mem_ptr, *line_head, *next_head = NULL;
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_WLAN_HAL)//TXPWR_LMT_8812 TXPWR_LMT_88E
...
#if 1//def TXPWR_LMT_8812
			{
				printk("[%s][PHY_REG_PG_8812_new]\n",__FUNCTION__);
...
//#if defined(CONFIG_RTL_8812_SUPPORT) || defined(TXPWR_LMT_88E) ||defined(CONFIG_WLAN_HAL_8881A)//TXPWR_LMT_8812 TXPWR_LMT_88E
#if defined (TXPWR_LMT_NEWFILE) || defined(RTK_AC_SUPPORT)
		if (reg_file == PHYREG_PG)
#if defined (TXPWR_LMT_NEWFILE)			
		if ((GET_CHIP_VER(priv) == VERSION_8812E)||(GET_CHIP_VER(priv) == VERSION_8188E) || GET_CHIP_VER(priv)==VERSION_8881A ||(GET_CHIP_VER(priv) == VERSION_8192E))
#elif defined(RTK_AC_SUPPORT)
		if ((GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv)==VERSION_8881A))
#endif
		{
...
void SwChnl(struct rtl8192cd_priv *priv, unsigned char channel, int offset)
{
	...
	if(under_apmode_repeater(priv))
	{
		...
		{		
			...
			//printk("DFS Channel=%d, 0x522=0x%x\n", channel, RTL_R8(TXPAUSE));		
		}
	}
#endif
#endif
...
//_TXPWR_REDEFINE ?? Working channel also apply to 5G ?? what if channel = 165 + 2 or 36 -2 ??
...
#ifdef TXPWR_LMT
	{
		...
		if ((GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8188E) || IS_HAL_CHIP(priv))
			find_pwr_limit_new(priv, offset);
		else
#endif
			find_pwr_limit(priv);
		}
...
#if defined(CONFIG_RTL_92E_SUPPORT) && defined(CONFIG_HW_ANTENNA_DIVERSITY)	
...
#ifdef __ECOS
	*((volatile unsigned long *)0xB800311C) |=  1 << 23; //kick watchdog
#else
	#ifndef CONFIG_ARCH_LUNA_SLAVE
	static unsigned long wtval;
	wtval = *((volatile unsigned long *)0xB800311C);
...
#if 1   //def CONFIG_RTL_8812AR_VN_SUPPORT
...
void RTLWIFINIC_GPIO_config(unsigned int gpio_num, unsigned int direction)
{
	...
	if ((gpio_num >= 0) && (gpio_num <= 7)) {
		...
		if (direction == 0x01) {
#ifdef CONFIG_RTL8672
			gpioConfig(gpio_num+RTL8192CD_GPIO_0, GPIO_FUNC_INPUT);
#endif
			RTL_W32(GPIO_PIN_CTRL, RTL_R32(GPIO_PIN_CTRL) & ~(BIT(gpio_num + 24) | BIT(gpio_num + 16)));
			return;
		} else if (direction == 0x10) {
#ifdef CONFIG_RTL8672
			gpioConfig(gpio_num+RTL8192CD_GPIO_0, GPIO_FUNC_OUTPUT);
#endif
