...
#if defined(CONFIG_USE_PCIE_SLOT_1) && !defined(CONFIG_ARCH_LUNA_SLAVE)
	#ifdef CONFIG_RTL8672
...
{
	panic_printk("\n\n#######################################################\n");
	panic_printk("RX_DESC=%d TX_DESC=%d\n",NUM_RX_DESC,NUM_TX_DESC);
	panic_printk("#######################################################\n\n");
	...
	if (!priv->pmib->dot11DFSEntry.disable_DFS &&
		((timer_pending(&priv->ch52_timer) && (priv->pmib->dot11RFEntry.dot11channel == 52)) ||
		 ...
		 (timer_pending(&priv->ch140_timer) && (priv->pmib->dot11RFEntry.dot11channel == 144)))) {
...
// TODO: check the following compile flag...ex: USE_TXQUEUE...  
		...
		if (priv->pmib->dot11RFEntry.tx2path) {
			...
			if (APSD_ENABLE) {
				priv->pmib->dot11RFEntry.tx2path = 0;
				panic_printk("Disable tx2path due to wmm-ps\n");
			}
#endif				
		}
		if (priv->pmib->dot11RFEntry.txbf) {
...
#ifdef CONFIG_RTL_8812_SUPPORT
	if (GET_CHIP_VER(priv)==VERSION_8812E)
		priv->pshare->fw_free_space = RTL8812_NUM_STAT - 2;
	else
...
//		priv->ps_ctrl = 0x11;
	...
	if (IS_ROOT_INTERFACE(priv)) {
		if (priv->pmib->miscEntry.vap_enable) {
			for (i=0; i<RTL8192CD_NUM_VWLAN; i++) {
				extern void feed_once_watchdog(void);
...
/*begin modify by shipeng 2013-08-11*/
//#ifdef CONFIG_RTL8672
#ifdef CONFIG_RTL8672NIC
/*end modify by shipeng 2013-08-11*/
#ifndef CONFIG_ARCH_LUNA_SLAVE
	{
