...
void rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
...
#endif
	/*begin add for add vlan tag by shipeng 2014-01-14*/
	...
	if((0xFF == pskb->data[0]) && (0xFF == pskb->data[1]) && (0xFF == pskb->data[2]) &&
    (0xFF == pskb->data[3]) && (0xFF == pskb->data[4]) && (0xFF == pskb->data[5]))
    {
    	/*Do Nothing*/
    }
	else if(!memcmp(fe_addr, pskb->data, MACADDRLEN))
	{
		skb_push_qtag(pskb,s_ui_management_vlan,0);
		pskb->vlan_tci = s_ui_management_vlan;		
	}
	else
	{
