...
struct pppol2tp_session
{
	...
	union {
		...
		struct pppol2tpin6_addr  tunnelin6_addr;
	}u;
#endif
				...
				int udp_len)
{
	...
	if (!(skb->dst->dev->features & NETIF_F_IPV6_CSUM)) {
...
pppol2tp_session_find(struct pppol2tp_tunnel *tunnel, u16 session_id)
{
	...
	read_lock_bh(&tunnel->hlist_lock);
	hlist_for_each_entry(session, walk, session_list, hlist) {
		if (((session->tunnel_sock->sk_family==AF_INET) && (session->u.tunnel_addr.s_session == session_id)) ||
			((session->tunnel_sock->sk_family==AF_INET6) && (session->u.tunnelin6_addr.s_session == session_id))) {
			read_unlock_bh(&tunnel->hlist_lock);
			return session;
		}
	}
...
static void pppol2tp_recv_dequeue_skb(struct pppol2tp_session *session, struct sk_buff *skb)
{
	...
	if (session_sock->sk_state & PPPOX_BOUND) {
		...
		extern int fp_on;
		if (fp_on && (1 == vpn_ppp_down_fastpath(&po->chan, &skb, 0)))
			goto exit;
#endif
		nf_reset(skb);
					       ...
					       struct sk_buff *skb)
{
	...
	if (sk->sk_family == PF_INET6) {
		if (!uh->check) {
			printk(KERN_INFO "L2TP: IPV6: checksum is 0\n");
			return 1;
		}
...
#else
	if (session->tunnel_sock->sk_family==AF_INET)
	{
		*bufp++ = htons(session->u.tunnel_addr.d_tunnel);
		*bufp++ = htons(session->u.tunnel_addr.d_session);
	}
	else
	{
		*bufp++ = htons(session->u.tunnelin6_addr.d_tunnel);
		*bufp++ = htons(session->u.tunnelin6_addr.d_session);
	}
#endif
	if (session->send_seq) {
...
#ifdef CONFIG_IPV6_VPN
	if (sk_tun->sk_family == AF_INET6)
		iphdr_len = sizeof(struct ipv6hdr);
#endif

	/* Allocate a socket buffer */
	error = -ENOMEM;
	skb = sock_wmalloc(sk, NET_SKB_PAD + iphdr_len +
...
#ifdef CONFIG_IPV6_VPN
	if (sk_tun->sk_family == AF_INET6)
		l2tp_xmit_ipv6_csum(sk_tun, skb, udp_len);
	else
#endif
...
int isL2tpPacket(struct sk_buff *skb)
{
	struct iphdr *iph = (struct iphdr *)skb->data;
	struct udphdr *uh = (struct udphdr *)((__u32 *)iph + iph->ihl);
	
	if ((IPPROTO_UDP == iph->protocol) && (iph->ihl == 5) && (uh->source == 1701))
		return 1;

	return 0;
}

int l2tp_up_fastpath(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	session = (struct pppol2tp_session *)(sk->sk_user_data);
	if (session == NULL) {
		printk("%s session is NULL.\n", __func__);
		goto abort;
	}
	
	sk_tun = session->tunnel_sock;
	if (sk_tun == NULL) {
		printk("%s tunnel sock is NULL.\n", __func__);
		goto abort;
	}
	tunnel = (struct pppol2tp_tunnel *)(sk_tun->sk_user_data);
	if (tunnel == NULL) {
		printk("%s tunnel is NULL.\n", __func__);
		goto abort;
	}

	hdr_len = pppol2tp_l2tp_header_len(session);
	headroom = sizeof(struct iphdr) + sizeof(struct udphdr) + hdr_len;
	
	if (skb_headroom(skb) < headroom) {
		struct sk_buff *tmpskb;
		
		tmpskb = skb_realloc_headroom(skb, headroom);
	...
	if (NULL == skb->dst) {
		printk("%s fatal error, dst_entry is NULL.\n", __func__);
		ip_queue_xmit(skb, 1);
		return 1;
	}
	...
	iph->id       = htons(inet_sk(sk_tun)->id);
	inet_sk(sk_tun)->id += 1;
...
int l2tp_down_fastpath(struct sk_buff **skb)
{
	struct iphdr *iph;
	struct udphdr *uh;
	unsigned char *ptr, *optr;
	...
	if(iph->frag_off & htons(0x3fff))
		return -1;
	uh = (struct udphdr *)((__u32 *)iph + iph->ihl);
	
	ptr = (unsigned char *)((__u8 *)uh + 8);

	/* Get length of L2TP packet */
	length = (*skb)->len - (ptr - optr);
	...
	tunnel = pppol2tp_tunnel_find(&init_net, tunnel_id);
	if (NULL == tunnel) {
		return -1;
	}
...
#endif //endof CONFIG_RTL867X_IPTABLES_FAST_PATH
...
static int pppol2tp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
...
#else
	if (sk_tun->sk_family == AF_INET) {
		headroom = NET_SKB_PAD + sizeof(struct iphdr) +
			sizeof(struct udphdr) + hdr_len + sizeof(ppph);
	}
	else {
		headroom = NET_SKB_PAD + sizeof(struct ipv6hdr) +
			sizeof(struct udphdr) + hdr_len + sizeof(ppph);
	}
#endif
	...
	//august add on June 1st: if vc*/nas* is del, neither the l2tp, 
	//skb->dst will be NULL, and it will crash.
	if(NULL == skb->dst)
		goto abort;
...
#else
			if (((session->tunnel_sock->sk_family==AF_INET) && (session->u.tunnel_addr.s_session != 0)) ||
				((session->tunnel_sock->sk_family==AF_INET6) && (session->u.tunnelin6_addr.s_session != 0)))
#endif
			{
			    ...
			    int sockaddr_len, int flags)
{
...
#ifdef CONFIG_IPV6_VPN
	struct sockaddr_pppol2tpin6 *sp6 = (struct sockaddr_pppol2tpin6 *) sp;
#endif
	...
	//QL 20120514 below variable add for IPv6 compatible.
	u16 tunnel_id, session_id;
	u16 peer_tunnel_id, peer_session_id;
	int fd;
	int error = 0;
	...
	if ((session_id == 0) && (peer_session_id == 0)) {
		...
		tunnel = pppol2tp_tunnel_find(sock_net(sk), tunnel_id);
	...
	if (tunnel->sock->sk_family == AF_INET6) {
		session->u.tunnelin6_addr = sp6->pppol2tp;
		sprintf(&session->name[0], "sess %hu/%hu",
			session->u.tunnelin6_addr.s_tunnel,
			session->u.tunnelin6_addr.s_session);
		
		session->stats.tunnel_id  = session->u.tunnelin6_addr.s_tunnel;
		session->stats.session_id = session->u.tunnelin6_addr.s_session;
	}
	else {
		session->u.tunnel_addr = sp->pppol2tp;
		sprintf(&session->name[0], "sess %hu/%hu",
			session->u.tunnel_addr.s_tunnel,
			session->u.tunnel_addr.s_session);
		
		session->stats.tunnel_id  = session->u.tunnel_addr.s_tunnel;
		session->stats.session_id = session->u.tunnel_addr.s_session;
	}
#endif
...
#else
	if (((tunnel->sock->sk_family == AF_INET6) && 
		(session->u.tunnelin6_addr.s_session == 0) && (session->u.tunnelin6_addr.d_session == 0)) ||
		((tunnel->sock->sk_family == AF_INET) && 
		(session->u.tunnel_addr.s_session == 0) && (session->u.tunnel_addr.d_session == 0))
		) {
		error = 0;
		sk->sk_user_data = session;
		goto out_no_ppp;
			    ...
			    int *usockaddr_len, int peer)
{
	...
	if (session->tunnel_sock->sk_family == AF_INET)
	{
		sp.sa_family	= AF_PPPOX;
		sp.sa_protocol	= PX_PROTO_OL2TP;
		memcpy(&sp.pppol2tp, &session->u.tunnel_addr,
			   sizeof(struct pppol2tp_addr));
	...
	{
		sp6.sa_family	= AF_PPPOX;
		sp6.sa_protocol	= PX_PROTO_OL2TP;
		memcpy(&sp6.pppol2tp, &session->u.tunnelin6_addr,
			   sizeof(struct pppol2tpin6_addr));

		len = sizeof(struct sockaddr_pppol2tpin6);
		memcpy(uaddr, &sp, len);
	}
#endif
...
#else
	if (((session->tunnel_sock->sk_family==AF_INET) && (session->u.tunnel_addr.s_session == 0) &&
		(session->u.tunnel_addr.d_session == 0)) || 
		((session->tunnel_sock->sk_family==AF_INET6) && (session->u.tunnelin6_addr.s_session == 0) &&
		(session->u.tunnelin6_addr.d_session == 0)))
#endif
	{
		err = -EBADF;
...
static void pppol2tp_seq_session_show(struct seq_file *m, void *v)
{
	...
	if (session->tunnel_sock->sk_family==AF_INET)
	{
		seq_printf(m, "  SESSION '%s' %08X/%d %04X/%04X -> "
			   "%04X/%04X %d %c\n",
			   session->name,
			   ntohl(session->u.tunnel_addr.addr.sin_addr.s_addr),
			   ntohs(session->u.tunnel_addr.addr.sin_port),
			   session->u.tunnel_addr.s_tunnel,
			   session->u.tunnel_addr.s_session,
			   session->u.tunnel_addr.d_tunnel,
			   session->u.tunnel_addr.d_session,
			   session->sock->sk_state,
			   (session == session->sock->sk_user_data) ?
			   'Y' : 'N');
	}
	...
	{
		seq_printf(m, "  SESSION '%s' %pI6/%d %04X/%04X -> "
			   "%04X/%04X %d %c\n",
			   session->name,
			   &session->u.tunnelin6_addr.addr.sin6_addr,
			   ntohs(session->u.tunnelin6_addr.addr.sin6_port),
			   session->u.tunnelin6_addr.s_tunnel,
			   session->u.tunnelin6_addr.s_session,
			   session->u.tunnelin6_addr.d_tunnel,
			   session->u.tunnelin6_addr.d_session,
			   session->sock->sk_state,
			   (session == session->sock->sk_user_data) ?
			   'Y' : 'N');
	}
