...
#if (CPU_TYPE==CPU_TYPE_5181)
#define DUAL_COUNTER_MODE	0 //5181 do not have dual counter mode
u32 cp3_type_select=
#if 1 /* Inst */
...
u32 cp3_type_select= /* Counter0 */((CP3CNT_DCACHE_MISS_TIMES)<< 0) |
			 ...
			 /* Counter3 */((CP3CNT_TOTAL_CYCLES)<<24);
...
void profile_proc_init(void)
{
	...
	if ( create_proc_read_entry ("reset", 0644, rg_kernel.proc_rg,
			profile_init_one, (void *)NULL) == NULL ) {
		printk("create proc reset failed!\n");
		return;
	}
	if ( create_proc_read_entry ("dump", 0644, rg_kernel.proc_rg,
