...
int32 rtl_igmpMldProcess(uint32 moduleIndex, uint8 * macFrame,  uint32 portNum, uint32 *fwdPortMask)
{
	...
	DBG_IGMPSNOOPING_PRK("Enter %s(moduleIndex:%d  portNum=%d   )\n",__func__,moduleIndex,portNum);
	...
	for(moduleIndex=0; moduleIndex<MAX_MCAST_MODULE_NUM ;moduleIndex++)
	{
		if(rtl_mCastModuleArray[moduleIndex].enableSnooping==TRUE)
		{
			...
			for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
				...
				{
					groupCnt++;
					rtlglue_printf( "    [%d] Group address:%d.%d.%d.%d\n",groupCnt,//hashIndex
					...
					{	
						...
						clientCnt++;
						rtlglue_printf( "        <%d>%d.%d.%d.%d\\port %d\\IGMPv%d\\",clientCnt,
						...
						rtlglue_printf( "%s",(clientEntry->groupFilterTimer>rtl_sysUpSeconds)?"EXCLUDE":"INCLUDE");
						if(clientEntry->groupFilterTimer>rtl_sysUpSeconds)
						{
							rtlglue_printf( ":%ds",clientEntry->groupFilterTimer-rtl_sysUpSeconds);
						}
						else
						{
							rtlglue_printf( ":0s");
						...
						{
							rtlglue_printf( "\\source list:");
						}

						while(sourceEntryPtr!=NULL)
						{
							rtlglue_printf( "%d.%d.%d.%d:",
							...
							{
								rtlglue_printf( "%ds",sourceEntryPtr->portTimer-rtl_sysUpSeconds);
							}
							else
							{
								rtlglue_printf( "0s");
			...
			for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
				...
				{
					groupCnt++;	
					rtlglue_printf( "    [%d] Group address:%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x\n",groupCnt,
					...
					{	
						...
						clientCnt++;
						rtlglue_printf( "        <%d>%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x\\port %d\\MLDv%d\\",clientCnt,
						...
						while(sourceEntryPtr!=NULL)
						{
							rtlglue_printf( "%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x:",
			...
			for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
				...
				while(mcastFlowEntry!=NULL)
				{
					...
					{
						rtlglue_printf( "    [%d] %d.%d.%d.%d-->",flowCnt,
...
static int _rtl_handle_igmpgroup_addhw(rtl_multicast_index_mapping_t* mapping_entry)
{
	...
	if(rtk_rg_multicastFlow_add(&mcFlow, &mapping_entry->hwflow_index)!=RT_ERR_RG_OK)
		return FAIL;
	else
		return SUCCESS;
}
...
{
	return rtk_rg_multicastFlow_del(mapping_entry->hwflow_index);
}
...
int rtl_handle_igmpgroup_change(rtl_multicastEventContext_t* param)	
{
	...
	DBG_IGMPSNOOPING_PRK("Enter %s\n",__func__);

    if(strlen(param->devName)==0)
    {		
		DBG_IGMPSNOOPING_PRK("return FAIL(%s %d)\n",__func__,__LINE__);
		return FAIL;
    }
    ...
    if(strcmp(param->devName,RG_IGMP_SNOOPING_MODULE_NAME)==0)
    {
		...
		DBG_IGMPSNOOPING_PRK("(%s %d) SUCCESS!! ",__func__,__LINE__);
		return SUCCESS;
	}
