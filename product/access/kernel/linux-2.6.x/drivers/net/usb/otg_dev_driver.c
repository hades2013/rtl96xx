...
static void read_bulk_callback(struct urb *urb)
{
	...
	//printk("dev->bulkin_start_data = %d, urb->actual_length= %d\n", dev->bulkin_start_data, urb->actual_length);
	dev->stats.rx_packets++;
	dev->stats.rx_bytes += urb->actual_length;
	data = dev->bulkin_start_data;
	for(i=0; i<urb->actual_length; i++) {
		if(d[i] != data) {
			printk("i = %d, dev->bulkin_start_data = %d\n", i, dev->bulkin_start_data);
			memDump(d, urb->actual_length, "urb->transfer_buffer:");
			dev->stats.rx_errors++;
...
static void intr_callback(struct urb *urb)
{
	...
	d = urb->transfer_buffer;	
	if((d[0] != ep3rec) || (urb->status !=0)) {
		print_ft2("F_usb2-%d: usb ep3  <%s,%d> urb->status = %d, d[0] = %d, ep3rec=%d\n", open_time, __func__, __LINE__, urb->status, d[0], ep3rec);
		eqc_fail_display();
		while(1);
	}

	ep3rec++;
	if( ep3rec== TEST_PKT_NUM) {
		print_ft2("P_usb1-%d\n", open_time);
