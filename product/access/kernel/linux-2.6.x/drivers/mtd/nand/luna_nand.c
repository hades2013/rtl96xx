...
static int rtk_read_oob (struct mtd_info *mtd, u16 chipnr, int chunk_id, int len, u_char *oob_buf)
{
	...
	return _nand_read_chunk(this->g_databuf,oob_buf,chunk_id);
}
			...
			u_char *data_buf, u_char *oob_buf)
{
	...
	return _nand_read_chunk(data_buf,oob_buf,chunk_id);
...
{
	return _nand_write_chunk(data_buf,oob_buf,chunk_id);	
}
...
_nand_read_chunk(u8_t *chunk_buf, spare_u *spare, u32_t chunk_id) {
  ...
  /* DMA start read */
	...
	if(block>0){
		...
		spare->u8_oob[nandflash_info.bbi_swap_offset]=0xff;
  	}
...
#if 1
u32_t nand_bbt[NUM_WORD(MAX_BLOCKS)];
...
create_bbt(nand_dma_buf_t *dma_buf,u32_t *bbt_table) {
	...
	dbg_printf("create bbt\n");
	memset(bbt_table, 0x0, NUM_WORD(MAX_BLOCKS));
    for (b=1;b<nandflash_info.num_block;++b) {
		
		_nand_read_bbi(nandflash_info.bbi_raw_offset,b*num_chunk_per_block);
        if (bbi==0) continue;
...
_nand_init(){
...
#if 0	 //test chip,ECC rs code
	//RS chip
	if(SOC_ID_0371==soc_tcl_SOC_ID){
		if (nand_strap_pin & NAFC_RC ) {// 2k
	...
	//RS chip
	nandflash_info.addr_cycles = ((nand_strap_pin & 0x60000000) >>29)+3; // 3,4,5 address cycle
#else //formal chip, bch code, 
//READ page size strap ping
 switch( (nand_strap_pin & 0xC0000000) >>30){
	...
	//Assign flash_info ,number of block 
	nandflash_info.num_block  = 512;
...
#endif	                               
#endif	//end CONFIG_USE_PRELOADER_PARAMETERS
