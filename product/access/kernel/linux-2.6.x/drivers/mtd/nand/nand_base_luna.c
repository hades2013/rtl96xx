			...
			size_t *retlen, u_char *buf, u_char *oob_buf, struct nand_oobinfo *oobsel)
{
	...
	while (data_len < len) {
		chunk_id = logical_block*ppb + chunk_offset;  
		if(oob_buf!=NULL){
			//rc=_nand_read_chunk(&buf[data_len],&oob_buf[oob_len],page);	
			rc = this->read_ecc_page (mtd, this->active_chip, chunk_id, &buf[data_len], &oob_buf[oob_len]);
		}else{
			rc = this->read_ecc_page (mtd, this->active_chip, chunk_id, &buf[data_len], &oob_area);
			//rc=_nand_read_chunk(&buf[data_len],&oob_area,page);
		}
...
//	printk("oob_buf is \n");
	//		dump_mem(oob_buf,16);
			rc=_nand_write_chunk(&buf[data_len],&oob_buf[oob_len],chunk_id);			
		//	rc = this->write_ecc_page (mtd, this->active_chip, chunk_id, &buf[data_len], &oob_buf[oob_len], 0);
			
		}else{

			rc=_nand_write_chunk(&buf[data_len],oob_area,chunk_id);
		
			//rc = this->write_ecc_page (mtd, this->active_chip, chunk_id, &buf[data_len],oob_area, 0);
		}
...
int rtk_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	while(1){
		...
		printk("%s(%d): pagesize=%x chunk_size=%x ,oob=%x mtd=%p(%p)\n",__FUNCTION__,__LINE__,page_size,chunk_size,oob_size,mtd,&mtd->writesize);
