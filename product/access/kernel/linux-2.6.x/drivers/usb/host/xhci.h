...
// create by xhci_alloc_container_ctx func.
// the size only 1024 or 2048. (4*256 or 8*256)
...
#if 0
#define xhci_dbg(xhci, fmt, args...) \
	do { if (XHCI_DEBUG) printk(":"  fmt , ## args); } while (0)
#define xhci_info(xhci, fmt, args...) \
	do { if (XHCI_DEBUG) printk(":"  fmt , ## args); } while (0)
#define xhci_err(xhci, fmt, args...) \
	printk(":"  fmt , ## args)
...
static inline u64 xhci_read_64(const struct xhci_hcd *xhci,__u64 __iomem *regs);
#endif
