...
#else 
		c->mfc_parent_num++;
		c->mfc_parent_other[c->mfc_parent_num-1]=mfc->mfcc_parent;
#endif
...
#else
    //Let statistics be correct values. in /proc/net/ip_mr_cache and /proc/net/ip_mr_vif
    vif = cache->mfc_parent_other[i];
    cache->mfc_parent = vif;
#endif
