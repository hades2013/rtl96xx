...
extern int l2tp_down_fastpath(struct sk_buff **skb);
extern int isL2tpPacket(struct sk_buff *skb);
...
#ifdef CONFIG_RTL867X_VLAN_MAPPING
	if (re_vlan_loaded()) {
		if (re_vlan_skb_recv(skb) == NET_RX_DROP)
			return NET_RX_DROP;
	}
#endif
...
#ifdef CONFIG_PPTP
			extern int pptp_down_fastpath(struct sk_buff **skb);
			if (IPPROTO_GRE == skb->data[9]) {
				if (1 == pptp_down_fastpath(&skb))
					return NET_RX_SUCCESS;
			}
			else
#endif //endof CONFIG_PPTP
#ifdef CONFIG_PPPOL2TP
			/*check l2tp pkt according to udp src_port*/
			if (isL2tpPacket(skb))
			{
				if (1 == l2tp_down_fastpath(&skb))
					return NET_RX_SUCCESS;
			}
			else
#endif //endof CONFIG_PPPOL2TP
#ifdef CONFIG_NET_IPIP
			if (IPPROTO_IPIP == skb->data[9])
			{
				if (1 == ipip_down_fastpath(skb))
					return NET_RX_SUCCESS;
			}
			else
#endif//end of CONFIG_NET_IPIP
...
#ifdef CONFIG_RTL867X_MCAST_FAST_PATH
	skb_reset_network_header(skb);
	if (skb->protocol == ETH_P_IP && MULTICAST(ip_hdr(skb)->daddr))	  // Only IP packet for multicast fast path
	{
...
/*  Bridge fastpath  */
	...
	if (unlikely(br && (br->wlan_blocking != WLAN_BLOCK_DISABLE))) {
		goto SKIP_FBRIDGE;
	}
    #endif
...
ncls:
	...
	/*linux-2.6.19*/
	// Kaohj --- for VLAN tag
	/*begin del by wanghuanyu 2013-08-12*/
	/*使用标准vlan收发包接口*/
	#if 0
	if (skb->protocol == htons(ETH_P_8021Q)) {
	...
	#endif
	/*end del by wanghuanyu 2013-08-12*/
