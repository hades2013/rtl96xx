...
linux.preinit:
	...
	touch  $(LINUXDIR)/include/linux/kernel_size
...
busybox-menuconfig:
	$(MAKE) -C $(SOFTWAREDIR)/busybox-1.17.3 menuconfig
linux-modules:
...
rootfs:
	...
	install -p $(CONFIGDIR)/default.txt $(ROOTFSDIR)/etc/config/
...
#	ln -s $(SOFTWAREDIR)/opconn/dvrs/src/drv_managevlan.c $(SOFTWAREDIR)/shared/drv/drv_managevlan.c
	...
	cp -rf $(CONFIGDIR)/busybox.config $(SOFTWAREDIR)/busybox-1.17.3/.config
	$(MAKE) -C $(PRODUCTDIR) oldconfig
	$(MAKE) -C $(SOFTWAREDIR)/busybox-1.17.3 oldconfig
platform.distclean:
	rm -f $(SOFTWAREDIR)/shared/drv/port_config.c
	rm -f $(SOFTWAREDIR)/shared/drv/drv_managevlan.c
	rm -f $(KERNELDIR)/drv/hal/switch/drv_common.c
