...
#define MAC_FILE "/tmp/macdump"

#define ZTE_OAM_OUI 					{0x00, 0x0F, 0x3E}
#define ZTE_ORGSPEC_HDR_LEN			6 /* OUI(3) + Ext. Opcode(1)  + type(1) + port(1)*/
...
/*DHCP action define*/
#define ZTE_DHCP_DISABLE 		0x02
#define ZTE_DHCP_ENABLE 		0x01
...
#define OAM_ZTE_EXT_LEAF_MAC_ADDR_AGING2     0x00a4 /* get & set */
...
#define OAM_ZTE_EXT_LEAF_MGMT_IP_DHCP        0x801F /* set */
...
struct oam_zte_uni_mac_query_entry{
	u8_t    mac_addr[6];//first 6:mac  last two:vlanid
}__attribute__((packed));
...
typedef struct oam_zte_mgmt_dhcp oam_zte_mgmt_dhcp_t;

extern void CTC_getVlanMode(UINT32 lport, UINT8* pVlanMode);

extern int write_mac_ucast_dump(void);
extern int zte_oam_init(void);
extern void eopl_zte_ctc_get_req(u16_t leaf, u8_t** pp_byte, u8_t* pReplybuf, u16_t* pGenlen);
extern void eopl_zte_ctc_set_req(ctc_varInstant_t *pVarInstant, ctc_varDescriptor_t stVarDesc, u8_t** pp_byte, u8_t* pReplybuf, u16_t* pGenlen);
