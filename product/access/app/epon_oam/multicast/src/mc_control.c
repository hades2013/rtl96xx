...
uint8 MC_MODE_SWITCH_OCCUR = 0;/*Add by huangmingjian 2013-12-06 for Bug 294*/
...
BOOL CTC_CheckValidVidForMcTranslation(UINT32 vlanId);/*Add by huangmingjian 2013-10-17 for Bug 203,257*/
...
chksum(void* dataptr, int len)
{
  ...
  if (len == 1) {
    ...
    mc_printf("inet: chksum: odd byte %d\n", *(uint8 *) dataptr);
  }
...
void mcGroupSemLock(void)
{
  if(mcGroupLock == 0)
  {
      ...
      {
          mc_printf("Group lock initial failed.\n");
          return;
      }
      mcGroupLock = 1;
  }
...
void mcMsgSemLock(void)
{
  if (mcMsgLock == 0)
  {
      ...
      {
          mc_printf("mcMsg lock initial failed.\n");
          return;
      }
      mcMsgLock = 1;
  }
...
void mcTimerSemLock(void)
{
  if (mcTimerLock == 0)
  {
      ...
      {
          mc_printf("mcTimer lock initial failed.\n");
          return;
      }
      mcTimerLock = 1;
  }
...
int mcTagTranslationTableNum(void)
{
	uint32 i;
	int num=0;
	for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++) 
	{
	    ...
	    //  if(mcTagTranslationTable[i].portId == portId) { /* match */		
		   mc_printf("***caution ***  there is a same entry \r\n");		
	    	num++;
	     // }
	    }
	}
...
OPL_STATUS mcTagTranslationTableUservidExist(uint32 portId, uint32 userVid)
{
  uint32 i;
  ...
  for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++) 
  {
    if(mcTagTranslationTable[i].enable==OPL_ENABLE) 
	{
      if(mcTagTranslationTable[i].portId==portId && userVid==mcTagTranslationTable[i].userVid) 
	  { 
         return OPL_TRUE;
      }
    }
...
/*Begin add by huangmingjian 2013-10-26:Bug 203,257*/
OPL_STATUS mcVlanMapShow()
{
	unsigned int i = 0;
	printf("\nVlanMap table:\n");
	for(i=0; i<OP_MAX_VLAN_NUMBER; i++)
	{
		if(mcVlanMap[i])
		{
			printf("vlan:%d portMask:0x%02x\n", 
					i,
					mcVlanMap[i]);
		}
	}
...
OPL_STATUS mcTagTranslationShow()
{
	unsigned int i = 0;
	printf("\nTranslation table:\n");
	for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++)
	{
		if(mcTagTranslationTable[i].enable)
		{
			printf("port:%d mcVlan:%d userVlan:%d\n", 
					mcTagTranslationTable[i].portId,
					mcTagTranslationTable[i].mcVid,
					mcTagTranslationTable[i].userVid);
		}
	}
}

OPL_STATUS mcTagOperPerPortShow()
{
	unsigned int i = 0;
	printf("\nOper table:\n");
	for(i=1; i<(ODM_NUM_OF_PORTS+1); i++)
	{
		printf("port:%d oper:%d\n", 
				i,
				mcTagOperPerPort[i]);
	}
}
...
OPL_STATUS odmMcTagOperSet(uint32 portId, MC_TAG_OPER_MODE_t tagOper)
{
  ...
  /*Begin modified by huangmingjian 2013-12-06 for Bug 294*/
  /************************** check if it is the same tagOper exclude translation**************************/
  if(tagOper==mcTagOperPerPortGet(portId) &&
  	 tagOper!=MC_TAG_OPER_MODE_TRANSLATION) {
    MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "%s,%d, tagOper does not change, tagOper=%d.\r\n", __FUNCTION__, __LINE__, tagOper);
    return OPL_OK;
  }
  /*End modified by huangmingjian 2013-12-06 for Bug 294*/
...
OPL_STATUS mctag_translation_check(unsigned int portId, unsigned int mcVid, unsigned int userVid)
{
	int i = 0;
	int ret = 0;
	...
	for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++) {
		...
		if (mcTagTranslationTable[i].userVid == userVid){
		   ...
		   mc_printf("userVid which equal to existed userVid in the translation table is not allowed \r\n");
	   	   return OPL_ERROR; 
		}
		else if (mcTagTranslationTable[i].userVid == mcVid) {
	      mc_printf("mcVid which equal to existed userVid in the translation table is not allowed \r\n");
	      return OPL_ERROR;
		 }
		else if(mcTagTranslationTable[i].mcVid == userVid){
	      mc_printf("userVid which equal to existed mcVid in the translation table is not allowed \r\n");
	   	  return OPL_ERROR;
	  	}
		else if(mcTagTranslationTable[i].portId != portId && mcTagTranslationTable[i].mcVid == mcVid)
		{
	      mc_printf("difference portid:mcVid which equal to existed mcVid, but userVid is difference in the translation table is not allowed \r\n");
	   	  return OPL_ERROR;
	  	}		
	}
	/*Begin add by huangmingjian 2013-10-17 for Bug 203,257*/
	/* mc translation check: same with mc vlan is not allowed*/
	if(MC_MODE_CTC != mcControlBlockModeGet())
	{
		mc_printf(" is not MC_MODE_CTC");
		if(1==mcVlanExist(mcVid) || 1==mcVlanExist(userVid))
		{	mc_printf("ERROR:mcVlanExist....");	
			return OPL_ERROR;
		}
	}

	/* mc translation check: same with ctc translation old ,new vlan ,default vlan, or ctc tag vlan is not allowed*/
	if(FALSE == CTC_CheckValidVidForMcvlan(mcVid) || FALSE == CTC_CheckValidVidForMcvlan(userVid))
	{
		return OPL_ERROR;
	}
	/*End add by huangmingjian 2013-10-17 for Bug 203,257*/
...
//	printf(" %s %d\n",__FUNCTION__,__LINE__);
  ...
  #endif
  /*Begin modified by huangmingjian 2013-10-26:Bug 203,257*/
	#if 0
	/* checking the existing mcvid or uservid. if the translation vlan list has existed, it returns ok. Others, if the mcvid or uservid has existed, it returns the error code. */
	for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++) {
		if (mcTagTranslationTable[i].enable!=OPL_ENABLE ||
	...
	/*End add by huangmingjian 2013-10-17 for Bug 203,257*/
	#endif
	/*End modified by huangmingjian 2013-10-26:Bug 203,257*/
...
OPL_STATUS mcInitFromConfigFile()
{
  ...
  if(mcControlBlock.mcMode==MC_MODE_CTC) {
    ...
    if(mcControlBlock.ctcControlType==MC_CTL_GDA_MAC_VID) {
	  ...
	  /*MC_MODE_CTC do not give attention to vlan, just to mac and port, otherwise it can not leave successfully*/		
	  //MC_BITMAP = CTC_CONTROL_BITMAP_GDA_MAC_VLAN_ID;
	  MC_BITMAP = CTC_CONTROL_BITMAP_GDA_MAC;
	  /*End modified by huangmignjian 2013-12-13*/
    }
    if(mcControlBlock.ctcControlType==MC_CTL_GDA_GDA_SA_MAC) {
     MC_BITMAP = CTC_CONTROL_BITMAP_GDA_MAC_SA_MAC;
    }
...
OPL_STATUS mcSnoopingQueryAction(MC_PACKET_DESC_t *desc)
{
  ...
  if(desc->ingressTagType==TAG_TYPE_UNTAG) {
    ...
    for(i=ODM_START_PORT_NUN; i<ODM_NUM_OF_PORTS+1; i++) {
	  /*Begin modified by huangmingjian 2013-12-06 for Bug 297*/
	  /* add translation vlan check if exist*/
	  if(OPL_TRUE==mcVlanMapExist(i, desc->cVlan) ||
	  	 OPL_OK==mcTagTranslationTableGet(i, desc->cVlan, &userVid)) 
	  {
  ...
  if(desc->igmpGroupAdress==0&&desc->destIp==MULTICAST_ALL_HOST_GROUP) {
    ...
    for(i=ODM_START_PORT_NUN; i<ODM_NUM_OF_PORTS+1; i++) {
      if(OPL_TRUE==mcVlanMapExist(i, desc->cVlan)
	  	 || OPL_OK==mcTagTranslationTableGet(i, desc->cVlan, &userVid)) {/*Add by huangmingjian 2013-12-06 for Bug 297:add translation query*/
        desc->egressPortId = i;
...
int vosHWAddrGet(UINT8 *ifName, UINT8 *macAddr)
{
    struct ifreq ifreq;
    INT32 sock;
...
int vosIPAddrGet(UINT8 *ifName, char *ipAddr)
{
    struct ifreq ifreq;
    INT32 sock;
  ...
  uint8 *queryPkt)
{
  ...
  #if 0
  if(vosIPAddrGet("eth0", hostSrcIp) != NO_ERROR) {//modify by zhouguanhua EPN204QID0018
    MC_DEBUG(DEBUG_LEVEL_WARNING, "Query generate: Get host IP error.\r\n");
    return OPL_ERROR;
  }
  #else
  memset(&info, 0, sizeof(sys_network_t));
  extern int oam_ipc_get_sys_networking(sys_network_t *info);
  ...
  tmpIpAddr = (char *)inet_ntoa(info.netif.ip);
  strcpy(hostSrcIp, tmpIpAddr);
  #endif
  ...
  /* If no mcVlan configured, set vid as pvid */
  	...
  	mc_printf("mcSnoopingQueryPacketGenerate: fatal error\n");
...
OPL_STATUS mcSnoopingNonFastLeaveAction(MC_PACKET_DESC_t *desc)
{
  ...
  if(group->querycnt==mcControlBlockLastQueryCountGet()) {
   ...
   /*End modify by zhouguanhua 2012/12/12 of EPN204QID0005*/
      ...
      RETURN_VAL_IF_FAIL(ret == OPL_OK, ret);/*Open by huangmingjian 2013-12-06*/
...
OPL_STATUS mcSnoopingLeaveAction(MC_PACKET_DESC_t *desc)
{
  ...
  unsigned int curr_hostPortId = 1;
  MC_GROUP_t *tmp_group = NULL;
  ...
  if(MULTICAST_ALL_ROUTERS_GROUP!=desc->destIp) {
   ...
   /*add by zhouguanhua 2013/8/29*/
  /*EPN104N recieve untag data will take  manage vlan,so it must been deleted tag */
  extern int oam_ipc_get_sys_mvlan(sys_mvlan_t *mvlan); 
  int ret_ipc;  sys_mvlan_t info;    
  ret_ipc = oam_ipc_get_sys_mvlan(&info);  
  if(IPC_STATUS_OK != ret_ipc)	  
  {	
        MC_DEBUG(DEBUG_LEVEL_WARNING, "SnoopingJoinAction: Get mvlan error.\r\n");    
        return OPL_ERROR;		   

  }  
  if(desc->cVlan == info.m_vlan)   
  {     
    /* untag */    
    desc->cVlan = 0; 
  }

  /*end add by zhouguanhua 2013/8/29*/
  /************************** group and host check **************************/
  group = mcGroupListGet(desc->ingressPortId, desc->igmpGroupMac, desc->igmpGroupAdress, desc->cVlan, MC_BITMAP);
  if(!group) { /* gruop does not exist */
  ...
  /* host exists */
  curr_hostPortId = group->hostPortId;
  ...
  if(mcControlBlockLeaveModeGet()==MC_LEAVE_MODE_FAST) {
    ...
    /*Begin add by huangmingjian 2013-12-12 for Bug 295*/
	/* 
	   resovle the problem:
	   snooping fastleave : such as uni1 just only has the authority to join 228.0.0.1,
	   uni2 just only has the authority to join 230.1.1.1,now uni1 and uni2 join 230.1.1.1 at the same time, 
	   when uni1 leave ,it also let uni2 leave, becase they share the same entry:
	       Mvlan 3001 Group 230.1.1.1
              epon-onu_0/2/1:1:1 
	*/
	for(i = ODM_START_PORT_NUN; i < ODM_NUM_OF_PORTS+1; i++) 
	{
		if(curr_hostPortId != i)
		{	
			/*if other port has the same entry, we just delete the current port host and group, then drop it*/
			tmp_group = mcGroupListGet(i, desc->igmpGroupMac, desc->igmpGroupAdress, desc->cVlan, MC_BITMAP);
			if(tmp_group)
			{
				return OPL_OK;
			}
    ...
    /*End add by huangmingjian 2013-12-12for Bug 295*/
...
//	printf(" %s %d desc->cVlan %d\n",__FUNCTION__,__LINE__,desc->cVlan);
   /*add by zhouguanhua 2013/8/29*/
  /*EPN104N recieve untag data will take  manage vlan,so it must been deleted tag */
  extern int oam_ipc_get_sys_mvlan(sys_mvlan_t *mvlan); 
  int ret_ipc;  sys_mvlan_t info;    
...
/*Begin modified by huangmingjian 2013-10-26:Bug 203,257*/
  MC_TAG_OPER_MODE_t oper_mode = mcTagOperPerPortGet(desc->ingressPortId);
  if (0 == desc->cVlan)
  { 
    for(j=1; j<OP_MAX_VLAN_NUMBER; j++) 
	{
		if(MC_TAG_OPER_MODE_TRANSLATION == oper_mode)
		{
      		if(OPL_TRUE==mcTagTranslationTableUservidExist(desc->ingressPortId, j)) 
		  	{
	       		num++;		
			}
       	}
  ...
  else
  {
  	if(MC_TAG_OPER_MODE_TRANSLATION == oper_mode)
  	{
		if(OPL_TRUE!=mcTagTranslationTableUservidExist(desc->ingressPortId, desc->cVlan)) {
		  return OPL_ERROR;
		}
  	}
	else
  ...
  uint32 isLastQueryPacket)
{
  ...
  /************************** send to cpu **************************/
   /*add by zhouguanhua 2013/8/29*/
  /*EPN104N recieve untag data will take  manage vlan,so it must been deleted tag */
  if(egressPortId==PORT_PON_LOGIC)//join or leave action  
  {        
    extern int oam_ipc_get_sys_mvlan(sys_mvlan_t *mvlan);        
    int ret_ipc;        
    sys_mvlan_t info;                
    ...
    if(info.m_vlan == sglEthHeader->vlan_tag.vid)        
    {            
        /* remove tag */            
        vosMemCpy(tmpPacketPointer, packet, 12);            
        vosMemCpy(tmpPacketPointer+12, packet+16, packetLen-16);            
  ...
  if(group->querycnt==mcControlBlockLastQueryCountGet()) {
    if(group->isLastReportRecevied==OPL_TRUE) { /* report received */
      ...
      if(OPL_TRUE==mcGroupListHostListEmpty(group->hostPortId, group->groupMac, group->groupAddress,
		...
		/*Begin modified by huangmingjian 2013-12-06 for Bug 297*/
		/*for untag and tag, port id and mcvlan are exist,but for control translation they are not exist as
		  they get from translation table.
		*/
		if(OPL_TRUE==mcVlanMapExist(group->hostPortId, group->mcVlan))
		{
...
OPL_STATUS mcCtcLeaveAction(MC_PACKET_DESC_t *desc)
{
 ...
 /* yxzhao added, igmp leave packets transmit to OLT transparently, 2009-12-31, 10:35:37 */
  /************************** group delete **************************/
  /* fast leave mode */
  if(mcControlBlockLeaveModeGet()==MC_LEAVE_MODE_FAST) {
    /* check if the host list is empty */
...
OPL_STATUS mcModeSwitch()
{
  ...
  MC_MODE_SWITCH_OCCUR = 1;/*Add by huangmingjian 2013-12-06 for Bug 294*/
...
static void * mcIgmpMsgThread(void *arg)
{
    ...
    while(mcIgmpMsgId >= 0)
    {
		...
		mcGroupShow();
		mcVlanMapShow();
		mcTagTranslationShow();
		mcTagOperPerPortShow();
	#endif
    }
    ...
    while(mcTimerMsgId >= 0)
    {
        ...
        switch (msg.msgType)
        {
            ...
            case MC_SNOOP_NON_FAST_LEAVE:
                mcSnoopingNonFastLeaveAction((MC_PACKET_DESC_t *)msg.desc);
                break;
            case MC_CTC_LEAVE_ACTIVE:
                mcCtcNonFastLeaveAction((MC_PACKET_DESC_t *)msg.desc);
                break;
            default:
                mc_printf("%s[%d]: Invalid Mc Timer msgType.\n", __FUNCTION__, __LINE__);
                break;
        }
    }
...
OPL_STATUS odmMulticastInit()
{
  ...
  VOS_THREAD_t mcMsgThreadId = (VOS_THREAD_t)NULL;

  vosMsqInit("/var/msq00000000");
  vosConfigInit();
  mcControlBlockIsInitializedSet((uint32)OPL_FALSE);
  ...
  if (mcIgmpMsgId == -1)
  {
      mc_printf("%s[%d], vosMsqCreate fail.\n", __FUNCTION__, __LINE__);
      return OPL_FALSE;
  }

  mcMsgThreadId = vosThreadCreate("tMcIgmp", OP_VOS_THREAD_STKSZ, 60,
                  (void *)mcIgmpMsgThread, (void *)NULL);
  if (mcMsgThreadId == (VOS_THREAD_t) NULL)
  {
      mc_printf("%s[%d], vosThreadCreate fail.\n", __FUNCTION__, __LINE__);
      return OPL_FALSE;
  }
  ...
  mcMsgThreadId = (VOS_THREAD_t) NULL;
  ...
  mcControlBlockIsInitializedSet((uint32)OPL_TRUE);
...
BOOL CTC_CheckValidVidForMcTranslation(UINT32 vlanId)
{	
	int i = 0;
	...
	for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++) 
	{
		if(OPL_ENABLE == mcTagTranslationTable[i].enable)
		{
			if(vlanId == mcTagTranslationTable[i].mcVid || vlanId == mcTagTranslationTable[i].userVid)
			{
				return FALSE;
			}
		}
...
/*Begin added by sunmingliang for bug 260*/
UINT32 CTC_CheckValidVidForLgcPort(UINT32 lport, UINT32 uiVid, UINT8* pVlanMode,UINT8* pVlanExist)
{
    UINT32 uiLPortIndex = lport;
    CTC_VLAN_CFG_S stVlanMode;

    if (!VALID_VLAN_ID(uiVid))
    {
        return 1;
    }
	
    (VOID)_CTC_GetPortVlanModeCfg(uiLPortIndex, &stVlanMode);
	*pVlanMode = stVlanMode.mode;
    if (CTC_VLAN_MODE_TRANSLATION == stVlanMode.mode)
    {
        UINT32 uiVlanIndex = 0;

        if (uiVid == stVlanMode.default_vlan)
        {
        	*pVlanExist = TRUE;
            return 0;
        }
        ...
        {
            if ((uiVid == stVlanMode.vlan_list[uiVlanIndex<<1]) ||
                (uiVid == stVlanMode.vlan_list[(uiVlanIndex<<1) + 1]))
            {
				*pVlanExist = TRUE;
                return 0;
            }
...
/*End added by sunmingliang for bug 260*/
...
* @see
*/
int odmTranslationVlanGet(uint32 portId, uint16 *vlanId, uint8 *num)
{

  uint32 j;
  uint8 vlanIdNum = 0;
  oam_mcast_vlan_translation_entry_t tranlation_entry[MC_VLAN_TRANSLATION_ENTRY];

  MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "odmTranslationVlanGet: portId=%d, vlanId=0x%X, num=0x%X.\r\n",
    portId, vlanId, num);
  ...
  DBG_PRINTF("vlanId:%d, num:%d", *vlanId, *num);
  memset(tranlation_entry, 0, sizeof(tranlation_entry));
  odmMulticastTagTranslationGet(portId, tranlation_entry, &vlanIdNum);
  while(*vlanId)
  	vlanId++;
  for(j=0; j<vlanIdNum; j++) 
  {
	 *vlanId = tranlation_entry[j].iptv_vlan;
	 vlanId++;
  }
  *num += vlanIdNum;
  DBG_PRINTF("vlanId:%d, num:%d", *vlanId, *num);
...
int odmMulticastControlEntryAdd(uint32 portId, uint32 vlanId, uint8 *mac)
{
  ...
  uint32 groupAddress;
  uint32 userVid = 0;
  int transl_exist = OPL_NOT_EXIST;
  uint32 port_oper = 0;
  ...
  if(mcControlBlockModeGet()!=MC_MODE_CTC) {
    DBG_PRINTF("Not in CTC mode.\r\n");
    return OPL_ERROR;
  }
  
  port_oper = mcTagOperPerPortGet(portId);
  vosMemSet(daMacStr, 0, sizeof(daMacStr));
  group = NULL;
  ret = 0;
  groupAddress = 0;

  /*Begin add by huangmingjian 2013-12-10 for Bug 298*/  
  if(MC_TAG_OPER_MODE_TRANSLATION == port_oper)
  {
	  if(OPL_NOT_EXIST==(transl_exist=mcTagTranslationTableGet(portId, vlanId, &userVid)))
	  {
		MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "Translation vlan is not exist.");
	  }
  }

  /*End add by huangmingjian 2013-12-10 for Bug 298*/
  ...
  if(odmMulticastControlTypeGet()==MC_CTL_GDA_MAC_VID
    ...
    /*Begin modified by huangmingjian 2013-12-10 for Bug 298*/
	if(MC_TAG_OPER_MODE_TRANSLATION != port_oper)
	{		
		if(OPL_OK != odmMulticastVlanAdd(portId, vlanId))
		{
			MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "Mc vlan add error.");
			return OPL_ERROR;
		}
    }
   }
  ...
  if(odmMulticastControlTypeGet()==MC_CTL_GDA_MAC
	...
	{  
		ret = odmMcGroupAdd(portId, mac, 0, userVid, GROUP_STATE_CONFIG_BY_OAM, MC_BITMAP);
		if(ret!=OPL_OK) 
		{
			mcGroupSemUnLock(); 
		}
  ...
  if(odmMulticastControlTypeGet()==MC_CTL_GDA_GDA_IP_VID) {
	...
	{
		ret = odmMcGroupAdd(portId, 0, groupAddress, userVid, GROUP_STATE_CONFIG_BY_OAM, MC_BITMAP);
		if(ret!=OPL_OK) {
			mcGroupSemUnLock(); }
		RETURN_VAL_IF_FAIL(ret == OPL_OK, ret);
	}
...
int odmMulticastControlEntryDelete(uint32 portId, uint32 vlanId, uint8 *mac)
{
  ...
  uint32 userVid = 0;
  int transl_exist = OPL_NOT_EXIST;
  uint32 port_oper = mcTagOperPerPortGet(portId);
  ...
  if(odmMulticastControlTypeGet()==MC_CTL_GDA_MAC
	...
	{
		group = mcGroupListGet(portId, mac, 0, userVid, MC_BITMAP);		
	}
	/*End modified by huangmingjian 2013-12-10 for Bug 298*/
  }
  if(odmMulticastControlTypeGet()==MC_CTL_GDA_GDA_IP_VID) {
    /************************ MC_CTL_GDA_GDA_IP_VID ************************/
    groupAddress = *(uint32 *)&mac[2];
	...
	{
		group = mcGroupListGet(portId, 0, groupAddress, userVid, MC_BITMAP);
	}
	/*End modified by huangmingjian 2013-12-10 for Bug 298*/	
  }
...
OPL_STATUS mcTagTranslationTableCheck(uint32 portId, uint32 mcVid, uint32 userVid)
{
  ...
  if(mcVid==userVid)
  {
  	mc_printf("mcVid should not equal to userVid \r\n");
  	return OPL_ERROR;
  }
  ...
  for(i=0; i<MC_VLAN_TRANSLATION_ENTRY; i++) {
    if(mcTagTranslationTable[i].enable==OPL_ENABLE) {
      ...
      {
          mc_printf("userVid which equal to existed mcVid or userVid in the translation table is not allowed \r\n");
	   return OPL_ERROR;
      }
      if((mcTagTranslationTable[i].portId == portId)
...
struct sock_filter muticast_bpf_filter [] = {
	/* IGMP */
...
static VOID setup_socket_filter(INT s)
{
        ...
        p.len = sizeof(muticast_bpf_filter)/sizeof (struct sock_filter);
        p.filter = muticast_bpf_filter;
...
OPL_BOOL g_bHostDsDbgEn = OPL_TRUE;//OPL_FALSE;
...
void eopl_pkt_process(void * pkt, UINT16 len)
{
    ...
    if (OPL_TRUE == g_bHostDsDbgEn)
    {
        ...
        for(index = 0; index < len; index++)
        {
            ...
            {
                mc_printf("%02x ",(UINT8)pucBuf[index]);
            }
        }
        mc_printf("\r\n");
    }
...
INT16 eopl_host_socket_recv(int socket_fd)
{
    ...
    while((usLen = recvmsg(socket_fd, &msg, 0)) > 0)    
    {
        ...
        mc_printf("\r       Packet received from Host interface:\n"
                       "\r       Iport : %d\n\t"
                       ,ptr_pkt_hdr->iport);
...
void *epon_muticast_rxThread(void *argu)
{
    int raw_socket;
    ...
    if (0 > raw_socket){
        mc_printf("raw_socket socket create error!\n");
        return;
    }
    ...
    mc_printf("socket init ok raw_socket=%d\n", raw_socket);
...
INT16 eopl_l2_send(unsigned char* packet, UINT16 length)
{
    ...
    mc_printf("eopl_l2_send portmask=%x\n", l2_info.portmask.pbits[0]);
...
INT16 eopl_host_send(void *pkt, UINT16 len)
{
    ...
    /* If need to log packet */
    mc_printf("\r       Packet sent from Host interface:\n"
	                "\r       eport : %d\n\t"
	                "\r       icos : %d\n\t"
	                ,ptr_pkt_hdr->eport
	                ,ptr_pkt_hdr->icos
