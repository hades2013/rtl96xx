...
#if defined(CONFIG_PRODUCT_EPN200) || defined(CONFIG_PRODUCT_EPN101R) || defined(CONFIG_PRODUCT_EPN101ZG)
...
static pthread_mutex_t m_DrvCfgMutex;
//#if !defined(CONFIG_PRODUCT_EPN200)
#define DRV_CFGLOCK   pthread_mutex_lock(&m_DrvCfgMutex)
#define DRV_CFGUNLOCK pthread_mutex_unlock(&m_DrvCfgMutex)

extern BOOL CTC_CheckValidVidForMcTranslation(UINT32 vlanId);/*Add by huangmingjian 2013-10-17 for Bug 203*/

int MutexCreate(pthread_mutex_t *mutex)
{
...
INT32 DRV_LwInit(void)
{
    ...
    retVal = MutexCreate(&m_DrvCfgMutex);
    if (0 != retVal)
    {
        return ERROR;
    }
...
static BOOL _CTC_CheckVidForTagMode(UINT32 uiLPortId, UINT32 uiVlanId)
{
    ...
    {
        extern int mcVlanExist(UINT32 vlanId);
        if (1 == mcVlanExist(uiVlanId))
        {
            return FALSE;
        }
    }
    ...
    /*ctc vlan can not be same with mc translation vlan.*/
	if (FALSE == CTC_CheckValidVidForMcTranslation(uiVlanId))
	{
		return FALSE;
	}
    /*End add by huangmingjian 2013-10-17 for Bug 203*/
...
static BOOL _CTC_CheckVlanForTranslationMode(UINT32 uiLPortId, UINT32 uiDefVlan, UINT32 uiVlanEntryNum, UINT32 *pVlanList)
{
    ...
    //#ifdef CTC_MULTICAST_SURPORT
    {
        ...
        for (uiVlanIndex = 0; uiVlanIndex < uiVlanEntryNum; uiVlanIndex++)
        {
            ...
            if ((1 == mcVlanExist(pVlanList[uiOldVlanIndex])) ||
                (1 == mcVlanExist(pVlanList[uiNewVlanIndex])))
            {
                return FALSE;
            }
        }
    ...
    /*ctc vlan can not be same with mc translation vlan.*/
	if(FALSE == CTC_CheckValidVidForMcTranslation(uiDefVlan))
	{
		return FALSE;
	}
	...
	for (uiVlanIndex = 0; uiVlanIndex < uiVlanEntryNum; uiVlanIndex++)
	{
		uiOldVlanIndex = uiVlanIndex << 1;
		uiNewVlanIndex = (uiVlanIndex << 1) + 1;
		if (FALSE == CTC_CheckValidVidForMcTranslation(pVlanList[uiOldVlanIndex]) ||
			FALSE == CTC_CheckValidVidForMcTranslation(pVlanList[uiNewVlanIndex]))
		{
			return FALSE;
		}
	}
...
INT32 CTC_DRV_SetTransparentVlan(UINT32 uiLPortId)
{
    ...
    if (CTC_VLAN_MODE_TAG == stVlanMode.mode)
    {
        iRet = Ioctl_ctc_del_acl_rule_for_tag(uiLPortId);/*reset tag*/
        if (DRV_OK != iRet)
        {
            diag_printf("func:%s,line:%d.\n",__FUNCTION__,__LINE__);
            return ERROR;
    ...
    else if (CTC_VLAN_MODE_TRANSLATION == stVlanMode.mode)
    {
        ...
        iRet = Ioctl_ctc_del_acl_rule_for_transl(uiLPortId);/*reset translation*/
        if (DRV_OK != iRet)
        {
            diag_printf("func:%s,line:%d.\n",__FUNCTION__,__LINE__);
            return ERROR;
    ...
    iRet = Ioctl_ctc_add_acl_rule_for_transp(uiLPortId, stVlanMode);/*set transparent*/
    if (DRV_OK != iRet)
    {
        diag_printf("func:%s,line:%d.\n",__FUNCTION__,__LINE__);
        return ERROR;
...
INT32 CTC_DRV_ResetTransparentVlan(UINT32 uiLPortId)
{
    ...
    iRet = Ioctl_ctc_del_acl_rule_for_transp(uiLPortId);/*reset transparent*/
    if (DRV_OK != iRet)
    {
        diag_printf("func:%s,line:%d.\n",__FUNCTION__,__LINE__);
        return ERROR;
...
*****************************************************************************/
/*used to identify vlan mode for YOTC olt*/
CTC_VLAN_MODE_E ctc_vlan_mode = CTC_VLAN_MODE_TRANSPARENT;

INT32 CTC_DRV_SetTranslationVlan(UINT32 uiLPortId, UINT32 uiVlanTag,
                                 UINT32 *puiVlanList, UINT32 uiVlanListEntry, CTC_VLAN_MODE_E mode)
{
    INT32 iRet = NO_ERROR;
    UINT32 i = 0;
    UINT32 uiPvid;
    ...
    iRet = CTC_DRV_SetTransparentVlan(uiLPortId);/*should be reset tag or translation,then set tranparent*/
    if (NO_ERROR != iRet)
    {
        diag_printf("func:%s,line:%d.\n",__FUNCTION__,__LINE__);
        return ERROR;
    ...
    iRet = CTC_DRV_ResetTransparentVlan(uiLPortId);/*should be reset transparent*/
    if (NO_ERROR != iRet)
    {
        diag_printf("func:%s,line:%d.\n",__FUNCTION__,__LINE__);
        return ERROR;
...
INT32 DRV_SetPortSpeed(UINT32 lport, PORT_SPEED_E enPortSpeed)
{
    ...
    if(DRV_OK == Ioctl_SetPortSpdlx(lport, spdlx_val)){
        (VOID)_CTC_SetUniSpeedCfg(lport, enPortSpeed);
        return NO_ERROR;
    }
    else{
        return ERROR;
...
*****************************************************************************/
INT32 DRV_GetPortFlowCtrl(UINT32 lport,  BOOL *bEnable)
{
	if(NULL == bEnable) {
		return ERROR;
	}
	...
	if(DRV_OK != Ioctl_GetPortFlowctrl(lport, bEnable))
	{
		return ERROR;
	}
...
*****************************************************************************/
INT32 DRV_GetPortLinkState(UINT32 uiLPortId, BOOL *bPortStatus)
{
	if(NULL == bPortStatus) {
		return ERROR;
	}
	
	if(FALSE == IsValidLgcPort(uiLPortId)) {
		return ERROR;
	}

    if(DRV_OK != Ioctl_GetPortEnable(uiLPortId, bPortStatus))
	{
        return ERROR;
    }
...
*****************************************************************************/
INT32 DRV_SetPortAutoNegAdmin(UINT32 lport,  BOOL bEnable)
{
    BOOL bCfgEnable = FALSE;
	...
	(VOID)_CTC_GetUniAutoNegModeCfg(lport, &bCfgEnable);
	if ( bEnable == bCfgEnable)
	{
	    return NO_ERROR;
	}

	if(DRV_OK == Ioctl_SetPortAutoneg(lport, bEnable, bEnable))
	{
		(VOID)_CTC_SetUniAutoNegModeCfg(lport, bEnable);
        return NO_ERROR;
    }
    else{
        return ERROR;
...
*****************************************************************************/
INT32 DRV_GetPortAutoNegAdmin(UINT32 lport,  BOOL *bEnable)
{
	if(NULL == bEnable) {
		return ERROR;
	}
	...
	if(DRV_OK != Ioctl_GetPortAutoneg(lport, bEnable))
	{
		return ERROR;
	}
...
*****************************************************************************/
INT32 DRV_SetFecMode(BOOL bEnable)
{
	if((FALSE != bEnable) && (TRUE != bEnable)) {
        return ERROR;
    }

	if(DRV_OK != Ioctl_SetFecMode(bEnable))
	{        
        return ERROR;
    }
...
*****************************************************************************/
INT32 DRV_GetFecMode(BOOL *bEnable)
{
	if(NULL == bEnable) {
		return ERROR;
	}
			
	if(DRV_OK != Ioctl_GetFecMode(bEnable))
	{
		return ERROR;
	}
...
void Ctc_AddClfRmk2Local(UINT32 lport,
	                         UINT32  precedenceOfRule,
	                         oam_clasmark_rulebody_t *pstClsRule,
	                         oam_clasmark_fieldbody_t *pstClsField)
{
	DRV_CFGLOCK;
	local_clasmark[lport].ClsRule[precedenceOfRule].precedenceOfRule = pstClsRule->precedenceOfRule;
	local_clasmark[lport].ClsRule[precedenceOfRule].lenOfRule = pstClsRule->lenOfRule;
	local_clasmark[lport].ClsRule[precedenceOfRule].queueMapped = pstClsRule->queueMapped;
	local_clasmark[lport].ClsRule[precedenceOfRule].ethPriMark = pstClsRule->ethPriMark;
	local_clasmark[lport].ClsRule[precedenceOfRule].numOfField = pstClsRule->numOfField;
	local_clasmark[lport].ClsField[precedenceOfRule].fieldSelect = pstClsField->fieldSelect;
	memcpy(local_clasmark[lport].ClsField[precedenceOfRule].matchValue, pstClsField->matchValue, 6);
	local_clasmark[lport].ClsField[precedenceOfRule].operator = pstClsField->operator;
	DRV_CFGUNLOCK;
}

void Ctc_ClearClfRmk2LocalByPort(UINT32 lport)
{
	DRV_CFGLOCK;	
...
void Ctc_ClearClfRmk2LocalByPrec(UINT32 lport, UINT32  precedenceOfRule)
{
	DRV_CFGLOCK;
	memset(&local_clasmark[lport].ClsRule[precedenceOfRule], 0, sizeof(oam_clasmark_rulebody_t));
	memset(&local_clasmark[lport].ClsField[precedenceOfRule], 0, sizeof(oam_clasmark_fieldbody_t));
	DRV_CFGUNLOCK;
}

void Ctc_GetClfRmk2Local(UINT32 lport,
	                         UINT32  precedenceOfRule,
	                         oam_clasmark_rulebody_t *pstClsRule,
	                         oam_clasmark_fieldbody_t *pstClsField)
{
	pstClsRule->precedenceOfRule = local_clasmark[lport].ClsRule[precedenceOfRule].precedenceOfRule;
	pstClsRule->lenOfRule = local_clasmark[lport].ClsRule[precedenceOfRule].lenOfRule;
	pstClsRule->queueMapped = local_clasmark[lport].ClsRule[precedenceOfRule].queueMapped;
	pstClsRule->ethPriMark = local_clasmark[lport].ClsRule[precedenceOfRule].ethPriMark;
	pstClsRule->numOfField = local_clasmark[lport].ClsRule[precedenceOfRule].numOfField;
	pstClsField->fieldSelect = local_clasmark[lport].ClsField[precedenceOfRule].fieldSelect;
	memcpy(pstClsField->matchValue, local_clasmark[lport].ClsField[precedenceOfRule].matchValue, 6);
	pstClsField->operator = local_clasmark[lport].ClsField[precedenceOfRule].operator;
}
	                         ...
	                         oam_clasmark_fieldbody_t *pstClsField)
{
    ...
    for(uiFieldId = 0; uiFieldId < uiFieldNum; uiFieldId++)
    {
        switch (pstClsField->fieldSelect)
        {
			...
			/*Begin modified by sunmingliang for bug 303*/			
            case OAM_CTC_CLS_FIELD_TYPE_IP_PRECEDENCE :
                uiAclRuleType = ACL_TRUST_IPV4_TOS;
                enRuleValue.ulValue = pstClsField->matchValue[5];
                break;
			/*End modified by sunmingliang for bug 303*/	
...
#if 1
				if(DRV_OK != Ioctl_AddCtcClfPriToQueueByPort(lport, precedenceOfRule, 
                                           uiQueueMapped))
		        {
		            return ERROR;
		        }
...
#if 1
	if(DRV_OK == Ioctl_DelCtcClfPriToQueueByPrec(lport, precedenceOfRule))
	{
        return ERROR;
    }
#endif
...
#if 1
	if(DRV_OK != Ioctl_ClearCtcClfPriToQueueByPort(lport))
	{
        return ERROR;
    }
#endif
...
INT32 DRV_AddPort2McastVlan(UINT32 uiLPort, UINT32 uiMcVid, MC_TAG_MODE_E enTagMode)
{
    ...
    {
        diag_printf("func:%s,line:%d,ret:%d,en:%d.\n",
