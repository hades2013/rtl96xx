 ...
 * Data Declaration
 */
extern unsigned char currCtcVer_webDis;
...
static unsigned char ctc_orgspec_action_flag;

oam_instance_tlv_t_2_0 port_instance_2_0;

unsigned char ctcExtInfoCheck = 0;
unsigned char ctcExtInfoCnt = 0;
    ...
    unsigned short *pReplyLen)  /* Reply size used by this handler*/
{
    ...
    *pPtr = pInstant->varWidth;
	...
	/*begin add code by shipeng 2013-05-21*/
	if(pInstant->varWidth == CTC_ORGSPEC_VARINSTANT_WIDTH20)
	{
		...
		*/
		if((pInstant->varData[0] == 0xFF) && (ctc_orgspec_action_flag == 0))
			pPtr[0] = 0x01;
		else	
	...
	else if(pInstant->varWidth == CTC_ORGSPEC_VARINSTANT_WIDTH21)
	{
		...
		*/
		if(((pInstant->parse.uniPort.portNo == 0xFF) || (pInstant->parse.uniPort.portNo == 0xFFFF))
			&& (ctc_orgspec_action_flag == 0))
		{
		...
		/*begin modify for bug id 149 by shipeng 2013-08-01*/
		/*PMC OLT bug, must use 0xffff for all port mask*/
		else if(((pInstant->parse.uniPort.portNo == 0xFF) || (pInstant->parse.uniPort.portNo == 0xFFFF))
			&& (ctc_orgspec_action_flag == 1))
		{
			pPtr[0] = pInstant->parse.uniPort.portType;/*the value must be reserved */
			pPtr[1] = pInstant->parse.uniPort.chassisNo;
			pPtr[2] = 0xFF;
		...
		/*end modify for bug id 149 by shipeng 2013-08-01*/
		else	
	    	memcpy(pPtr, pInstant->varData, sizeof(unsigned char) * pInstant->varWidth);
	}
	/*end add code by shipeng 2013-05-21*/
    ...
    *pPtr = pContainer->varDesc.varBranch;
    ...
    {	
    	...
    	*pPtr = pContainer->varWidth;
		...
		if(*pVarContainer_Total_Len > pContainer->varWidth)
		{
			memcpy(pPtr, pContainer->pVarData, *pVarContainer_Total_Len);
			pPtr += *pVarContainer_Total_Len; /* Data */
		}
		else
		{
...
//    dataLen = strlen(pAuthLoid->loid);
	dataLen = strlen(epon_info.epon_loid);
...
//   memcpy(pPtr + (CTC_ORGSPEC_ONUAUTH_LOID_LEN - dataLen), pAuthLoid->loid, dataLen);
	memcpy(pPtr + (CTC_ORGSPEC_ONUAUTH_LOID_LEN - dataLen), epon_info.epon_loid, dataLen);
    pPtr += CTC_ORGSPEC_ONUAUTH_LOID_LEN; /* LOID */
    /* Password */
//    dataLen = strlen(pAuthLoid->password);
	dataLen = strlen(epon_info.epon_passwd);
...
//    memcpy(pPtr + (CTC_ORGSPEC_ONUAUTH_PASSWORD_LEN - dataLen), pAuthLoid->password, dataLen);
	...
	memcpy(pPtr + (CTC_ORGSPEC_ONUAUTH_PASSWORD_LEN - dataLen), epon_info.epon_passwd, dataLen);
    ...
    unsigned short *pReplyLen)  /* Reply size used by this handler*/
{
    ...
    unsigned short remainLen;
	unsigned short varContainer_Total_Len = 0;
    ...
    *pReplyLen = 0;
    ...
    while(remainLen >= CTC_ORGSPEC_VARDESC_LEN)
    {
		...
		EPON_OAM_PRINT(EPON_OAM_DBGFLAG_INFO, "[OAM:%s:%d] Branch = 0x%02x Leaf=0x%04x\n", __FILE__, __LINE__, varDesc.varBranch, varDesc.varLeaf);
        ...
        case CTC_VAR_REQBRANCH_INSTANT20:
			...
			port_instance_2_0.branch = varInstant.varDesc.varBranch;
			port_instance_2_0.leaf = varInstant.varDesc.varLeaf;
			port_instance_2_0.width = varInstant.varWidth;
			port_instance_2_0.value = varInstant.varData[0];
            break;
        case CTC_VAR_REQBRANCH_STDATTR:
        case CTC_VAR_REQBRANCH_EXTATTR:
        case CTC_VAR_REQBRANCH_STDACT:
        case CTC_VAR_REQBRANCH_EXTACT:
               ...
               (0 == (varCb.op & CTC_VAR_OP_GET)))
            {
				...
				/*zte brantch*/
				u8_t** pp_byte;
				pProcPtr -= CTC_ORGSPEC_VARDESC_LEN;
				pp_byte = &pProcPtr;
				eopl_zte_ctc_get_req(varDesc.varLeaf, pp_byte, pReplyBuf, &genLen);
				pReplyBuf += genLen;
				*pReplyLen += genLen;
            ...
            ret = varCb.handler_get(llidIdx, CTC_VAR_OP_GET, pVarInstant, varDesc, &varContainer_Total_Len, &varContainer);
            if(EPON_OAM_ERR_OK == ret)
            {
    ...
    unsigned short *pReplyLen)  /* Reply size used by this handler*/
{
    ...
    *pReplyLen = 0;
    ...
    while(remainLen >= CTC_ORGSPEC_VARCONTAINER_MIN)
    {
        ...
        //varContainer.varWidth = (0 == varContainer.varWidth) ? 0x80 : varContainer.varWidth;/*when olt reboot onu, the varContainer have no the member or the varWidth = 0*/
        CTC_BUF_ADD(pProcPtr, remainLen, CTC_ORGSPEC_VARCONTAINER_MIN); /* Variable Container */
		EPON_OAM_PRINT(EPON_OAM_DBGFLAG_INFO, "[OAM:%s:%d] Branch = 0x%02x Leaf=0x%04x, Width=%d\n", __FILE__, __LINE__, varContainer.varDesc.varBranch, varContainer.varDesc.varLeaf, varContainer.varWidth);
        ...
        case CTC_VAR_REQBRANCH_EXTACT:
               ...
               (0 == (varCb.op & CTC_VAR_OP_SET)))
            {
				...
				u8_t** pp_byte;
				pProcPtr -= CTC_ORGSPEC_VARCONTAINER_MIN;
				pp_byte = &pProcPtr;
				eopl_zte_ctc_set_req(pVarInstant, varContainer.varDesc, pp_byte, pReplyPtr, &genLen);
				*pReplyLen += genLen + (pReplyPtr - pReplyBuf);
				return EPON_OAM_ERR_OK;
#endif
    ...
    case CTC_OAM_FSM_STATE_WAIT_REMOTE:
        ...
        else
        {
            ...
            *pReplyLen = expectLen;
            ...
            /* Start CTC check counter to wait for the OLT reply */
            /*for ctc ext info reply check*/
           // ctcExtInfoCheck = 1;
            ctcExtInfoCnt=0;
    ...
    unsigned short *pReplyLen)  /* Reply size used by this handler*/
{
    ...
    *pReplyLen = 0;
	...
	currCtcVer_webDis = currVer;/*Add by huangmingjian 2013-08-22*/
    ...
    case CTC_EXTOAM_OPCODE_SWDOWNLOAD:
		#if CTC_OAM_UPGRADE_ENABLE
        ctc_oam_orgSpecHdr_gen(oui, CTC_EXTOAM_OPCODE_SWDOWNLOAD, pReplyBuf, &genLen);
        CTC_BUF_ADD(pReplyBuf, bufLen, genLen);
        *pReplyLen += genLen;
		
        genLen= 0;
		eopl_ctc_soft_download(
        pOamPdu->llidIdx, pFrame, length, pReplyBuf, bufLen, &genLen);
        *pReplyLen = 0;
		#endif
        break;
    case CTC_EXTOAM_OPCODE_CHURNING:
		/*Disable churning key as onu which registed on ZTE OLT  is online/offline */
