...
unsigned char currCtcVer_webDis = 0;

extern unsigned char ctcExtInfoCheck;
extern unsigned char ctcExtInfoCnt;
    ...
    unsigned short *pReplyLen)
{
	...
	/*begin add by shipeng for I think we must clear the buf 2013/05/29*/
	memset(pReplyPtr, 0, (sizeof(char) * bufLen));
	/*end add by shipeng for I think we must clear the buf 2013/05/29*/
...
ponStatus_webDis_t *g_ponStatus = NULL;
extern unsigned char oam_discovery_state;
void ponStatusParasGet( unsigned char llidIdx)
{
	unsigned char ctcVer = 0;
	unsigned char *pParsePtr= NULL;
	unsigned char mac[6] = {0};
	oam_oamInfo_t remoteInfo;
	ponStatus_webDis_t *ponStatus = g_ponStatus;
	
	ponStatus->isOnline = (oam_discovery_state==EPON_OAM_STATE_COMPLETE?1:0);
	if(1 == ponStatus->isOnline)
	{	
		if(currCtcVer_webDis <= 0x20)
		{
			sprintf(ponStatus->ctcVer, "CTC 2.0");	
		}
		else
		{
			sprintf(ponStatus->ctcVer, "CTC 2.1");	
		}

		memset(&remoteInfo, 0x0, sizeof(oam_oamInfo_t));
		epon_oam_remoteInfo_get(llidIdx, &remoteInfo);
		ponStatus->remoteInfovalid = remoteInfo.valid;
        if(remoteInfo.valid)
        {	
        	sprintf(ponStatus->oamVer, "%d", remoteInfo.oamVer);
			sprintf(ponStatus->oui, "%02x:%02x:%02x",
										remoteInfo.oui[0],
										remoteInfo.oui[1],
										remoteInfo.oui[2]);
			sprintf(ponStatus->venderSpecInfo, "%02x:%02x:%02x:%02x",
										remoteInfo.venderSpecInfo[0],
										remoteInfo.venderSpecInfo[1],
										remoteInfo.venderSpecInfo[2],
										remoteInfo.venderSpecInfo[3]);
        }
	}
}
#endif	
    ...
    unsigned short length)
{
    ...
    case EPON_OAMPDU_CODE_INFO:
        ...
        if(ctcExtInfoCheck == 1)
            {
                ctcExtInfoCnt++;
                printf("\n oam cnt before CTC oam init ok (%d)\n",ctcExtInfoCnt);
                if(ctcExtInfoCnt > 5)
                {
                    ctcExtInfoCnt=0;
                    printf("\n CTC timeout!! dislable LLID\n");
                    epon_oam_event_send(0, EPON_OAM_EVENT_KEEPALIVE_TIMEOUT);
                }
        }
		#endif
        break;
...
void *epon_oam_rxThread(void *argu)
{
    ...
    printf("Init LW drv ...(%s,%s,%d) ",__FILE__,__FUNCTION__,__LINE__);
    if(0 != DRV_LwInit())
    {
        printf("failed\r\n");
        return NULL;
	...
	/*Begin add by huangmingjian 2013-08-22*/
#ifdef CONFIG_PRODUCT_5500		
	g_ponStatus = ponStatus_webDis_shmInit();
	memset(g_ponStatus, 0, sizeof(ponStatus_webDis_t));
	printf("oam share init...\r\n");
#endif	
	/*End add by huangmingjian 2013-08-22*/
