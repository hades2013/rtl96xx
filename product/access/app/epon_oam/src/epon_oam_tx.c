    ...
    unsigned short *pPushedLen)
{
    ...
    if(bufLen >= EPON_OAMPDU_HDR_LENGTH) 
    {
        ...
        *pPtr = EPON_OAMPDU_CODE_INFO;
		pPtr++; /*Add by huangmingjian 2013-12-20 Bug 284*/
    ...
    oam_eventOrgSpec_t *pEvent)
{
     ...
     */
    totalLen = EPON_OAMPDU_HDR_LENGTH + EPON_EVENT_OAMPDU_SEQUENCE_LEN;
    pCurrEvent = pEvent;
    ...
    *pCurr = ((unsigned char *)(&eventSeq[llidIdx]))[1];
    ...
    DUMP_HEX_VALUE(EPON_OAM_DBGFLAG_TXOAM, pReplyPtr, pCurr - pReplyPtr);
    EPON_OAM_PRINT(EPON_OAM_DBGFLAG_TXOAM, "\n");
    ...
    unsigned short replyLen)
{
    int ret;
    unsigned char *pReplyPtr, *pCurr;
	unsigned short tmp_len = 0;/*add by shipeng*/
	...
	/*begin modify by shipeng for I think we most hold the total replyLen > 60 2013/05/29*/
	if(replyLen + EPON_OAMPDU_HDR_LENGTH < 60)
	{
		tmp_len = 60 - (replyLen + EPON_OAMPDU_HDR_LENGTH);
	}

    pReplyPtr = 
        (unsigned char *) malloc(sizeof(unsigned char) * replyLen + EPON_OAMPDU_HDR_LENGTH + tmp_len);
    ...
    unsigned short len)
{
	...
	len = (len>=60?len:60);
    pPtr = (unsigned char *) malloc(sizeof(unsigned char) * len);
    ...
    DUMP_HEX_VALUE(EPON_OAM_DBGFLAG_TXOAM, pPtr, pCurr - pPtr);
    EPON_OAM_PRINT(EPON_OAM_DBGFLAG_TXOAM, "\n");
