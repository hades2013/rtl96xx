...
unsigned char fiberhome_onu_vendor[4] = {'W','R','I', 0};
unsigned char zte_onu_vendor[4] = {'Z','T','E', 0};

unsigned char zte_onu_id[3] = {0x0,0xd,0xb6};
unsigned char    zte_oui[3] = {0x00, 0x0F, 0x3E};
unsigned char 	h3c_oui[3] = {0x00, 0x0f, 0xe2};
unsigned char	huawei_oui[3] = {0xbc, 0x61, 0x4e}; 
unsigned char 	bdcom_oui[3] = {0x00, 0xe0, 0x0f};
unsigned char 	yotc_oui[3] = {0x00, 0x0c, 0xd8};
unsigned char	raisecom_oui[3] = {0x00,0x13,0x25};
/*some olt send oam share this OUI*/
unsigned char 	share_oui[3] = {0x00, 0x0c, 0xd5};
/*add by linguobin 2013-12-2*/
#if defined(CONFIG_ONU_COMPATIBLE)
unsigned char   zte_olt_vender[4] = {0x37, 0x23, 0x00, 0x01};
unsigned char   zte_c300_olt_vender[4] = {0x00, 0x00, 0x00, 0x00};
unsigned char   fiberhome_olt_vender[4] = {0x11, 0x00, 0x00, 0x01};
extern int oam_fd;
#endif
...
static void epon_oam_test_init()
{
    oam_config_t config;
    ...
    if(0 == regEntry.llidIdx)
    {
        epon_oam_config_get(0, &config);
        config.oamEnabled = 1;
        memcpy(&config.macAddr[0], &regEntry.mac.octet[0], sizeof(config.macAddr));

        epon_oam_config_set(0, &config);
    }
    else
    {
        printf("%s:%d Testing config failed\n", __FILE__, __LINE__);
    }
}
/*
 * User APIs
...
int epon_oam_database_init(void)
{
    ...
    infoOam.oui[1] = 0x11;
    infoOam.oui[2] = 0x11;
	/*modify by shipeng 2013-05-8*/
	memcpy(infoOam.venderSpecInfo, oam_onu_vendor, sizeof(oam_onu_vendor));
	/*modify by shipeng 2013-05-8*/
