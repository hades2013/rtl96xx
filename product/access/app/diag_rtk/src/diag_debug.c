    ...
    cparser_context_t *context)
{
    ...
    if (drv_swcore_cid_get(&chip_id, &chip_rev_id) == RT_ERR_OK)
    {
        if( (chip_id == APOLLO_CHIP_ID) && (chip_rev_id == CHIP_REV_ID_0) )
        {
            diag_util_mprintf("Chip probe result : Apollo (ID = 0x%04X, Rev = 0x%04X)\n", chip_id, chip_rev_id);
        }
        else if( (chip_id == APOLLO_CHIP_ID) && (chip_rev_id == CHIP_REV_ID_A) )
        {
            diag_util_mprintf("Chip probe result : Apollo Rev.B (ID = 0x%04X, Rev = 0x%04X)\n", chip_id, chip_rev_id);
        }
        else if( (chip_id == APOLLOMP_CHIP_ID))
        {
            diag_util_mprintf("Chip probe result : Apollo-MP (ID = 0x%04X, Rev = 0x%04X)\n", chip_id, chip_rev_id);
        }
        else if( (chip_id == RTL9601B_CHIP_ID))
        {
            diag_util_mprintf("Chip probe result : RTL9601B (ID = 0x%04X, Rev = 0x%04X)\n", chip_id, chip_rev_id);
        }
        else
        {
