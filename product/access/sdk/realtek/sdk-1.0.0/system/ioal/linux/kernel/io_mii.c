...
int io_mii_memory_write(uint32 memaddr,uint32 data)
{
    ...
    uint32 baseAddr;
    uint32 spiIO=0;
    baseAddr = memaddr&0x00FFFFFF;

#if 0      
    if(baseAddr<0x900400 && baseAddr>0x900000)
        spiIO = 1;
    else if(baseAddr<0x910400 && baseAddr>0x910000)
        spiIO = 1;
    else if(baseAddr<0x920400 && baseAddr>0x920000)
        spiIO = 1;    
    else
        spiIO = 0;    
#endif    
