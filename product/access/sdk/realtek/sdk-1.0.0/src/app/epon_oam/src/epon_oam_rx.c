...
void *epon_oam_rxThread(void *argu)
{
    ...
    while(1)
    {
        ...
        if(ret > 60)
        {
            ...
            EPON_OAM_PRINT(EPON_OAM_DBGFLAG_INFO,
                "[OAM:%s:%d] Received message size: %d (0x%08x)\n", __FILE__, __LINE__, dataLen, payload);
            /* MODIFY ME - fill in the actual llidIdx from switch */
            epon_oam_rx_handler(0, payload, dataLen);
        }
        else if(ret > 0)
        {
