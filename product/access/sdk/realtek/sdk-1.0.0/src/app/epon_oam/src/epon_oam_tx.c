    ...
    unsigned short replyLen)
{
    ...
    pReplyPtr = 
        (unsigned char *) malloc(sizeof(unsigned char) * replyLen + EPON_OAMPDU_HDR_LENGTH);
    ...
    unsigned short len)
{
    ...
    pPtr = 
        (unsigned char *) malloc(sizeof(unsigned char) * len + EPON_OAMPDU_HDR_LENGTH);
    ...
    /* Fill in standard header */
    epon_oam_stdHdr_gen(llidIdx, EPON_OAMPDU_CODE_INFO, pPtr);
    pCurr = pPtr + EPON_OAMPDU_HDR_LENGTH;
    /* Payload */
