    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case APOLLO_CHIP_ID:
			/*port 7 for ACL*/
		    if ((ret = reg_array_field_read(STAT_PRVTE_DROP_COUNTERr, 7, REG_ARRAY_INDEX_NONE, PKT_INFOf, &val)) != RT_ERR_OK)
		    {
		        RT_ERR(ret, (MOD_DAL|MOD_STAT), "");
		        return ret;
		    }
			diag_util_printf("Drop Trap Index\n");
            diag_util_printf("%-4s %-4s %d\n",         
				(val & 0x200) ? "V":"", 
        		(val & 0x100) ? "V":"", 
				val&0x7F);
            break;
 ...
 */
cparser_result_t
cparser_cmd_acl_set_reason_cvlan_svlan_priority_policing_forward_extend(
    cparser_context_t *context)
{
    int32 ret = RT_ERR_FAILED;
    uint32 val;
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case APOLLO_CHIP_ID:
		    ...
		    if ((ret = reg_field_write(DOT1X_CFG_0r, ACL_RSNf, &reason)) != RT_ERR_OK)
		    {
		        RT_ERR(ret, (MOD_DAL|MOD_STAT), "");
		        return ret;
		    }
...
}    /* end of cparser_cmd_acl_set_reason_cvlan_svlan_priority_policing_forward_extend */
