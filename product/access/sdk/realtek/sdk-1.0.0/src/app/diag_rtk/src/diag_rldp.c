    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case APOLLOMP_CHIP_ID:
            if('e' == TOKEN_CHAR(4,0))
                DIAG_UTIL_ERR_CHK(dal_apollomp_rlpp_trapType_set(RTK_RLPP_TRAPTYPE_CPU), ret);
            else if ('d' == TOKEN_CHAR(4,0))
                DIAG_UTIL_ERR_CHK(dal_apollomp_rlpp_trapType_set(RTK_RLPP_TRAPTYPE_NONE), ret);
            else
                return CPARSER_NOT_OK;		
            break;
#endif
...
#if defined(CONFIG_SDK_APOLLOMP)
        case APOLLOMP_CHIP_ID:
            DIAG_UTIL_ERR_CHK(dal_apollomp_rlpp_trapType_get(&apollomp_type), ret);
            diag_util_mprintf("RLPP Trap to CPU: %s\n",
                diagStr_enable[(RTK_RLPP_TRAPTYPE_NONE == apollomp_type) ? DISABLED : ENABLED]);
            break;
#endif
        default:
            diag_util_printf("%s\n", DIAG_STR_NOTSUPPORT);
