 ...
 *
 * $Revision: 39285 $
 * $Date: 2013-05-09 10:45:03 +0800 (Thu, 09 May 2013) $
 *
...
#if 1
{
    ...
    ext_portmask.bits[0] = 0x3f;
    ret = rtk_port_isolationExt_set(1, &portmask, &ext_portmask);
    if(ret != RT_ERR_OK)
    {
        diag_util_printf("\n port 1, ret = %d", ret);
    }

    portmask.bits[0] = 0x6F;
    ext_portmask.bits[0] = 0x1F;
    ret = rtk_port_isolationExt_set(2, &portmask, &ext_portmask);
    if(ret != RT_ERR_OK)
    {
        diag_util_printf("\n port 2, ret = %d", ret);
    }
    ...
    portmask.bits[0] = 0x6F;
    ext_portmask.bits[0] = 0x1F;
    ret = rtk_port_isolationExt_set(3, &portmask, &ext_portmask);
    if(ret != RT_ERR_OK)
    {
        diag_util_printf("\n port 3, ret = %d", ret);
    }
    ...
    portmask.bits[0] = 0x7F;
    ext_portmask.bits[0] = 0x3F;
    ret = rtk_port_isolationExt_set(4, &portmask, &ext_portmask);
    if(ret != RT_ERR_OK)
    {
        diag_util_printf("\n port 4, ret = %d", ret);
    }
    ...
    ext_portmask.bits[0] = 0x0F;
    ret = rtk_port_isolationExt_set(5, &portmask, &ext_portmask);
    if(ret != RT_ERR_OK)
    {
        diag_util_printf("\n port 5, ret = %d", ret);
    }
