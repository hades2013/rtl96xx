...
/*for IPv4 Part*/
static int32 
l34lite_netif_ip_set(uint32 netifId, uint32 ipaddr, uint32 prefix)
{
	int32 ret;
...
static int32 
l34lite_ct_add(struct nf_conn *ct, uint32 *naptId)
{
	rtk_l34_tuple_t tuple;
	rtk_ip_addr_t    natIp;
	uint32 		  natport;
	...
	if((ret = rtk_l34_connectTrack_get(tuple,&info))==RT_ERR_OK)
	{
 	...
 	if((ret = rtk_l34_connectTrack_add(dir,tuple, natIp,natport))!=RT_ERR_OK)
 	{
 		VD_DEBUG(NETVD_LOG_LEVEL_HIGH,"%s(), ret=%d\n",__FUNCTION__,ret);
		return  VD_ERR_ADD_FAILED;
 	}
