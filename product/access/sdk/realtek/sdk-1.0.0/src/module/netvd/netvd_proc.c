...
static const char name_ifname[]	 = "ifname";
...
void vd_proc_cleanup(struct net *net)
{
	struct vd_net *vn = net_generic(net, vd_net_id);

	if (vn->proc_vd_conf)
		remove_proc_entry(name_log, vn->proc_vd_dir);

	if (vn->proc_vd_conf)
		remove_proc_entry(name_conf, vn->proc_vd_dir);
...
static int vd_proc_write_ifname(struct file *file, const char __user *buffer,unsigned long count, void *data) 
{ 
...
static int vd_proc_read_ifname( char* page, char** start, off_t off, int count, int* eof, void* data) 
{
    int len = 0 ;
...
/*
 *	Create /proc/net/vd entries
 */

int vd_proc_init(struct net *net)
{
	struct vd_net *vn = net_generic(net, vd_net_id);

	vn->proc_vd_dir = proc_net_mkdir(net, name_root, net->proc_net);
	if (!vn->proc_vd_dir)
		goto err;

	/*for configure netvd interface*/
	vn->proc_vd_conf = proc_create(name_conf, S_IFREG|S_IRUSR|S_IWUSR,
				     vn->proc_vd_dir, &vd_fops);
	if (!vn->proc_vd_conf)
		goto err;

	/*for ct configureation*/
	vn->proc_vd_ct = proc_create(name_ct, S_IFREG|S_IRUSR|S_IWUSR,
				     vn->proc_vd_dir, &vd_ct_fops);
	vn->proc_vd_ct->write_proc = vd_ct_write_proc;
	if (!vn->proc_vd_ct)
		goto err;

	/*for log config*/
	vn->proc_vd_log = proc_create_data(name_log,S_IFREG|S_IRUSR|S_IWUSR,vn->proc_vd_dir,NULL,NULL);
	vn->proc_vd_log->read_proc = vd_proc_read_log;
	vn->proc_vd_log->write_proc = vd_proc_write_log;

	/*for periodic time*/
	vn->proc_vd_time = proc_create_data(name_time,S_IFREG|S_IRUSR|S_IWUSR,vn->proc_vd_dir,NULL,NULL);
	vn->proc_vd_time->read_proc = vd_proc_read_time;
	vn->proc_vd_time->write_proc = vd_proc_write_time;

	/*for wan name*/
	vn->proc_vd_ifname = proc_create_data(name_ifname,S_IFREG|S_IRUSR|S_IWUSR,vn->proc_vd_dir,NULL,NULL);
	vn->proc_vd_ifname->read_proc = vd_proc_read_ifname;
	vn->proc_vd_ifname->write_proc = vd_proc_write_ifname;
...
int vd_proc_add_dev(struct net_device *vdev)
{
	...
	struct vd_net *vn = net_generic(dev_net(vdev), vd_net_id);
	...
	dev_info->dent =
		proc_mkdir(vdev->name, vn->proc_vd_dir);
	proc_file_config = 
...
int vd_proc_rem_dev(struct net_device *vdev)
{
	...
	if (vd_dev_info(vdev)->dent) {
		remove_proc_entry(vd_dev_info(vdev)->dent->name,
				  vn->proc_vd_dir);
