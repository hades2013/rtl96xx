 ...
 *
 * $Revision: 41016 $
 * $Date: 2013-07-10 14:03:23 +0800 (Wed, 10 Jul 2013) $
 *
...
int getNetIfIdByDevName(const char *devName)
{
	char *p,*end;
	int netifId=-1,len=2;
...
static int netdev_create_handler(const struct net_device *dev)
{
	...
	netifId = getNetIfIdByDevName(dev->name);

	if(netifId < 0)
		return RT_ERR_INPUT;

	/*check interface type wan or lan*/
	type = (netifId >= NETVD_WAN_START)  ?  NETVD_NETIF_WAN : NETVD_NETIF_LAN;
...
static int netdev_mtu_handler(const struct net_device *dev)
{
	...
	VD_DEBUG(NETVD_LOG_LEVEL_LOW,"%s, mtu=%d, ret=%d\n",dev->name,mtu,ret);
...
static int vd_device_event(struct notifier_block *nb, unsigned long event,void *ptr)
{
	...
	if(strstr(dev->name,wanName)!=NULL || strstr(dev->name,"ppp")!=NULL){
...
static int ip_event(struct notifier_block *nb, unsigned long events, void *ptr)
{
	...
	VD_DEBUG(NETVD_LOG_LEVEL_LOW,"DevName: %s, IPv4: %u.%u.%u.%u, Mask: %d\n",dev->name,NIPQUAD(ifa->ifa_address),ifa->ifa_prefixlen);

	/*is br interface to configure to HW*/
	if(strstr(dev->name,wanName)!=NULL)
	{
	...
	case NETDEV_UP:
		VD_DEBUG(NETVD_LOG_LEVEL_LOW,"%s,%s\n",dev->name,"NETDEV_UP");
		ret = mapper->hal_netif_ip_set(netifId,ifa->ifa_address, ifa->ifa_prefixlen);
	break;
	case NETDEV_DOWN:
		VD_DEBUG(NETVD_LOG_LEVEL_LOW,"%s,%s\n",dev->name,"NETDEV_DOWN");
		ret = mapper->hal_netif_ip_set(netifId, 0, 0);
	break;
	}
	VD_DEBUG(NETVD_LOG_LEVEL_LOW,"set ip %u.%u.%u.%u, ret=%d\n",NIPQUAD(ifa->ifa_address),ret);
...
static int conntrack_new_event( struct nf_conn *ct)
{
	...
	ret = mapper->hal_ct_add(ct,&naptId);
...
ip_mroute_update(void)
{
	...
	{	
		...
		if((ret = netvd_get_defaultGw(i,&gwInfo))!=VD_ERR_OK)
		{
			...
			netifId = getNetIfIdByDevName(gwInfo.devName);
			if((ret = mapper->hal_arp_get(gwInfo.gwIp,&mac[0]))!=VD_ERR_OK)
			{
				memset(&mac[0],0,ETHER_ADDR_LEN);
				VD_DEBUG(NETVD_LOG_LEVEL_LOW,"%s(): DELETE ROUTE TABLE %d DEFAULT GW\n",__FUNCTION__,i);
			}else{
...
ip_route_update(void)
{
	...
	if((ret = netvd_get_defaultGw(RT_TABLE_MAIN,&gwInfo))!=VD_ERR_OK)
	{
		...
		ret = mapper->hal_arp_get( gwInfo.gwIp,&mac[0]);
		if(ret !=VD_ERR_OK)  goto mroute;
