 ...
 *
 * $Revision: 40712 $
 * $Date: 2013-07-03 11:32:24 +0800 (Wed, 03 Jul 2013) $
 *
...
static int32 _raw_rtl9601b_acl_rule_set(rtl9601b_raw_acl_ruleEntry_t *pAclRule)
{
    ...
    if ((ret = table_field_set(RTL9601B_ACL_MASKt, RTL9601B_ACL_MASK_TAGStf, &tmpData,fieldData)) != RT_ERR_OK)
    { 
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
        return ret;
    }
    ...
    tmpData = 0;
    if ((ret = table_field_set(RTL9601B_ACL_MASKt, RTL9601B_ACL_MASK_RSVtf, &tmpData,fieldData)) != RT_ERR_OK)
    { 
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
        return ret;
    }
...
dal_rtl9601b_acl_igrRuleAction_set(rtk_acl_ingress_entry_t *pAclRule)
{
    ...
    val = pAclRule->valid;
    if ((ret = reg_array_field_write(RTL9601B_ACL_ACTIONr, REG_ARRAY_INDEX_NONE, index, RTL9601B_VALIDf, &val)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
        return ret;
    }
    ...
    if(pAclRule->act.enableAct[ACL_IGR_LOG_ACT])
    {
		...
		if ((ret = table_field_set(RTL9601B_ACL_ACTION_TABLEt, RTL9601B_ACL_ACTION_TABLE_POLICACTtf, &val, actionData)) != RT_ERR_OK)
	    {
	        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
	        return ret;
	    }
...
dal_rtl9601b_acl_igrRuleAction_get(rtk_acl_ingress_entry_t *pAclRule)
{
    ...
    if ((ret = reg_array_field_read(RTL9601B_ACL_ACTIONr, REG_ARRAY_INDEX_NONE, index, RTL9601B_VALIDf, &val)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
        return ret;
    }
    ...
    /*Logging or Policing action control*/
	if ((ret = table_field_get(RTL9601B_ACL_ACTION_TABLEt, RTL9601B_ACL_ACTION_TABLE_POLICACTtf, &val, actionData)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
        return ret;
    }
...
dal_rtl9601b_acl_igrRuleEntry_add(rtk_acl_ingress_entry_t *pAclRule)
{
    ...
    if ((ret = reg_field_read(RTL9601B_ACL_CFGr, RTL9601B_MODEf, &val)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
        return ret;
    }
...
dal_rtl9601b_acl_igrRuleEntry_delAll(void)
{
    ...
    if((ret = rtl9601b_l2_table_clear(RTL9601B_ACL_DATAt,0,index)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
		return ret;
    }

    if((ret = rtl9601b_l2_table_clear(RTL9601B_ACL_MASKt,0,index)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
		return ret;
    }
    ...
    if((ret = rtl9601b_l2_table_clear(RTL9601B_ACL_ACTION_TABLEt,0,index)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
		return ret;
    }
