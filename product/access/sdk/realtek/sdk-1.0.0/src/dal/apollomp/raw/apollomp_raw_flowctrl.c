...
int32 apollomp_raw_flowctrl_ponLatchMaxUsedPage_set( rtk_enable_t enable)
{
    ...
    tmp = (!enable)<<1;
    if ((ret = reg_array_field_write(APOLLOMP_RSVD_EGR_OUTQr, REG_ARRAY_INDEX_NONE, 0, APOLLOMP_RSVD_MEMf, &tmp)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }
...
int32 apollomp_raw_flowctrl_ponLatchMaxUsedPage_get( rtk_enable_t *pEnable)
{
    ...
    if ((ret = reg_array_field_read(APOLLOMP_RSVD_EGR_OUTQr, REG_ARRAY_INDEX_NONE, 0, APOLLOMP_RSVD_MEMf, &tmp)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }

    *pEnable = !((tmp&0x0002)>>1); 
