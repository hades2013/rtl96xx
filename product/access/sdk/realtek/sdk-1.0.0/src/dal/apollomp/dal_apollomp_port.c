 ...
 *
 * $Revision: 40051 $
 * $Date: 2013-06-06 16:32:22 +0800 (Thu, 06 Jun 2013) $
 *
...
dal_apollomp_port_adminEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
	...
	/*begin add by shipeng 2013-06-19*/	
    else if(HAL_IS_PHY_EXIST(port))
    {
	    if((ret = dal_apollomp_port_phyReg_get(port, 0, PHY_REG_CONTROL, &data)) != RT_ERR_OK)
	    {
	        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
	        return ret;
	    }
...
dal_apollomp_port_adminEnable_set(rtk_port_t port, rtk_enable_t enable)
{
	...
	/*begin modify by shipeng 2013-06-19*/
	#if 0
    if(DISABLED == enable)
        data |= (0x01 << port);
    else
	...
	if(HAL_IS_PHY_EXIST(port))
	{
		if((ret = phy_enable_set(port, enable)) != RT_ERR_OK)
	    {
	        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
	        return ret;
	    }
	}		
	/*end modify for link up or down by shipeng 2013-06-19*/
