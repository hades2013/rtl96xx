...
#define APOLLOMP_MII_CLOCK_GPIO_PIN (60)
#define APOLLOMP_MII_DATA_GPIO_PIN  (61)
    ...
    #define RTL9601B_DEFAULT_MIIM_PAGE    (0xA40)
        ...
        rtl9601b_acl_rule_entry_t Rtl9601bVirtualAclDataTable[SDK_ASICDRV_RTL9601B_TEST_CFACTION_SIZE + 1];
...
#if 0  /*TBD*/
    int32 rtl9601b_lutStToTblData(apollomp_lut_table_t *pL2Table, uint32 *pTblData);
    int32 rtl9601b_tblDataToLutSt(apollomp_lut_table_t *pL2Table, uint32 *pTblData);
#endif
    #if defined(CONFIG_SDK_ASICDRV_TEST)
    ...
    uint32  endIdx)
{
    ...
    {
        /*set address*/
        if ((ret = reg_field_set(RTL9601B_TBL_ACCESS_CTRLr, RTL9601B_ADDRf, &addr, &regData)) != RT_ERR_OK)
        {
            return ret;
        }
    ...
    uint32      *pData)
{
    ...
    {
        if(phyReg <= 7)
            real_page = RTL9601B_DEFAULT_MIIM_PAGE;
        else if(phyReg <= 15)
            real_page = RTL9601B_DEFAULT_MIIM_PAGE + 1;
        else
            real_page = page;
    }
    else
