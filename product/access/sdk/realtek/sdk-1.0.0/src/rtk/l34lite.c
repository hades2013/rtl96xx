...
static int32
l34_arp_range_assign(uint32 netifId, uint32 *arpStart,uint32 *arpEnd)
{	
	uint32 arpSize;
	arpSize 	= (HAL_L34_ARP_ENTRY_MAX()/L34_IF_NUM_MAX);
	*arpStart 	= (netifId*arpSize) >> 2;
	*arpEnd   	= ((netifId*arpSize) + arpSize -1) >> 2;
	return RT_ERR_OK;
}
...
{
	uint32 tmp;	
	l34_arp_range_assign(netifId,&route->arpStart,&route->arpEnd);
	route->netifIdx 	= netifId;
	route->process  	= L34_PROCESS_ARP;
	route->ipAddr   	= 0;
	route->ipMask 	= 0;
	route->rt2waninf 	=  (netifType==L34_NETIF_TYPE_WAN) ?  1 : 0;
...
static int32
l34_us_connection_add(uint32 netifId, rtk_l34_tuple_t tuple, rtk_ip_addr_t natIp,uint16 natport)
{
	...
	hashId = l34_hashIn(isTcp,natIp,natport);
	/*start to assign naptr table*/
	osal_memset(&naptr,0,sizeof(rtk_l34_naptInbound_entry_t));
	naptr.extIpIdx		= netifId;
	naptr.extPortLSB	= natport & 0xff;
	naptr.isTcp = isTcp;
	naptr.intIp = tuple.srcIp;
	naptr.intPort = tuple.srcport;
	naptr.priId = iptable.prival;
	naptr.priValid = iptable.pri;
	...
	if((ret=rtk_l34_naptOutboundTable_set(DISABLED, idx, &napt))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
...
static int32
l34_ds_connection_add(uint32 netifId, rtk_l34_tuple_t tuple, rtk_ip_addr_t natIp,uint16 natport)
{
	...
	hashId = l34_hashOut(isTcp,natIp,natport,tuple.srcIp,tuple.srcport);
	...
	if((ret=rtk_l34_naptOutboundTable_set(DISABLED, hashId, &napt))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
	RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "add upstream connection to napt id %d\n",idx);
...
l34_globalState_get(rtk_enable_t *status)
{
	...
	if((ret=rtk_l34_globalState_get(L34_GLOBAL_L34_STATE,&status[L34_GLOBAL_L34_STATE])) != RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}

	if((ret=rtk_l34_globalState_get(L34_GLOBAL_L3NAT_STATE,&status[L34_GLOBAL_L3NAT_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}

	if((ret=rtk_l34_globalState_get(L34_GLOBAL_L4NAT_STATE,&status[L34_GLOBAL_L4NAT_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
	if((ret=rtk_l34_globalState_get(L34_GLOBAL_L3CHKSERRALLOW_STATE,&status[L34_GLOBAL_L3CHKSERRALLOW_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}

	if((ret=rtk_l34_globalState_get(L34_GLOBAL_L4CHKSERRALLOW_STATE,&status[L34_GLOBAL_L4CHKSERRALLOW_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}

	if((ret=rtk_l34_globalState_get(L34_GLOBAL_NAT2LOG_STATE,&status[L34_GLOBAL_NAT2LOG_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
	if((ret=rtk_l34_globalState_get(L34_GLOBAL_FRAG2CPU_STATE,&status[L34_GLOBAL_FRAG2CPU_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}

	if((ret=rtk_l34_globalState_get(L34_GLOBAL_TTLMINUS_STATE,&status[L34_GLOBAL_TTLMINUS_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
	if((ret=rtk_l34_globalState_get(L34_GLOBAL_BIND_STATE,&status[L34_GLOBAL_BIND_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
	if((ret=rtk_l34_globalState_get(L34_GLOBAL_PPPKEEP_STATE,&status[L34_GLOBAL_PPPKEEP_STATE]))!=RT_ERR_OK){
		RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
		return ret;
	}
...
l34_compare_ip6_addr(rtk_ipv6_addr_t ipA,rtk_ipv6_addr_t ipB,uint32 prefixLen)
{
	...
	a = ipA.ipv6_addr[max]  >> shiftBit;
	b = ipB.ipv6_addr[max]  >> shiftBit;
...
rtk_l34_route_add(rtk_l34_routeType_t routeType,rtk_l34_routeTable_t routeTable)
{
	...
	{
		if((ret = l34_global_route_set(L34_STATIC_ROUTE,routeTable.netifId,routeTable.ipaddr,routeTable.ipmask,routeTable.gatewayMac))!=RT_ERR_OK)
		{
			RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
			return ret;
		}
...
rtk_l34_route_get(uint32 routeIndex,rtk_l34_routeInfo_t *routeInfo)
{
	...
	case L34_PROCESS_ARP:
	{
		...
		routeInfo->arpEnd   = route.arpEnd <<2;		
	}
	break;
	case L34_PROCESS_NH:
	{
 ...
 *	  natip: napt transfer ip.
 *      natport:  napt transfer ip
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK
 ...
 */
int32
rtk_l34_connectTrack_add(rtk_l34_direct_t dir, rtk_l34_tuple_t tuple,rtk_ip_addr_t natIp, uint16 natport)
{
    	int32 ret=0;
	rtk_ip_addr_t extIp;
	uint32 netifId;
	...
	extIp = (dir==L34_DIR_UPSTREAM) ?  natIp :  tuple.dstIp;
	...
	case L34_DIR_UPSTREAM:
		if((ret =l34_us_connection_add(netifId,tuple,natIp,natport)) != RT_ERR_OK){
			RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
			return RT_ERR_FAILED;
		}
	break;
	case L34_DIR_DOWNSTREAM:
		if((ret =l34_ds_connection_add(netifId,tuple,natIp,natport)) != RT_ERR_OK){
			RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L34), "return failed ret value = %x",ret);
			return RT_ERR_FAILED;
		}
	break;
