...
/*specific definition wrong, set1 real mapping to ABCD*/
#define GPIO_SET1_IRQ BSP_GPIO_EFGH_IRQ
/*specific definition wrong, set2 real mapping to EFGH*/
#define GPIO_SET2_IRQ BSP_GPIO_ABCD_IRQ
...
static rtk_gpio_intrMode_t gpioImrBackup[GPIO_IMR_NUM];
/*
 * Macro Definition
 */
...
gpio_interrupt_th(int irq, void *dev_instance)
{
	...
	/*should disable all IMR*/
	/*uint32 reg=0xb8003330,value = 0;
	value = RTL_R32(reg);
	printk("%s: IMR value %x\n",__FUNCTION__,value);	
	RTL_W32(reg,0);
	*/
