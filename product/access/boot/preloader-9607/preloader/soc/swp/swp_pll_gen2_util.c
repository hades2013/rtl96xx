...
#include "swp_pll_gen2_util.h"
#include "swp_util.h"

#define swp_info_t pll_gen2_info_t
#define PARA_SECTION __attribute__ ((section (".cli_pll_para")))
#include "swp_access_soc.h"

inline static u32_t
_swp_pg2_query_cpu_clk(const pll_gen2_info_t *i) {
    pll_gen2_mhz_t mhz;
    pll_gen2_get_to_mhz(i, &mhz);
    return mhz.cpu;
}
inline static u32_t
_swp_pg2_query_mem_clk(const pll_gen2_info_t *i) {
    pll_gen2_mhz_t mhz;
    pll_gen2_get_to_mhz(i, &mhz);
    return mhz.mem;
}
inline static u32_t
_swp_pg2_query_lx_clk(const pll_gen2_info_t *i) {
    pll_gen2_mhz_t mhz;
    pll_gen2_get_to_mhz(i, &mhz);
    return mhz.lx;
}
...
swp_query_mem_clk(void){
	return _swp_pg2_query_mem_clk(&para_pll_info);
}

u32_t
query_pg2_uart_baudrate(void) {
    ...
    pll_gen2_mhz_t mhz;
	pll_gen2_get_to_mhz(&para_pll_info, &mhz);
...
u32_t
set_pg2_uart_baud_div(const u32_t baudrate) {
    /* Foumula: baudrate = LX_CLK_HZ/(16*divisor)
            <-->divisor = LX_CLK_HZ/(16*baudrate) */
    u32_t br_div;
...
define_rv_para(sys_cpu_pll_ctl0, 0);//CPU/DSP/MEM
define_rv_para(sys_cpu_pll_ctl1, 0);//CPU/DSP/MEM
    define_vv_para(cpu_clk, _swp_pg2_query_cpu_clk(info), -1, DPT_DECIMAL);
define_rv_para(sys_mem_pll_ctl0, 0);//CPU/DSP/MEM    
define_rv_para(sys_mem_pll_ctl1, 0);//CPU/DSP/MEM
    define_vv_para(mem_clk, _swp_pg2_query_mem_clk(info), -1, DPT_DECIMAL);
define_rv_para(sys_lx_pll_ctl0, 0);//CPU/DSP/MEM
define_rv_para(sys_lx_pll_ctl1, 0);//CPU/DSP/MEM
    define_vv_para(lx_clk, _swp_pg2_query_lx_clk(info), -1, DPT_DECIMAL);

extern as_para_t cli_pll_para_begin, cli_pll_para_end;
SECTION_ON_FLASH int
get_pll_gen2_info(const pll_gen2_info_t *info, const char *pname, char *buf) {
    //return swp_access_soc_get(info, pname, buf, (&__pll_para_begin)+1, &__pll_para_end);
    return swp_access_soc_get(info, pname, buf, &cli_pll_para_begin, &cli_pll_para_end);
}
SECTION_ON_FLASH int
set_pll_gen2_para(pll_gen2_info_t *info, const char *pname, const char *value) {
    //return swp_access_soc_set(info, pname, value, (&__pll_para_begin)+1, &__pll_para_end);
    return swp_access_soc_set(info, pname, value, &cli_pll_para_begin, &cli_pll_para_end);
}
SECTION_ON_FLASH void
dump_pll_gen2_info(const pll_gen2_info_t *info, const char *fmt, u32_t flags, u32_t flags_mask) {
    //return swp_access_soc_dump(info, fmt, flags, flags_mask, (&__pll_para_begin)+1, &__pll_para_end);
