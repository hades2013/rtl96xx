...
/* #define VER_STR "$Revision: 1465 $" */
#define FLASH_INIT_PROMPT_STRING "$Revision: 1465 $"
#else
...
static sffunc_ptr sffunc_wren_wrdi_eqcmd, sffunc_rdid, sffunc_en4b, sffunc_wrsr, sffunc_rdsr_rdsr2, sffunc_rdbusy;
...
#define SFCMD_EN4B_FUNC (sffunc_en4b)
 ...
 */
	...
	for (; fi<fi_end; fi++) {
		...
		} else if((fi_generic == NULL) && (GENERIC_FLASH_ID == /*fi->id*/genFlash.id)) {
...
static u32_t sffunc_en4b(u8_t cid, u32_t sfcsr_val, u32_t opcode, const u32_t data, u32_t wr_leng, void *wr_buf) {
	SFCSR = sfcsr_val;
	CHECK_CTRL_READY();
	SFDR = opcode;
	return 0;
