...
#include <soc.h>
#include <pblr.h>
/*begin add by leijinbao for mac Encryption*/
#include <cipher.h>
/*end  by leijinbao for mac Encryption*/
...
/*begin del by shipeng for set mac 2103-08-19*/
#if 0
...
#endif
#endif
/*end del by shipeng for set mac 2103-08-19*/
		/*
...
{
	/*modify by leijinbao 2014/05/06 for  bug 565*/
	char szClearText[64];
	char *cp = NULL;
	/*end by leijinbao 2014/05/06 for  bug 565*/
	...
	if (gd->flags & GD_FLG_ENV_READY) { /* after import into hashtable */
		...
		e.data	= NULL;
		hsearch_r(e, FIND, &ep, &env_htab);
		/*begin add by leijinbao for mac Encryption 2013/12/23*/
		if(0 == strcmp(name,"ethaddr"))
		{
			if(NULL != ep->data)
			{
				if(1 == IsStringN63Cipher(ep->data))
		    	{	
		    		memset(szClearText,0,sizeof(szClearText));
					DecryptN63Password(ep->data,szClearText);
				/*add by leijinbao 2014/05/06 for mac&product Encryption  bug 565*/
				...
				/*end by leijinbao 2014/05/06 for mac&product Encryption bug 565*/
					sprintf(ep->data, "%s", szClearText);
					return ep->data;
				}
				
			}
		}
		/*end by leijinbao for mac Encryption  2013/12/23*/
