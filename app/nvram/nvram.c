...
nvram_handle_t * nvram_open(const char *file, int rdonly)
{
	...
	if( (fd = open(file ? file : mtd, O_RDWR)) > -1 )
	{
		...
		{
			if(nvram_erase_size<NVRAM_SPACE)
			...
			{
                NVRAM_DBG("mmap_area[%d]=0x%x",i,((uint32_t *)mmap_area)[i]);
			...
			else if( (h = malloc(sizeof(nvram_handle_t))) != NULL )
			{
                ...
                NVRAM_DBG("header->magic=0x%x",header->magic);
...
/*****************************************************************
Function:nvram_handle_remalloc
Description:remalloc nvram_handle 
Author:liaohongjun
Date:2012/6/23
INPUT:file ,rdonly        
OUTPUT:         
...
char * nvram_find_staging(const char *file)
{
	...
	if( (stat(file, &s) > -1) && (s.st_mode & S_IFREG) )
	{
		return file;
	}
...
int nvram_to_staging(const char *file)
{
	...
	if( (mtd != NULL) && (nvram_erase_size > 0) )
	{
		if( (fdmtd = open(mtd, O_RDONLY)) > -1 )
		{
			...
			{
				if((fdstg = open(file, O_WRONLY | O_CREAT, 0600)) > -1)
				{
		...
		if( (fdstg = open(NVRAM_STAGING, O_RDONLY)) > -1 )
		{
			if( read(fdstg, buf, sizeof(buf)) == sizeof(buf) )
			{
				if( (fdmtd = open(mtd, O_WRONLY | O_SYNC)) > -1 )
				{
                    ...
                    /*begin added by liaohongjun 2012/6/25*/
                    erase.start = 0;
                    erase.length = nvram_erase_size;
                    ret = ioctl(fdmtd, MEMERASE, &erase);
                    /*end added by liaohongjun 2012/6/25*/
                    NVRAM_DBG("ret=%ld",ret);
                    #endif
					ret = write(fdmtd, buf, sizeof(buf));
                    NVRAM_DBG("ret=%ld",ret);
					fsync(fdmtd);
					close(fdmtd);
					stat = 0;
...
nvram_handle_t * nvram_open_rdonly(const char *path)
{
  /*begin modified by huangmingjian 2012-08-15 for EPN104QID0005*/
  const char *file = nvram_find_staging(path);
  /*end modified by huangmingjian 2012-08-15 for EPN104QID0005*/
...
{
    if( nvram_find_staging(file) != NULL || nvram_to_staging(file) == 0 )
    {
        NVRAM_DBG("open /tmp/%s and do mmap",file);
        return nvram_open(file, NVRAM_RW);
    }
    return NULL;
}
/*end modified by huangmingjian 2012-08-15 for EPN104QID0005*/
