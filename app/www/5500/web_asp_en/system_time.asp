...
function apply() 
{
	...
	if (f.tSYNC.checked && (f.tSERVER.value.trim() == '')){
		alert("Please input valid time server URL");
		return ;	
	}
...
<tr>
	<td width=30% class=bgblue>Local Time</td>
	<td width=70% class=bggrey><span><font color=green size=2><b id=localTime> </b></font></span></td>
</tr>
<tr>
	<td class=bgblue>Time Zone</td>
	<td class=bggrey>
...
<tr>
	<td class=bgblue>Defalut Time Server(Optional)</td>
	<td class=bggrey>
		<table width=100% border=0 cellpadding=3 cellspacing=1>
		<tr>
			<td width=20% nowrap>Synchronous</td>
			<td width=80%><input type=checkbox name=tSYNC onClick=on_sync_change();></td>
		</tr>
		<tr>
			<td nowrap>Time Server</td>
			<td><input name=tSERVER size=25 maxlength=30></td>
		</tr>
        </table>
	</td>
</tr>
<tr>
	<td class=bgblue>Time Setup</td>
	<td class=bggrey>
		<table width=100% border=0 cellpadding=3 cellspacing=1>
		<tr>
			<td>
				<select name=tYEAR size=1 onChange=on_input_time_change();>
	  				<script>createSelectTagChild(2011,2099,pc_date.getFullYear(),"")</script>
	  			</select>Year
