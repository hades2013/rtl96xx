...
function ip_proto_on_change()
{
	...
	if(f.PROTO.value == 'static') {
		...
		str += '<tr>';
		str += '<td class=tb_title>IP 地址</td>';
...
//		ip_set(f.DNS2, get_dns('static', 1));
		...
		str += '<td class=tb_content width=35% align=center><input name="dhr" type=button value="Update" onclick=apply_all("renew");>';
		//str += '&nbsp;&nbsp;&nbsp;&nbsp;<input name="dhr" type=button value="Release" onclick=apply_all("none"); disabled=true></td>';
		str += '</tr>';
		str += '<tr><td class=tb_title>IP 地址</td>';
	...
	<tr>
		...
		<table width=100% border=0 align=center cellpadding=5 cellspacing=1	bgcolor="#FFFFFF">
			...
			<tr>
				...
				<td class=tb_content><input name=HN size=20 style="ime-mode:disabled" maxlength=16>(1-16 characters)</td>
			</tr>
		</table>
