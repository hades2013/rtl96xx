...
env['sys.parent_name'] = 'StormFilter';
...
var storm_ctl = new Array(<% CGI_GET_SINGLE_STORMCTL(); %>);
var numPorts = 4;
//var storm_ctl = new Array('1','1','1','5','3');
var pMsk = storm_ctl[4];
...
function storm_rate_init()
{
	var f=document.forms[0];
	var rate; 
	
	f.cfg_uucast_rate.value = storm_ctl[0];
	f.cfg_umcast_rate.value = storm_ctl[1]
	f.cfg_bcast_rate.value = storm_ctl[2];
	f.cfg_storm_rate.value = storm_ctl[3];
/*	
	rate = parseInt(storm_uucast(), 10);
	f.cfg_uucast_rate.value = (rate == 0) ? '0' : '1';
	f.cfg_uucast_rate_value.disabled = (rate == 0) ? true : false;
	if (rate != 0){
		if (rate_in_mbps(rate)){
			rate = rate / 1024;
			f.uucast_rate_unit[0].checked = true;	
		}	else {
			f.uucast_rate_unit[1].checked = true;	
		}
		f.cfg_uucast_rate_value.value = rate;
	}else {
			f.cfg_uucast_rate_value.value = "";		
			f.uucast_rate_unit[0].checked = true;			
	}
		
	rate = parseInt(storm_umcast(), 10);
	f.cfg_umcast_rate.value = (rate == 0) ? '0' : '1';
	f.cfg_umcast_rate_value.disabled = (rate == 0) ? true : false;
	if (rate != 0){
		if (rate_in_mbps(rate)){
			rate = rate / 1024;
			f.umcast_rate_unit[0].checked = true;	
		}	else {
			f.umcast_rate_unit[1].checked = true;	
		}
		f.cfg_umcast_rate_value.value = rate;
	}else {
			f.cfg_umcast_rate_value.value = "";		
			f.umcast_rate_unit[0].checked = true;			
	}
	
	rate = parseInt(storm_bcast(), 10);
	f.cfg_bcast_rate.value = (rate == 0) ? '0' : '1';
	f.cfg_bcast_rate_value.disabled = (rate == 0) ? true : false;
	if (rate != 0){
		if (rate_in_mbps(rate)){
			rate = rate / 1024;
			f.bcast_rate_unit[0].checked = true;	
		}	else {
			f.bcast_rate_unit[1].checked = true;	
		}
		f.cfg_bcast_rate_value.value = rate;
	}else {
			f.cfg_bcast_rate_value.value = "";		
			f.bcast_rate_unit[0].checked = true;			
	}
	
	rate = parseInt(storm_mcast(), 10);
	f.cfg_mcast_rate.value = (rate == 0) ? '0' : '1';
	f.cfg_mcast_rate_value.disabled = (rate == 0) ? true : false;
	if (rate != 0){
		if (rate_in_mbps(rate)){
			rate = rate / 1024;
			f.mcast_rate_unit[0].checked = true;	
		}	else {
			f.mcast_rate_unit[1].checked = true;	
		}
		f.cfg_mcast_rate_value.value = rate;
	}else {
			f.cfg_mcast_rate_value.value = "";		
			f.mcast_rate_unit[0].checked = true;			
	}	
	*/	
}
...
function apply()
{
	...
	*/
	...
	diffCfg('uucasten', 'uucasten',   f.cfg_uucast_rate.value);	
	diffCfg('umcasten', 'umcasten',   f.cfg_umcast_rate.value);	
    diffCfg('bcasten', 'bcasten',   f.cfg_bcast_rate.value);	
	diffCfg('rate', 'rate', f.cfg_storm_rate.value);
	diffCfg('portmask', 'portmask', f.portmask.value);	
	subForm(f,'goform/command','STORM_RATE', "port_storm.asp");
}
...
<table width=100% cellpadding=5 cellspacing=0>
	<tr ><td colspan=2 class="tb_head">广播风暴抑制</td> </tr>
	...
	<tr>
		<td width=40% class=tb_title>未知单播报文抑制</td>
		<td width=60% class=tb_content>
			<select name=cfg_uucast_rate style="WIDTH: 100px" onchange="rate_on_change();">
			<option value="0">关闭</option>
			<option value="1">开启</option>
			</select>
			<!--
			<input name=cfg_uucast_rate_value size=8 maxlength=8>
			<input type=radio name=uucast_rate_unit value="Mbps"> Mbps
			<input type=radio name=uucast_rate_unit value="Kbps"> Kbps
			-->
		</td>
	</tr>
	<tr>
		...
		<td class=tb_content>
			<select name=cfg_umcast_rate style="WIDTH: 100px" onchange="rate_on_change();" >
			<option value="0">关闭</option>
			<option value="1">开启</option>
			</select>
			<!--
			<input name=cfg_umcast_rate_value size=8 maxlength=8>
			<input type=radio name=umcast_rate_unit value="Mbps"> Mbps
			<input type=radio name=umcast_rate_unit value="Kbps"> Kbps
			-->
		</td>
	</tr>	
	<tr>
		<td class=tb_title>广播报文抑制</td>
		<td class=tb_content>
			<select name=cfg_bcast_rate style="WIDTH: 100px" onchange="rate_on_change();">
			<option value="0">关闭</option>
			<option value="1">开启</option>
			</select>
			<!--
			<input name=cfg_bcast_rate_value size=8 maxlength=8>
			<input type=radio name=bcast_rate_unit value="Mbps"> Mbps
			<input type=radio name=bcast_rate_unit value="Kbps"> Kbps
			-->
		</td>
	</tr>
	<tr ><td colspan=2 class="tb_head">广播风暴抑制比设置</td> </tr>			
	<tr>
		<td width=20% class=tb_title>抑制比</td>
		<td width=80% class=tb_content>
			<select name=cfg_storm_rate style="WIDTH: 100px" onchange="rate_on_change();">
