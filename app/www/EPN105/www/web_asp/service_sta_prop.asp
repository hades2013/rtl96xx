...
function get_dev_status()
{
	...
	var inner_str;	
	if (get_dev_id() < 0){
		inner_str = '<td width=80%> 不支持设备</td>';
	}else {
		var str_onoff = (dev_is_online() ? '在线' : '不在线');
		var str_config;
		var str_button;
		if (dev_is_unconfig()){
			str_config = '已配置!';	
		}else {
			str_config = "已配置,使用模板 '" + get_dev_tmpl_name() + "'";
		}
		str_button = dev_in_userlist() ? '编辑配置' : '建立配置';
...
function init()
{
	document.getElementById('page_header').innerHTML = '设备 - ' + get_dev_mac();
}
</script>
</head>
<body onload=init()>
...
<table width=100% border=0 align=center cellpadding=5 cellspacing=1
	class=box_tn>
	<tr>
		<td colspan=2 class=titleCell>设备信息</td>
	</tr>
	<tr>
...
<tr>
<td width=15% class=titleCell>端口</td>
<td width=10% class=titleCell>连接状态</td>
<td width=15% class=titleCell>速率</td>
<td width=30% class=titleCell>发送报文总数</td>
<td width=30% class=titleCell>接收报文总数</td>
</tr>
<script> document.write(get_dev_port_stats()); </script>
</table>
<p>
<input type=button value='刷新' onclick="reload();">&nbsp;&nbsp;&nbsp;&nbsp;
<input type=button value='返回' onclick="gotoUrl('service_map.asp');">
</p>
</form>
