...
<table width=100% border=0 align=center cellpadding=0 cellspacing=0
	...
	<tr>
		...
		<tr>
				<td width=30% class=bgblue>组名</td>
 				<td width=70% class=bggrey><input type="text" size="32" name="SNMP_GROUP_NAME" id="SNMP_GROUP_NAME" maxlength="32">(1~32字符)</TD> 
				<input style="display:none" mce_style="display:none"><!--只有一个input类型为text的,使其回车不提交表单-->
		</tr>
		
		<tr>
			<td class=bgblue>安全级别</td>
 			<td class=bggrey><select name="SNMP_GROUP_SECURITY" style="WIDTH: 228px"><option value=0> 不认证不加密</option><option value=1>只认证不加密</option> <option value=2>既认证又加密</option></select></td>
		</tr>
		
		<tr>
			<td class=bgblue>v3访问模式</td>
 			<td class=bggrey><select name="SNMP_GROUP_MODE" style="WIDTH: 228px"><option value=0> 只读</option><option value=1>读写</option></select></td>
		</tr>
		<tr>
