...
function verifyMAC(ma,s,sp){
	...
	for (var i=0;i<6;i++)
	{
		if (!t.test(m[i])) { alert(s+" 包含非法字符!!!");  return false; }
    }
    return true
}
...
function ipMskChk(mn,str)
{
	...
	{
		m=mn.value.split('.');
		if (m.length!=4) { alert(str+" 包含非法字符!!!") ; return 0; }
	}

	var t = /[^0-9]{1,}/;
	for (var i=0;i<4;i++)
	{
		if (t.test(m[i])||m[i]>255) { alert(str+" 包含非法字符!!!") ; return 0; }
	}
	...
	if (f==0) { alert(str+" 包含非法字符!!!") ; return 0; }
	return 1;	
}
