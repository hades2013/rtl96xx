...
function init()
{
	pageview_add('局端', 0, '5%', false);
	pageview_add('MAC地址', 2, '20%', true, null, "sta_info");
	pageview_add('型号', 6, '15%', true, dev_string);
	pageview_add('状态', 7, '10%', true, link_string);
	pageview_add('发送速率(Mbps)', 4, '12%', false, rate_string);
	pageview_add('接收速率(Mbps)', 5, '12%', false, rate_string);
	pageview_add('用户名', 9, '17%', false);
	pageview_init(client_list, 10, document.getElementById('list_table'), true, checkbox_click, set_row_color, '设备列表');
}
</script>
</head>
...
<table width=100% border=0 align=center cellpadding=0 cellspacing=1>
	<tr><td>
	<input id="btn_wht" type=button value="添加到白名单" onclick="apply('ADD_TO_WHITE');" disabled=true>
	<input id="btn_blk" type=button value="添加到黑名单" onclick="apply('ADD_TO_BLACK');" disabled=true>&nbsp;&nbsp;&nbsp;&nbsp;
	<input type=button value="   刷新   " onclick=refresh("service_map.asp")>
	</td></tr>
</table>
</form>
