...
function CheckForm()
{
    ...
    if(!isNumber(input))
    {
    	alert("Invalid VLAN ID, please input numbers");
    	return false;
    }
    var iInput=parseInt(input);
    if(iInput<1 || iInput>4094)
    {
        alert ("Invalid VLAN ID, please input integer from 1 to 4094！");
        return false;
    }
    
     if(vlan_mode() == '0' && isAnyVlanNotExist(input, vlan_ids))
     {
 			alert("VLAN " + iInput + " do not exist in VLAN table. Please input again.");
 			return;	
    }
    
    if (getCfg('MVID') != iInput){
	    if(get_linktype('eth0') == 'access') { 
	        if(!confirm('PVID of Port ' + port_name('eth0') + ' will be changed to ' + iInput + ' , Confirm?'))
	            return;
	    }
	    if(get_linktype('eth1') == 'access') { 
	        if(!confirm('PVID of Port ' + port_name('eth1') + ' will be changed to ' + iInput + ' , Confirm?'))
	            return;
	    }
...
<table width=100% border=0 align=center cellpadding=0 cellspacing=0
	...
	<tr>
		...
		<table width=100% border=0 align=center cellpadding=5 cellspacing=1
			bgcolor="#FFFFFF">
			<tr>
				<td class=bgblue>Management VLAN ID</td>
			...
			<tr>
				...
				<td class=bggrey>
					...
					&nbsp;&nbsp;&nbsp;Port: <input name="MSSH_PORT" size=10 maxlength=10></input>
					&nbsp;&nbsp;<!--(1 - 65535, 默认端口: 22)-->
				</td>
			</tr>
			<tr>
				...
				<td class=bggrey>
					...
					&nbsp;&nbsp;&nbsp;Port: <input name="MTEL_PORT" size=10 maxlength=10></input>
					&nbsp;&nbsp;<!--(1 - 65535, 默认端口: 23)-->
				</td>
			</tr>
		</table>
