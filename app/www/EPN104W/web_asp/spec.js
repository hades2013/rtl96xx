...
,new Page(2,'工作状态信息', 'dev_status.asp', '工作状态信息', dev_status_desc, '', 1)
,new Page(2,'工作状态信息', 'dev_status.asp', '工作状态信息', dev_status_desc, '')
...
,new Page(2,'无线设置','wlan_ap_basic.asp', '无线设置','', '')
,new Page(2,'无线安全','wlan_ap_ssid_config.asp', '无线设置', '', '', 2)
,new Page(2,'接入控制','wlan_ap_acl.asp', '无线设置', '', '', 2)
,new Page(2,'高级设置', 'wlan_ap_advance.asp', '无线设置', '', '', 2)
,new Page(2,'无线用户列表', 'wlan_ap_client.asp', '无线设置', '', '', 2)
,new Page(2,'环路检测', 'loopback_detection.asp', '环路检测', loop_dec, '',2)
,new Page(2,'环路检测', 'loopback_detection.asp', '环路检测', loop_dec, '')
,new Page(2,'系统日志','system_log_set.asp', ' 系统日志', systemlog_desc, '')
,new Page(2,'system_log','system_log.asp', '日志信息','', '', 2)
,new Page(0,'setdebug','setdebug.asp', 'SETDEBUG','', '', 2)
,new Page(0,'debug','debug.asp', 'DEBUG','', '', 2)
/*
...
//	m += '<td valign=top><img src=images/cat_advancedsetup.gif border=0>';
	...
	m += '<tr>';
	m += '<td></td>';
	m += '<td><input class=btn_o type=button value="退出" onclick="IsLogout();"></td>';
	m += '<td></td>';
	m += '</tr>';
	m += '</table>';
	m += '</td></tr>';
...
function pageHead(PROD_MODEL)
{
	var v = new Array(PROD_MODEL);
	pageHead(v);
}	

function pageHead(webVars,PROD_MODEL)
{
...
function pageTail(str_copyright)
{
var m = rightTail();
...
function enalbeElementByid(id, action)
{
	if(id=='')
	{
		alert("ID is null!");
	...
	document.getElementById(id).disabled=action==1?false:true;
}
