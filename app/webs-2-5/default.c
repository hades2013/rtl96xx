...
#include	<ipc_client.h>/*add by zhouguanhua 2012/9/29 of EPN104QID0065*/
#include	"utils.h"
#include    <systemlog.h>		//add by leijinbao 2013/11/21 for USER_LOG for 281
...
userInfo_t websUser[MAX_USER_NUM];

#include <str_utils.h>

void websAuthInit(void)
{
	memset(websUser, 0, sizeof(websUser));
}

/*Begin add by huangmingjian 2013-08-22*/

#ifdef CONFIG_PRODUCT_5500
#include <memshare.h>

ponStatus_webDis_t * gm_ponStatus = NULL;
void shareMemeInit(void)
{
	gm_ponStatus = ponStatus_webDis_shmInit();
}
/*End add by huangmingjian 2013-08-22*/
...
void websAuthDelUser(uint32_t ipaddr)
{
	int i;
	for(i = 0; i < MAX_USER_NUM; i ++){
		if (websUser[i].ip_addr == ipaddr){
			websUser[i].logout_time = 0;
			return ;
		}
	}
}
...
void websAuthAddUser(char *name, char *pwd, uint32_t ipaddr, uint32_t timeout)
{
	int i, older_id = 0;
	uint32_t t = time(NULL);
	uint32_t older = websUser[0].logout_time;
	
	for(i = 0; i < MAX_USER_NUM; i ++){
		if (websUser[i].ip_addr == ipaddr){
			websUser[i].timeout = timeout;
			websUser[i].logout_time = t + websUser[i].timeout;
			safe_strncpy(websUser[i].name, name, sizeof(websUser[i].name));			
			safe_strncpy(websUser[i].password, pwd, sizeof(websUser[i].password));
			return ;
		}
		if (websUser[i].logout_time < older){
			older = websUser[i].logout_time;
			older_id = i;
		}
	}
	// replace the older one
	websUser[older_id].ip_addr = ipaddr;
	websUser[older_id].timeout = timeout;
	websUser[older_id].logout_time = t + websUser[older_id].timeout;
	safe_strncpy(websUser[older_id].name, name, sizeof(websUser[older_id].name)); 		
	safe_strncpy(websUser[older_id].password, pwd, sizeof(websUser[older_id].password));
}


extern int asp_get_config_file_ready(void);
/*begin modify by zhouguanhua 2012/9/29 of EPN104QID0065*/
extern int ipc_fd;
/*end modify by zhouguanhua 2012/9/29 of EPN104QID0065*/	
...
/*****************************************************************
  ...
  Description:    user_is_logined
  Author: 	         huangmingjian
  Date:   		  2013/11/29
  INPUT:           
  OUTPUT:         
  Return:   	0:user logout, 1:user login   
  Others:	       
*****************************************************************/

int user_is_logined(char *ipaddr)
{
	uint32_t host = 0;
	int i = 0;
	struct in_addr in;
...
/*end modify by zhouguanhua 2012/9/29 of EPN104QID0065*/		
	...
	if (strstr(lpath, ".js") != NULL ||
		strstr(lpath, ".jpg") != NULL ||
		strstr(lpath, ".gif") != NULL ||		
		strstr(lpath, ".bin") != NULL ||
		strstr(lpath, ".config") != NULL ||
		strstr(lpath, ".css") != NULL ||
		strstr(lpath, ".png") != NULL ||
		strstr(lpath, "login.asp") != NULL ) 
	{	
		// passs 
		if (strstr(lpath, "login.asp") != NULL){
		/*add by leijinbao for 281*/	
			for(i = 0; i < MAX_USER_NUM; i ++)
			{
				if((websUser[i].ip_addr == host)&&(1 != log_msg))
				{
					USER_LOG(LangId,WEB,LOG_NOTICE,M_LOGOUT,"",wp->ipaddr);   //add by leijinbao 2013/11/21 for 281
				}
			}
		/*end by leijinbao for 281*/	
			websAuthDelUser(host);
		}

        /*begin modify by liaohongjun 2012/8/1 of QID0003*/
        sprintf(cfgName, "%s.config", PRODUCT_NAME);
		if (strstr(lpath, cfgName) != NULL){
		//if (strstr(lpath, "config.bin") != NULL){
        /*end modify by liaohongjun 2012/8/1 of QID0003*/
			if (!asp_get_config_file_ready()){
				websRedirect(wp, "/do_cmd.asp?DELAY=0&ERR=1&MSG=fail&GO=index.asp");
			}
			
			/*Begin add by huangmingjian 2013/11/29 for Bug 283*/
			if(!user_is_logined(wp->ipaddr))	
			{
				return 0;
			}
			/*End add by huangmingjian 2013/11/29 for Bug 283*/
		}
        /*Begin add by huangmingjian 2013/11/29 for Bug 283*/
        sprintf(cfgName, "%s-syslog.bin", PRODUCT_NAME);
		if (strstr(lpath, cfgName) != NULL)
		{
			if(!user_is_logined(wp->ipaddr))	
			{
	...
	if(pack && pack->hdr.status == IPC_STATUS_OK) {
        for(i = 0; i < MAX_USER_NUM; i ++)
        {
            if ((websUser[i].ip_addr == host)
                && (websUser[i].logout_time > t)) 
            {
                websUser[i].timeout = pack->admin.idle_time;
            }
        }
    }
	...
	ipc_sys_time_ack_t *time_pack = NULL; 
 	if(is_timepage_post)
	{   
		is_timepage_post = 0;
	    time_pack = ipc_sys_time(ipc_fd, IPC_CONFIG_GET, IPC_APPLY_NONE, NULL);
 		if(time_pack && time_pack->hdr.status == IPC_STATUS_OK) 
 		{
			for(i = 0; i < MAX_USER_NUM; i ++)
			{
				if ((websUser[i].ip_addr == host) && (websUser[i].logout_time > time_pack->time.time_before_change))
				{
					websUser[i].logout_time = t + websUser[i].timeout;
					if(time_pack) 
					{
	...
	/*End add by huangmingjian 2013/9/06 for logout when setting time on web*/
