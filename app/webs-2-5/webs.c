...
static char_t	websLogname[64] = T("/tmp/log.txt");	/* Log filename */
static int 		websLogFd;						/* Log file handle */
#endif

#ifdef WEBS_TRACE_SUPPORT
static char_t	websTracename[64] = T("/tmp/trace.txt");	/* Log filename */
...
*/
                ...
                if(!strcmp(wp->url,"/goform/upldForm")&&(wp->clen*10 > MAX_UPLOAD_FILE_LEN)){
                    wp->flags |= WEBS_POST_CLEN_OVERFLOW;
                }else{
...
/*****************************************************************
  Function:        websWriteSafe
  Description:    websWriteSafe
  Author: 	          huangmingjian
  Date:   		   2013/10/09
  INPUT:           
  OUTPUT:         
  Return:   	   
  Others:	         support max string length:1024*5, can fix if necessary  
*****************************************************************/

int websWriteSafe(webs_t wp, char *str, int len)
{
	char buf[1024] = {0};
	char *ptr = NULL;
	unsigned int sd_len = WEBS_BUFSIZE-1;
	int ret = 0;
	int ack_len = 0;
	...
	if(!str)
	{	
		trace(0, T("webs: str is null\n"));
		return -1;
	}
	if(len>1024*5)
	{
		trace(0, T("webs: len is too long\n"));
		return -1;
	}
	...
	while(len>0)
	{
		strncpy(buf, ptr, sd_len);
		if((ret = websWrite(wp, T("%s"), buf))<0)
		{
			return -1;
		}
		ack_len += ret;
