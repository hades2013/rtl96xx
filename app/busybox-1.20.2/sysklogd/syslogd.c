...
typedef struct {
	...
	unsigned long int remotePort;    //add by leijinbao 2013/09/09
} remoteHost_t;
#endif
...
#define GLOBALS \
	...
	int logLevel;                           \
	/*modify by zhouguanhua 2013 6/4*/      \
	int sendlogLevel;                       \  
...
#endif
	.logLevel = 8,
	.sendlogLevel = 8,/*modify by zhouguanhua*/
#if ENABLE_FEATURE_ROTATE_LOGFILE
...
enum {
	...
	OPTBIT_outfile, // -O
	OPTBIT_loglevel, // -l
	OPTBIT_sendloglevel, //r  //add by zhouguanhua for send message to remote level
	OPTBIT_small, // -S
	...
	OPT_loglevel    = 1 << OPTBIT_loglevel, /*modify by zhouguanhua*/
	OPT_sendloglevel    = 1 << OPTBIT_sendloglevel,
...
#define OPTION_STR "m:nO:l:r:S" \  
	...
	/*modify by zhouguanhua  add *opt_r */
	...
	while (config_read(parser, tok, 3, 2, "# \t", PARSE_NORMAL | PARSE_MIN_DIE)) {
		...
		if (s)
		{
			*s++ = '\0';
			...
			if (str)
				*str++ = '\0'; 
			rh->remotePort = strtoul(str, NULL, 10);
			llist_add_to(&remoteAddrList, (void *)s);
			//safe_strncpy(llist_pop(&remoteAddrList), s, strlen(s));
...
static void log_locally(time_t now, char *msg, logFile_t *log_file)
{
	...
	/*Begin add by huangmingjian 2014-02-20 for Bug 338 */
	char buf[512] = {0};
	if(!msg && !log_file)
	{
	...
	if(log_file->path && strcmp(DEV_CONSOLE, log_file->path)==0)
	{
		sprintf(buf, "%s%s", "\n", msg);
		memset(msg, 0, len);
		strcpy(msg, buf);
	}
	/*End add by huangmingjian 2014-02-20 for Bug 338 */
...
/*add by zhouuguanhua for selet send date  of pri to remote */
int is_remote_pri(char *tmpbuf, int len)
{
	char *p = tmpbuf;
    int send_flag=0;
	...
	while (p < tmpbuf) {
		...
		*q = '\0';
/*edit by leijinbao for send log Level 2013/10/20*/		
		logRule_t *ruleSend;
		uint8_t facility = LOG_FAC(pri);
		uint8_t prio_bit = 1 << LOG_PRI(pri);
	
       // if (LOG_PRI(pri) < G.sendlogLevel)
       for (ruleSend = G.log_rules; ruleSend; ruleSend = ruleSend->next) {
	       if (ruleSend->enabled_facility_priomap[facility] & prio_bit)
	        {
				send_flag=1;   //send date to remote   
	        }
	        else
	        {        
				send_flag=0; 
	...
	return send_flag;
/*end by leijinbao 2013/10/20  for send log Level*/	
}
/*end add by zhouuguanhua for selet send date  of pri to remote */
...
//	timestamp_and_log_internal("syslogd started: BusyBox v" BB_VER);
 ...
 read_again:
		...
		for (item = G.remoteHosts; item != NULL; item = item->link) {
            ...
            /*add by zhouguanhua 2013/5/20 */
            recvbuf[sz] = '\0'; /* ensure it *is* NUL terminated */
            if(is_remote_pri(recvbuf, sz)!=1)
            {
                continue;
            }
            recvbuf[sz] = '\n';
            /*end add by zhouguanhua 2013/5/20 */
...
int syslogd_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 /*add by zhouguanhua  send remote level*/   
    if (opts & OPT_sendloglevel) // -r
        G.sendlogLevel = xatou_range(opt_r, 1, 8);
 /*add by zhouguanhua  send remote level*/ 
	//if (opts & OPT_small) // -S
#if ENABLE_FEATURE_ROTATE_LOGFILE
