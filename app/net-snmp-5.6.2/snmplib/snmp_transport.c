...
unsigned int trap_dest_port = SNMP_TRAP_PORT;
unsigned long trap_ip_addr = 0;
#endif
...
/*Begin add by linguobin 2014-05-21 for bug606*/
#if defined(CONFIG_ZBL_SNMP)

int get_trap_remote_addr(char *host, unsigned long *trap_ip)
{
	char *cp = NULL;
	unsigned int port = -1;
	char *p = NULL;
	p = host;
    if ((p != NULL) && (trap_ip != NULL)) 
	{	
		if ((cp = strtok(p, ":")) != NULL)
		{
			*trap_ip = inet_addr(cp);
		}
...
#endif
/*End add by linguobin 2014-05-21 for bug606*/
/*
                               ...
                               const char *default_target)
{
	...
	/*Begin add by linguobin 2014-05-21 for bug606*/
	#if defined(CONFIG_ZBL_SNMP)
	if(str && 0!=strcmp(str, ""))/*It is trap host: ip:port*/
	{
		trap_dest_port = get_trap_remote_addr(str,&trap_ip_addr);
	}
	#endif
	/*End add by linguobin 2014-05-21 for bug606*/
    for(;;) {
