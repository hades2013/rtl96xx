...
register_prenetsnmp_mib_handler(const char *type,
                                ...
                                void (*releaser) (void), const char *help)
{	
	/*Begin modified by huangmingjian 2012/12/20 for EPN104QID0089:snmp*/ 
	#if 0
    return internal_register_config_handler(type, token, parser, releaser,
					    help, PREMIB_CONFIG);
	#endif
	 return internal_register_config_handler(type, token, parser, releaser, help, NORMAL_CONFIG);
	/*End modified by huangmingjian 2012/12/20 for EPN104QID0089:snmp*/ 
                                    ...
                                    size_t * len)
{
    ...
    if (strncasecmp(readfrom, "0x", 2) == 0) {
        ...
        *len = ilen;
         ...
         */
       /*Begin add by huangmingjian 2013/02/02 */ 
  		SNMP_FREE(*str);
	   /*End add by huangmingjian 2013/02/02 */ 
        if (*str == NULL) {
