...
#include <lw_config.h>  //Begin add by linguobin 2014-05-07 for get acer products
...
netsnmp_udpipv4base_transport(struct sockaddr_in *addr, int local)
{
    ...
    if (local) {
      ...
      t->local = (u_char *) malloc(6);
         ...
         */
		/*Begin add by linguobin 2014-05-27*/ 
		/*浙广网管ip搜索功能要求trap发送广播报文*/
		#if defined(CONFIG_ZBL_SNMP)			
			int broadcastFlag = 1;
			if (setsockopt(t->sock, SOL_SOCKET, SO_BROADCAST,
					(char *) &broadcastFlag, sizeof(broadcastFlag)) < 0) {
				return -1;
			}	
		#endif
		/*End add by linguobin 2014-05-27*/ 
