...
/*****************************************************************
    Function:v3_ip_check
    Description:check v3 ip between "from" and "com2SecList"
...
int v3_ip_check(void *opaque)
{
    const com2SecEntry *c= NULL;
	char buf0[INET_ADDRSTRLEN] = {0};
	...
	for (c = com2SecList; c != NULL; c = c->next) 
	{
		DEBUGMSGTL(("netsnmp_udp_getSecName","v3#: <\"from:%s, community:%s\", config:%s/%s>",
						   inet_ntoa(from->sin_addr), 
						   c->community,
						   inet_ntop(AF_INET, &c->network, buf1, sizeof(buf1)),
						   inet_ntop(AF_INET, &c->mask, buf2, sizeof(buf2))));
		if((from->sin_addr.s_addr & c->mask) == c->network) 
		{	
			 DEBUGMSG(("netsnmp_udp_getSecName", "... SUCCESS\n"));
			 return 1;
		}
