                     ...
                     void **opaque, int *olength)
{
	...
	/*Begin add by huangmingjian 2013/01/09 for EPN104QID0089:snmp*/ 
	DEBUGMSGTL(("netsnmp_udp", "recvfrom: %d, engineMaxMessageSize:%d\n",rc, engineMaxMessageSize));
	if(rc > engineMaxMessageSize)
	{
		DEBUGMSGTL(("netsnmp_udp", "error:recvfrom size too big!\n"));
		return -1;

	}
	/*End add by huangmingjian 2013/01/09 for EPN104QID0089:snmp*/ 
...
#if defined(CONFIG_ZBL_SNMP)
extern unsigned int trap_dest_port;
extern unsigned long trap_ip_addr;
#endif
                     ...
                     void **opaque, int *olength)
{
	...
	/*Begin add by huangmingjian 2013/01/09 for EPN104QID0089:snmp*/ 
	DEBUGMSGTL(("netsnmp_udp", "sendto: %d, engineMaxMessageSize:%d\n",size, engineMaxMessageSize));
	if(size > engineMaxMessageSize)
	{
		DEBUGMSGTL(("netsnmp_udp", "error:send size too big!\n"));
		return -1;
	...
	/*判断发送报文是否为回应节点1.3.6.1.4.1.39596.2.2.1的trap 报文*/
	if((0xa4 == tmp[14]) && (0x82 == tmp[24]) && (0xb5 == tmp[25]) && (0x2c == tmp[26]))
	{
		addr_pair->remote_addr.sin_port = remain_port;/*修改端口号使发送端口号与接收报文端口号一致*/
		addr_pair->remote_addr.sin_addr.s_addr = inet_addr("255.255.255.255");
		to = (struct sockaddr *) &(addr_pair->remote_addr);
	}
	else if((0xa4 == tmp[14]) && (0x81 == tmp[24]) && (0x88 == tmp[25]) && (0x01 == tmp[26]))
	{
		addr_pair->remote_addr.sin_port = trap_dest_port;
		addr_pair->remote_addr.sin_addr.s_addr = trap_ip_addr;
		to = (struct sockaddr *) &(addr_pair->remote_addr);
	}
	else
	{
