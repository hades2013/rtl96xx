...
u_long   snmpVersion = 9;
u_long   engineMaxMessageSize = 1500;
...
free_engineID(int majorid, int minorid, void *serverarg,
	      void *clientarg)
{
    DEBUGMSGTL(("snmpv3", "free engineID callback called\n"));
    SNMP_FREE(engineID);
...
/*****************************************************************
    Function:snmpVersion_conf
    Description:snmpVersion_conf
...
void snmpVersion_conf(const char *word, char *cptr)
{
    snmpVersion = atoi(cptr);
    DEBUGMSGTL(("snmpv3", "snmpVersion: %lu\n", snmpVersion));	
	return;
}

/*****************************************************************
    Function:engineMaxMessageSize_conf
    Description:engineMaxMessageSize_conf
    Author:huangmingjian
    Date:2012/08/01
    Input:  
    Output:         
...
void engineMaxMessageSize_conf(const char *word, char *cptr)
{
	engineMaxMessageSize = atoi(cptr);
	DEBUGMSGTL(("snmpv3", "engineMaxMessageSize: %lu\n", engineMaxMessageSize));	
	return;
}
...
init_snmpv3(const char *type)
{
  ...
  clockticks = sysconf(_SC_CLK_TCK);
	...
	register_config_handler(type, "snmpVersion", snmpVersion_conf, NULL, NULL);	
    register_config_handler(type, "engineMaxMessageSize", engineMaxMessageSize_conf, NULL, NULL);	
