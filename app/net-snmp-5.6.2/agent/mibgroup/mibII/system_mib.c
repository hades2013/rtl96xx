...
/*****************************************************************
    Function:sysNameGet
    Description: get the sysName via mib browser set the globle value "sysName"
...
init_system_mib(void)
{
	...
	char *nodename = CONFIG_DEFAULT_HOSTNAME;
	if(strstr(CONFIG_DEFAULT_HOSTNAME, "EPN104W"))
	{
		nodename = 	CONFIG_DEFAULT_HOSTNAME"HG";
	}

    uname(&utsName);
    snprintf(version_descr, sizeof(version_descr),
            "%s %s %s %s %s", utsName.sysname,
            nodename, utsName.release, utsName.version,
            utsName.machine);
...
#endif
#endif
	/*Modified add by huangmingjian 2013-02-26 for adding get_sysName */ 
	sysNameGet();
	/*End modified by huangmingjian 2013-02-26 for adding get_sysName*/ 
