...
#endif
	(void)shmdt((void *)g_snmp_data);/*Add by huangmingjian 2013/01/15 for EPN104QID0089:snmp*/
    netsnmp_running = 0;
#ifdef WIN32SERVICE
    /*
...
RETSIGTYPE SnmpdReadShmAndProcess(int a)
{	
	char ret = 0;

	if(MASTER_TO_SNMP_GET == g_snmp_data->access)
	{
	}
	
	else if(MASTER_TO_SNMP_SET == g_snmp_data->access)
	{
		switch(g_snmp_data->type)
		{

		    case REREAD_CONFIG:				 
				 DEBUGMSGTL(("netsnmp_udp","REREAD_CONFIG:%d\n", SIGUSR2, g_snmp_data->type));
				 reconfig = 1;
				 break;
				 
			case SEND_IP_CHANGE_TRAP:
				 DEBUGMSGTL(("netsnmp_udp","signal SIGUSR2:%d data is:%s\n", SIGUSR2, g_snmp_data->data));
				send_iptrap(g_snmp_data->data);
		
				break; 
			default:			
				 DEBUGMSGTL(("netsnmp_udp","signal type error:%d\n", g_snmp_data->type));
				 break; 
		}
	}
	else
	{
		DEBUGMSGTL(("netsnmp_udp","signal access error:%d\n", g_snmp_data->access));
	}
	return;
}
static void
...
#endif
{
	...
	g_snmp_data = snmp_data_shmInit();/*Add by huangmingjian 2013/01/15 for EPN104QID0089:snmp*/ 
    init_snmp_ipc_interface();//add by zhouguanhua 2013/3/8
    /*
...
#ifdef SIGUSR2
    signal(SIGUSR2, SnmpdReadShmAndProcess);
#endif
