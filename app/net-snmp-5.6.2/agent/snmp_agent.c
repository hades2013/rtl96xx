...
#include <lw_config.h>  //Begin add by linguobin 2014-04-24 for setting special oid 1.3.6.1.2.1.1.5 error status and error index
...
/*Begin add by linguobin 2014-04-24 for setting special oid 1.3.6.1.2.1.1.5 error status and error index*/
#if defined(CONFIG_ZBL_SNMP)
int system_name = 0;
void special_oid(netsnmp_pdu *pdu)
{
	int i = 0;
	const oid sysOid[8] =	{1,3,6,1,2,1,1,5};

	if(pdu->variables->name_loc)
	{
		if(0 == memcmp(sysOid,pdu->variables->name_loc,sizeof(sysOid)))
		{	
			if(1 == system_name)
				system_name = 0;
			else
...
#endif
/*End add by linguobin 2014-04-24 for setting special oid 1.3.6.1.2.1.1.5 error status and error index*/
...
handle_var_requests(netsnmp_agent_session *asp)
{
    ...
    for (i = 0; i <= asp->treecache_num; i++) {
        ...
        case MODE_SET_COMMIT:
			...
			/*Begin modified  by huangmingjian 2013-02-28 for EPN204QID0020 */ 
			/*
			...
			*/
	   		retstatus =netsnmp_check_requests_error(asp->treecache[i].requests_begin);
			/*End modified  by huangmingjian 2013-02-28 for EPN204QID0020 */ 
			break;
