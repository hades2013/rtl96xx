...
void vty_event(enum event, int, struct vty *);
...
/*begin added by liaohongjun 2012/8/10*/
void vtysh_welcome_promt(struct vty *vty)
{
    int i = 0;
	char *product_name = NULL;
		
#if defined(PRIV_PRODUCT_NAME_SUPPORT)
	product_name = CONFIG_PRODUCT_NAME_PRIV;
#else
	product_name = PRODUCT_NAME;
#endif
	
    vty_out(vty, "*******************************************************************************%s",VTY_NEWLINE);
    /*Begin modified by liaohongjun for EPN204QID0002 2012/11/19*/
    vty_out(vty, "*    Welcome to use our product of %s",product_name);
    /*End modified by liaohongjun for EPN204QID0002 2012/11/19*/
    vty_out(vty, ".");
    for(i = 0; i < (43 - strlen(product_name)-1); i++)
    {
        vty_out(vty, " ");
    }
    vty_out(vty, "*%s",VTY_NEWLINE);
    vty_out(vty, "*    %s.",DEV_COPYRIGHT_STR);
    for(i = 0; i < (73 - strlen(DEV_COPYRIGHT_STR)-1); i++)
    {
        vty_out(vty, " ");
    }
    vty_out(vty, "*%s",VTY_NEWLINE);    
    vty_out(vty, "*    All rights Reserved.                                                     *%s",VTY_NEWLINE);
    vty_out(vty, "*    No decompiling or reverse-engineering without the owner's permission.    *%s",VTY_NEWLINE);
    vty_out(vty, "*******************************************************************************%s",VTY_NEWLINE);
    //sleep(1);
}
/*end added by liaohongjun 2012/8/10*/
...
static void vty_prompt(struct vty *vty) {
	...
	if(0 != update_prompt_from_web())
	{
		vty_out(vty,"get hostname error!%s", VTY_NEWLINE);
	}
	...
	if (vty->type == VTY_TERM) {
		...
		vty_out(vty, cmd_prompt(vty), hostname);
	}
}
...
vty_new() {
	...
	memset(new, 0, sizeof(struct vty));
...
static void vty_auth(struct vty *vty, char *buf) {
    ...
    switch (vty->node) {
        case AUTH_NODE:
	        //vty->fail = 0;
            if(vty->username) XFREE (MTYPE_HOST, vty->username);
            vty->username = XSTRDUP(MTYPE_HOST, buf);
	        if(0 == strlen(vty->username))
	        {
                vty->node = AUTH_NODE;
                return;
            }    
            vty->node = AUTH_PASSWD_NODE;
            return;
        case AUTH_PASSWD_NODE:
            ...
            if (host.advanced)
                ...
                next_node = VIEW_NODE;
            #endif
            passwd = vty_user[vty->current_no].passwd;
            next_node = VIEW_NODE;
            break;
    ...
    if (passwd){
        if(AUTH_ENABLE_NODE != vty->node){
            //DBG_PRINTF("buf = %s,passwd = %s,vty->username = %s,vty_user[%d].username = %s",buf,passwd,vty->username,\
            //vty->current_no,vty_user[vty->current_no].username);
            if (!strcmp(vty_user[vty->current_no].passwd_type, PASSWD_TYPE_CIPHERTEXT)){
                if((!strcmp(crypt(buf, passwd), passwd)) && (!strcmp(vty->username, vty_user[vty->current_no].username))){
                    fail = 0;
                }else{
                    fail = 1;
                }            
            }else{
                if((!strcmp(buf, passwd)) && (!strcmp(vty->username, vty_user[vty->current_no].username))){
                    fail = 0;
                }else{
                    fail = 1;
                }
    ...
    if (!fail) {     
        vty->fail = 0;
        vty->v_timeout = vty_user[vty->current_no].timeout;
        /*begined added by liaohongjun 2012/9/20 of EPN104QID0023*/
        vty_event(VTY_TIMEOUT_RESET, 0, vty);
        /*end added by liaohongjun 2012/9/20 of EPN104QID0023*/
        /*Begin modified by feihuaxin  for bug ID 78 2013/5/28*/
        /*begin added by liaohongjun 2013/4/22 of bug id 14*/
        vty_user[vty->current_no].login_flag = 1;  
        /*end added by liaohongjun 2013/4/22 of bug id 14*/
        /*End modified by feihuaxin  for bug ID 78 2013/5/28*/
        vty->node = next_node; /* Success ! */
    } else {
        vty->fail++;
        /*begined modified by liaohongjun 2012/9/25 of EPN104QID0061*/
        //DBG_PRINTF("vty->fail = %d",vty->fail);
        if (vty->node == AUTH_PASSWD_NODE) {
            vty_out(vty, "%% Access denied, bad username or passwords!%s",
                VTY_NEWLINE);
...
static int vty_command(struct vty *vty, char *buf) {
	...
	/*begin modify by leijinbao 2013/10/31 for EPN104NQID237*/
	//ret = cmd_execute_command(vline, vty, NULL, 0);
	ret = cmd_execute_command(vline, vty, NULL, 1);
	/*end modify by leijinbao 2013/10/31 for EPN104NQID237*/
...
static void vty_write(struct vty *vty, const char *buf, size_t nbytes) {
	if ((vty->node == AUTH_PASSWD_NODE) || (vty->node == AUTH_ENABLE_NODE) || \
        (ENTER_NODE == vty->node)){
        if((vty->node == AUTH_PASSWD_NODE) || (vty->node == AUTH_ENABLE_NODE))
		{	/*Begin modified by huangmingjian 2012/09/26 for EPN104QID0052*/  
			/*if the key is del or letf arrow or right arrow or the cursor is not on the end,
			   we nolmally output the buf,the buf may be the "*" in the auth node*/
			if((1 == vty->is_del) || (1 == vty->is_letf_and_right) || (vty->cp != vty->length))
           	{
			...
			{	
				buffer_put(vty->obuf, "*", 1);
			}
			/*End modified by huangmingjian 2012/09/26 for EPN104QID0052 */
        }
		return;
	}
...
static void vty_delete_char(struct vty *vty) {
	...
	//if (vty->node == AUTH_NODE || vty->node == AUTH_ENABLE_NODE)
		//return;
	/*Begin add by huangmingjian 2012/09/26 for EPN104QID0052*/  
	/*in auth node,if we don't delete the chars from the end,we need to write "*" to the end */
	if((vty->node == AUTH_PASSWD_NODE) || (vty->node == AUTH_ENABLE_NODE))
	{	if(1 == vty->is_del)
		{	for(i=0; i<(size - 1); i++)
			{	
	...
	/*End add by huangmingjian 2012/09/26 for EPN104QID0052 */
...
static void vty_complete_command(struct vty *vty) {
	...
	struct cmd_element *tmp_element;  
	/*End add by by huangmingjian 2012/09/03 for EPN104QID0018 */
...
/*Begin modified by huangmingjian 2012/09/04 for EPN104QID0018*/ 
		cnode = vector_slot(cmdvec, vty->node);
		tmp_vector = cnode->cmd_vector;
		for(index = 0; index < vector_active (cnode->cmd_vector); index++)
		{	
			tmp_element = tmp_vector->index[index];		
			if(strstr(tmp_element->string, matched[0]))
			{  
				if(tmp_element->attr == CMD_ATTR_HIDDEN)
				{  
...
/*End modified by huangmingjian 2012/09/04 for EPN104QID0018 */
	...
	case CMD_COMPLETE_LIST_MATCH:
		for (i = 0; matched[i] != NULL; i++) 
		{
			...
			{	
				tmp_element = tmp_vector->index[index];
				if(strstr(tmp_element->string, matched[i]))
				{  
					if(tmp_element->attr == CMD_ATTR_HIDDEN)
					{ 
...
/*Begin add by huangmingjian 2012/09/03 for EPN104QID0018*/ 
	struct cmd_node *cnode = NULL;
    struct cmd_element *tmp_element = NULL;
	vector tmp_vector;
    int index = 0;
/*End add by by huangmingjian 2012/09/03 for EPN104QID0018 */	
	vline = cmd_make_strvec(vty->buf);
 ...
 for (i = 0; i < vector_active (describe); i++)
		if ((desc = vector_slot (describe, i)) != NULL) 
		{
			...
			{	
				...
				{   
					tmp_element = tmp_vector->index[index];
					if(strstr(tmp_element->string, desc->cmd))
					{  
						if(tmp_element->attr == CMD_ATTR_HIDDEN)
						{  
							break;
	...
	if ((desc = desc_cr)) {
		if (!desc->str)
			vty_out(vty, "  %-s%s", desc->cmd[0] == '.' ? desc->cmd + 1
					: desc->cmd, VTY_NEWLINE);
		else if (desc_width >= strlen(desc->str))
			vty_out(vty, "  %-*s  %s%s", width, desc->cmd[0] == '.' ? desc->cmd
					+ 1 : desc->cmd, desc->str, VTY_NEWLINE);
		else
			vty_describe_fold(vty, width, desc_width, desc);
	}
...
static int vty_execute(struct vty *vty) {
    ...
    switch (vty->node) {
        ...
        case ENTER_NODE:
            if (!strcmp(vty_user[vty->current_no].authmode, AUTH_MODE_NONE)){
                vty->node = VIEW_NODE;
                /*Begin modified by feihuaxin  for bug ID 78 2013/5/28*/
                /*begin added by liaohongjun 2013/4/22 of bug id 14*/
                if(!strcmp(vty->address, "Console"))
                ...
                vty_out(vty,"%s", VTY_NEWLINE);
                vty->v_timeout = vty_user[vty->current_no].timeout;
                /*begin added by liaohongjun 2012/9/20 of EPN104QID0023*/
                vty_event(VTY_TIMEOUT_RESET, 0, vty);
                /*end added by liaohongjun 2012/9/20 of EPN104QID0023*/
            }else if(!strcmp(vty_user[vty->current_no].authmode, AUTH_MODE_PASSWORD)){
                vty->node = AUTH_NODE;
                vty_out(vty,"%s", VTY_NEWLINE);
                vty->v_timeout = VTY_AUTH_INPUT_TIMEOUT; 
                /*begin added by liaohongjun 2012/9/20 of EPN104QID0023*/
                vty_event(VTY_TIMEOUT_RESET, 0, vty);
                /*end added by liaohongjun 2012/9/20 of EPN104QID0023*/
            }else{
                vty_out(vty,"Unknow vty auth-mode! %s", VTY_NEWLINE);
            }
            break;
...
static int vty_read(struct thread *thread) {
	...
	/*Begin add by huangmingjian 2012/09/26 for EPN104QID0052*/
	vty->is_del = 0;
	vty->is_letf_and_right = 0;
	/*End add by huangmingjian 2012/09/26 for EPN104QID0052*/
	vty->t_read = NULL;
	memset(buf, 0x0, VTY_READ_BUFSIZ);
	...
	{
		/*Begin add by huangmingjian 2012/09/26 for EPN104QID0052*/ 
		/*when it is in the auth node,we mask the flowing cmds.
		 */
		if((vty->node == AUTH_PASSWD_NODE) || (vty->node == AUTH_ENABLE_NODE))
		{	
			if(    (CONTROL('A')==buf[i])  || (CONTROL('B')==buf[i]) || (CONTROL('D')==buf[i]) \
				 || (CONTROL('E')==buf[i]) || (CONTROL('F')==buf[i]) || (CONTROL('K')==buf[i]) \
				 || (CONTROL('T')==buf[i]) || (CONTROL('U')==buf[i]) || (CONTROL('W')==buf[i]) \
				 || (CONTROL('N')==buf[i]) || (CONTROL('P')==buf[i]) || (('\t')==buf[i]) || ((' ')==buf[i]))
			{	
				continue;
			}
		
			/*special keys include HOME,END,PAGE UP ,PAGE DOWN,INSERT,UP ARROW ,DOWN ARROW ,PAUSE,F1~F12,ESC,
			   and so on,they are starting with 0x1b,and nbytes>=3,we should mask them.But we unmask
			   0x435b1b(<-:LEFT ARROW ) and 0x445b1b(->:RIGHT ARROW )
			*/
			if(0x1b == buf[0] )
	 		{
				if(!((0x1b == buf[0]) && (0x5b == buf[1]) && (buf[2]>=0x43) && (buf[2]<=0x44)))
				{	
					break;
				}
			}
        ...
        else
        {
    		...
    		case CONTROL('H'):
    		case 0x7f:
				vty->is_del = 1;/*Add by huangmingjian 2012/09/23 for EPN104QID0052*/ 
    			vty_delete_backward_char(vty);
    			break;
    		case CONTROL('K'):
    			vty_kill_line(vty);
    		...
    		case '\t':
    			...
    			break;
    		case '?':
				if((vty->node == AUTH_PASSWD_NODE) || (vty->node == AUTH_ENABLE_NODE))/*Modified by huangmingjian 2012/09/26 for EPN104QID0052*/ 
	...
	/* Check status. */
	if (vty->status == VTY_CLOSE){
        if(vty->address && !strcmp(vty->address, "Console")){
            /*Reopen CONSOLE vty*/
            vty_close(vty);
            tty_fd = console_init();
...
static int vty_flush(struct thread *thread) {
	...
	if ((vty->lines == 0) || (vty->width == 0))
		...
		flushrc = buffer_flush_window(vty->obuf, vty->fd, vty->width, 1, erase,
				0);
	else
...
static int set_vty_telnet_user(struct vty *vty,int vty_sock)
{
    int vty_index= 0;
    
    for(vty_index = MIN_VTY_USER_NO_OF_TELNET; vty_index < MAX_VTY_COUNT; vty_index++)
    {
        if(!vty_user[vty_index].active){
            vty->current_no = vty_index;
            /*Begin modified by feihuaxin  for bug ID 78 2013/5/28*/
            /*begin modified by liaohongjun 2013/4/22 of bug id 14*/
            vty_user[vty->current_no].active = 1;
            /*begin modified by liaohongjun 2013/4/22 of bug id 14*/
            /*End modified by feihuaxin  for bug ID 78 2013/5/28*/
            vty_user[vty->current_no].vty_sockfd = vty_sock;
            break;
        }
    }
    if(MAX_VTY_COUNT == vty_index){   
        vty_out(vty, "%s%% Support two telnet users Only. %s", VTY_NEWLINE, VTY_NEWLINE);
        vty->status = VTY_CLOSE;   		
        vty_close(vty);
        return 0;                
    }  
...
struct vty *vty_create(int vty_sock, union sockunion *su) {
    ...
    /* Setting up terminal. */
    if (su) {// By Ryan, only needed by telnet connection
        vty_will_echo(vty);
    ...
    /*vty[0] is always for consol,telnet user start at vty[1]*/
    if(su)
    {      
        if(!strcmp(ipc_get_telnet_service(service), ENABLE_STR)){
            vty->address = sockunion_su2str (su); 
            if(!set_vty_telnet_user(vty, vty_sock)){
                return NULL;
            }
        }else{
            vty_out(vty, "%s%% Telnet service is not available.%s", VTY_NEWLINE, VTY_NEWLINE);
            vty->status = VTY_CLOSE; 
            vty_close(vty);
            return NULL;
        }    
        /*telnet user should never reach ENTER_NODE*/
        if (!strcmp(vty_user[vty->current_no].authmode, AUTH_MODE_NONE)){
            vty->node = VIEW_NODE;
            /*Begin modified by feihuaxin  for bug ID 78 2013/5/28*/
            /*begin added by liaohongjun 2013/4/22 of bug id 14*/
		...
		vty->address = strdup("Console");
	   	else
		vty->address = strdup("ssh");
        vty->current_no = VTY_USER_NO_OF_CONSOLE;
        /*Begin modified by feihuaxin  for bug ID 78 2013/5/28*/
        /*begin modified by liaohongjun 2013/4/22 of bug id 14*/
        vty_user[vty->current_no].active = 1;
        /*end modified by liaohongjun 2013/4/22 of bug id 14*/
    ...
    if(!strcmp(vty_user[vty->current_no].authmode, AUTH_MODE_PASSWORD)){
        /* Vty is not available if password isn't set. */
        if (!strcmp(vty_user[vty->current_no].passwd, VTY_PASSWD_NULL) && \
            (VTY_USER_NO_OF_CONSOLE != vty->current_no)){
            vty_out(vty, "%% Vty[%d] password is not set.%s", vty->current_no, VTY_NEWLINE);
            vty->status = VTY_CLOSE;
            vty_close(vty);
            return NULL;
        }
    ...
    if(!strcmp(vty_user[vty->current_no].authmode, AUTH_MODE_PASSWORD)){
        if(VTY_USER_NO_OF_CONSOLE == vty->current_no){			
			if(strcmp(vty->address,"ssh")!=0)
				{
            vty_out(vty, "%sAccess authentication via Console use vty[%d] %s%s", VTY_NEWLINE, vty->current_no,
                VTY_NEWLINE, VTY_NEWLINE);
				}
				
        }else{
            vty_out(vty, "%sAccess authentication via Telnet use vty[%d] %s%s", VTY_NEWLINE, vty->current_no,
                VTY_NEWLINE, VTY_NEWLINE);
        }
    }else{
        if(VTY_USER_NO_OF_CONSOLE == vty->current_no){
			if(strcmp(vty->address,"ssh")!=0)
            vty_out(vty, "%sAccess via Console use vty[%d] succeed %s%s", VTY_NEWLINE, vty->current_no,
                VTY_NEWLINE, VTY_NEWLINE);
        }else{
            vty_out(vty, "%sAccess via Telnet use vty[%d] succeed %s%s", VTY_NEWLINE, vty->current_no,
                VTY_NEWLINE, VTY_NEWLINE);
        }
    }    
...
static int vty_accept(struct thread *thread) {
	...
	if (p->family == AF_INET && vty_accesslist_name) {
		if ((acl = access_list_lookup(AFI_IP, vty_accesslist_name))
			...
			char *buf;
			zlog(NULL, LOG_INFO, "Vty connection refused from %s", (buf
					= sockunion_su2str(&su)));
			free(buf);
			close(vty_sock);
	...
	on = 1;
    /*begin modified by liaohongjun 2012/9/20 of EPN104QID0023*/
	ret = setsockopt(vty_sock, SOL_SOCKET, SO_KEEPALIVE, (char *) &on,
			sizeof(on)); 
    /*end modified by liaohongjun 2012/9/20 of EPN104QID0023*/
	if (ret < 0)
		zlog(NULL, LOG_INFO, "can't set sockopt to vty_sock : %s",
				safe_strerror(errno));

	zlog(NULL, LOG_INFO, "Vty connection from %s", (bufp
			= sockunion_su2str(&su)));
	if (bufp)
...
static void vty_serv_sock_family(const char* addr, unsigned short port,
	...
	if (naddr)
		...
		case -1:
			...
			break;
		case 0:
			zlog_err("error translating address %s: %s", addr, safe_strerror(
					errno));
			naddr = NULL;
		}
...
void vty_close(struct vty *vty) {
    ...
    /*set user unactive*/
	if(vty_user[vty->current_no].active)
    {
        vty_user[vty->current_no].active = 0;
    }
	/* Close socket. */
	if (vty->fd > 0)
		close(vty->fd);
...
static int vty_timeout(struct thread *thread) {
    ...
    /* Clear buffer*/    
    buffer_reset(vty->obuf);
    if((AUTH_NODE == vty->node) || (AUTH_PASSWD_NODE == vty->node))
    {
        if(vty->address && strcmp(vty->address, "Console")) {
            vty_out(vty, "%s%% Input timed out in %d secs %s",VTY_NEWLINE,VTY_AUTH_INPUT_TIMEOUT,\
                VTY_NEWLINE);
        }else{
            vty_out(vty, "%s%% Input timed out in %d secs %s%s",VTY_NEWLINE,VTY_AUTH_INPUT_TIMEOUT, \
                VTY_NEWLINE, VTY_NEWLINE);            
        }
    }
    else
    {    
        if(vty->address && strcmp(vty->address, "Console")) {
            vty_out(vty, "%s%% Vty connection is timed out %s", VTY_NEWLINE, VTY_NEWLINE);
        }else{
            vty_out(vty, "%s%% Vty connection is timed out %s%s", VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE);
        }
    }
    /* Close connection. */
    if(vty->address && strcmp(vty->address, "Console")) {
...
void vty_log_fixed(const char *buf, size_t len) {
	unsigned int i;
	struct iovec iov[2];
       ...
       "Display who is on vty\n")
{
	...
	for (i = 0; i < vector_active (vtyvec); i++)
	{
		if ((v = vector_slot (vtyvec, i)) != NULL)
            /*Begin modified by feihuaxin  for bug ID 78 2013/5/28*/
            /*begin modified by liaohongjun 2013/4/22 of bug id 14*/
            if(vty_user[v->current_no].login_flag)
            {
                #if 0
    			vty_out(vty, "%svty[%d] connected from %s%s", v->config ? "*"
    					: " ", v->current_no, v->address, VTY_NEWLINE); 
                #endif
    			vty_out(vty, "%svty[%d] connected from %s%s",vty->current_no == v->current_no ? " *"
    					: "  ", v->current_no, v->address, VTY_NEWLINE);                 
            }
            /*end modified by liaohongjun 2013/4/22 of bug id 14*/
            /*End modified by feihuaxin  for bug ID 78 2013/5/28*/
...
static int vty_config_write(struct vty *vty) {
	...
	if (vty_ipv6_accesslist_name)
		vty_out(vty, " ipv6 access-class %s%s", vty_ipv6_accesslist_name,
				VTY_NEWLINE);
