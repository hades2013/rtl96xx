...

static const struct message mstr[] = { { MTYPE_THREAD, "thread" }, {
		MTYPE_THREAD_MASTER, "thread_master" }, { MTYPE_VECTOR, "vector" }, {
		MTYPE_VECTOR_INDEX, "vector_index" }, { MTYPE_IF, "interface" }, { 0,
		NULL }, };

...
zerror(const char *fname, int type, size_t size) {
	zlog_err("%s : can't allocate memory for `%s' size %d: %s\n", fname,
			lookup(mstr, type), (int) size, safe_strerror(errno));
		...
		for (m = ml->list; m->index >= 0; m++)
			if (m->index && mstat[m->index].alloc) {
				...
				fprintf(stderr, "%s: memstats:  %-30s: %10ld%s\n", prefix,
						m->format, mstat[m->index].alloc, mstat[m->index].alloc
								< 0 ? " (REPORT THIS BUG!)" : "");
				i = j = 1;
			}
	}
