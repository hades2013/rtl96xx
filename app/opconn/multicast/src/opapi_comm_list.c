...
#define TEST_CL_LIST_DEBUG
#undef TEST_CL_LIST_DEBUG
...
#ifdef TEST_CL_LIST_DEBUG

typedef struct _list_test list_test;
struct _list_test {
  int key;
};
...
int main()
{
  ...
  node2.key = 2;
  node3.key = 3;
  list = clListAppend(list, &node1);
  list = clListAppend(list, &node2);
  list = clListAppend(list, &node3);
  clListPrint(list);
  t = clListNth(list, 0);
  printf("node is %d\n", clListPosition(list, t));
  t = clListNth(list, 1);
  printf("node is %d\n", clListPosition(list, t));
  t = clListNth(list, 0);
  list = clListRemove(list, t->data);
  clListPrint(list);
