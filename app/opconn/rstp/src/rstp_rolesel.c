...
**    MODULE     :  RSTP
**
**    FILE       :  rstp_role_sel.c
**      
**    DESCRIPTION:  This file contains Port Role Selection state machine. 
**    
**
**    NOTES      :  None.
**
...
/* Port Role Selection state machine : 802.1D 2004 Edition*/
#include "rstp_base.h"
#include "rstp_stpm.h"
...
static void
updtRoleDisableTree (STPM_T* this)
{               /* 17.10.20 */
...
updtRootPrio (STATE_MACH_T* this)
{
  ...
  opl_rstp_printf(("Root cost=%d.\n", (int)stpm->rootPrio.root_path_cost));
  ...
  for (port = stpm->ports; port; port = port->next) {
    ...
    if (STP_VECT_compare_vector (&rootPathPrio, &stpm->rootPrio) < 0) {
		 ...
		 opl_rstp_printf(("Port %d rootPathPrio=%d.\n",(int)port->port_id, (int)rootPathPrio.root_path_cost));
		 
      STP_VECT_copy (&stpm->rootPrio, &rootPathPrio);
      STP_copy_times (&stpm->rootTimes, &port->portTimes);
      /* rzhou del dm = (8 +  stpm->rootTimes.MaxAge) / 16;
      if (!dm)
        dm = 1;*/
      dm = 1; //always 1 second, rzhou add 
...
static void
updtRolesTree (STATE_MACH_T* this)
{               /* 17.19.21 */
  register PORT_T* port;
  register STPM_T* stpm;
    ...
    switch (port->infoIs) {
      ...
      case Received:
        if (stpm->rootPortId == port->port_id)
		{
		  ...
		  if (0 != stpm->rootTimes.MessageAge)
	  	  {
	  	  	port->portTimes.MessageAge = stpm->rootTimes.MessageAge - 1;
		  }
...
STP_rolesel_enter_state (STATE_MACH_T* this)
{
  ...
  switch (this->State) {
    ...
    case INIT_BRIDGE:
      ...
      break;
    case ROLE_SELECTION:
      OP_DEBUG(DEBUG_LEVEL_DEBUGGING, " case ROLE_SELECTION \n");
