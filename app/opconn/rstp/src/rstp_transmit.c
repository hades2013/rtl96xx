...
**    MODULE     :  RSTP
**
**    FILE       :  rstp_transmit.c
**      
**    DESCRIPTION:  This file contains Port Transmit state machine : 802.1D 2004 Edition.   
**    
**
**    NOTES      :  None.
**
...
#include "rstp_base.h"
#include "rstp_stpm.h"
#include "rstp_to.h" /* for STP_OUT_get_port_mac & STP_OUT_tx_bpdu */
...
STP_transmit_enter_state (STATE_MACH_T* this)
{
  ...
  switch (this->State) {
    case BEGIN:
    case TRANSMIT_INIT:
      // rzhou to be verified below port->newInfo = True;
      port->newInfo = False;
      port->txCount = 0;
      break;
    case TRANSMIT_PERIODIC:
      port->newInfo = port->newInfo ||(port->role == DesignatedPort) ||((port->role == RootPort) && (port->tcWhile!=0));
      //rzhou note to be verified!!!
      //                      ((port->role == DesignatedPort) ||
      //                       ((port->role == RootPort) && port->tcWhile));
      break;
    case IDLE:
    	//rzhou note
    	port->helloWhen = stpm->rootTimes.HelloTime;
      break;
    case TRANSMIT_CONFIG:
      port->newInfo = False;
      OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "txConfig \n");
      txConfig (this);
      port->txCount++;
      port->tcAck = False;
      break;
    case TRANSMIT_TCN:
      port->newInfo = False;
      txTcn (this);
      OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "txTcn \n");
      port->txCount++;
      break;
    case TRANSMIT_RSTP:
      port->newInfo = False;
...
STP_transmit_check_conditions (STATE_MACH_T* this)
{
  ...
  switch (this->State) {
    ...
    case IDLE:
      ...
      if (!port->sendRSTP && port->newInfo && (port->txCount < TxHoldCount) && (port->role == DesignatedPort) && port->helloWhen)
        return STP_hop_2_state (this, TRANSMIT_CONFIG);
      //rzhou to be verified  port->role == ???  
      if (!port->sendRSTP && port->newInfo && (port->txCount < TxHoldCount) && (port->role == RootPort) && port->helloWhen)
        return STP_hop_2_state (this, TRANSMIT_TCN);
        
      if (port->sendRSTP && port->newInfo && (port->txCount < TxHoldCount) && port->helloWhen)
      {
        if ((port->role == DesignatedPort) || (port->role == RootPort))
        {
