...
**    FILE       :  rstp_sttrans.c
**      
**    DESCRIPTION:  This file contains Port State Transition state machine:802.1D  
**    2004 Edition.
**
**    NOTES      :  None.
**
...
STP_sttrans_enter_state (STATE_MACH_T *this)
{
  ...
  switch (this->State) {
    ...
    case DISCARDING:
	  ...
	  if ((port->role == AlternatePort) || (port->role == BackupPort))
  	  {
	    rstpwarningstate[port->port_index-1] = 1;
	  }
#ifdef STRONGLY_SPEC_802_1W
    ...
    case FORWARDING:
	  ...
	  if (((port->role == RootPort) || (port->role == DesignatedPort)) 
	  	&& (rstpwarningstate[port->port_index-1] == 1))
  	  {
            rstpwarningstate[port->port_index-1] = 0;
	  }
