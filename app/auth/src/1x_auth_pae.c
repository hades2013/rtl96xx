				...
				u_char * dev_svr, u_char * dev_supp)
{
	...
	//----endif
	{
		...
		#if 1
		/*xl_yue:20091021 set udpsock_connected true  only when connect succeed*/
		if(lib1x_nal_connect( dev_txrx->network_svr,  & dev_txrx->radsvraddr, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_AUTH) < 0)
		{
			printf("udpsock connect failed!\n");
		}else{
			dev_txrx->network_svr->udpsock_connected = 1;
		}
		/*xl_yue:20091021 end*/
		#else
		if(lib1x_nal_connect( dev_txrx->network_svr,  & dev_txrx->radsvraddr, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_AUTH) < 0)
