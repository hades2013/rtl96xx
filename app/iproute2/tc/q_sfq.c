...
{
	fprintf(stderr, "Usage: ... sfq [ limit NUMBER ] [ perturb SECS ] [ quantum BYTES ] [hash {original | ctorigsrcport}]\n");
}
...
static int sfq_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "quantum") == 0) {
			...
			ok++;
                        /* begin: add by c03628 for SHD12651, 2010/11/12 */
		} else if (strcmp(*argv, "hash") == 0) {
			NEXT_ARG();
                        if (strcmp(*argv, "original") == 0) {
                            opt.hash_type = 0;
                        }
                        else if (strcmp(*argv, "ctorigsrcport") == 0) {
                            opt.hash_type = 1;
                        }
                        else {
                            fprintf(stderr, "Illegal \"hash\", must be {original | ctorigsrcport}\n");
                            return -1;
                        }
			ok++;
                        /* end: add by c03628 for SHD12651, 2010/11/12 */
...
static int sfq_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	...
	if (qopt->perturb_period)
        ...
        /* begin: add by c03628 for SHD12651, 2010/11/12 */
        switch (qopt->hash_type ) {
            case 1:
                fprintf(f, "ctorigsrcport ");
                break;
