...
static void explain(void)
{
	...
	fprintf(stderr, "       SAMPLE := { ip | ip6 | udp | tcp | icmp | u{32|16|8} | mark | cnsrc | cnsport } SAMPLE_ARGS [divisor DIVISOR]\n");
...
static int parse_cnsrc(int *argc_p, char ***argv_p, struct nlmsghdr *n)
{
	int res = -1;
	int argc = *argc_p;
	char **argv = *argv_p;
	...
	cnsrc.val = addr.data[0];
	cnsrc.mask = mask;
    cnsrc.en = 1;	/* add by z03241 for SHD10277, 2009-09-17 */

//	printf ("cnsrc.val= %x ,mask= %x",cnsrc.val,cnsrc.mask);

	addattr_l(n, MAX_MSG, TCA_U32_CNSRC, &cnsrc, sizeof(cnsrc));
	res = 0;
...
static int parse_cnsport(int *argc_p, char ***argv_p, struct nlmsghdr *n)
{
	int res = -1;
	int argc = *argc_p;
	char **argv = *argv_p;
	struct tc_u32_cnsport cnsport;

	if (argc <= 1)
		return -1;

	if (get_u16(&cnsport.val, *argv, 0)) {
		fprintf(stderr, "Illegal \"cnsport\" value\n");
		return -1;
	}
	NEXT_ARG();

	if (get_u16(&cnsport.mask, *argv, 0)) {
		fprintf(stderr, "Illegal \"cnsport\" mask\n");
		return -1;
	}
	NEXT_ARG();

//	printf("DBG cnsport.val=%u,cnsport.mask=%u,after and is %u\n",cnsport.val,cnsport.mask,cnsport.val & cnsport.mask);
	if ((cnsport.val & cnsport.mask) != cnsport.val) {
		fprintf(stderr, "Illegal \"cnsport\" (impossible combination)\n");
		return -1;
	}

    cnsport.en = 1;		/* add by z03241 for SHD10277, 2009-09-17 */
	addattr_l(n, MAX_MSG, TCA_U32_CNSPORT, &cnsport, sizeof(cnsport));
	res = 0;
...
/* Added by z03241 for SHD10358, 2009/10/9 */
static int parse_pktlen(int *argc_p, char ***argv_p, struct nlmsghdr *n)
{
	int res = -1;
	int argc = *argc_p;
	char **argv = *argv_p;
	struct tc_u32_pktlen pktlen;

	if (argc <= 1)
		return -1;

	if (get_u16(&pktlen.val, *argv, 0)) {
		fprintf(stderr, "Illegal \"pktlen\" value\n");
		return -1;
	}
	NEXT_ARG();

	if (get_u16(&pktlen.mask, *argv, 0)) {
		fprintf(stderr, "Illegal \"pktlen\" mask\n");
		return -1;
	}
	NEXT_ARG();

	if ((pktlen.val & pktlen.mask) != pktlen.val) {
		fprintf(stderr, "Illegal \"pktlen\" (impossible combination)\n");
		return -1;
	}

    pktlen.en = 1;
	addattr_l(n, MAX_MSG, TCA_U32_PKTLEN, &pktlen, sizeof(pktlen));
	res = 0;
...
static int parse_selector(int *argc_p, char ***argv_p, struct tc_u32_sel *sel, struct nlmsghdr *n)
{
	...
	if (matches(*argv, "cnsrc") == 0) {
		NEXT_ARG();
		res = parse_cnsrc(&argc, &argv, n);
		goto done;
	}
	if (matches(*argv, "cnsport") == 0) {
		NEXT_ARG();
		res = parse_cnsport(&argc, &argv, n);
		goto done;
	}
    /* Added by z03241 for SHD10358, 2009/10/9 */
    if (matches(*argv, "pktlen") == 0) {
        NEXT_ARG();
        res = parse_pktlen(&argc, &argv, n);
        goto done;
    }
    /* End for SHD10358 */
...
static int u32_parse_opt(struct filter_util *qu, char *handle, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
        ...
        } else if (strcmp(*argv, "outdev") == 0) {
            char outd[IFNAMSIZ + 1];
            memset(outd, 0, sizeof (outd));
            argc--;
            argv++;
            if (argc < 1) {
                fprintf(stderr, "Illegal outdev\n");
                return -1;
            }
            strncpy(outd, *argv, sizeof (outd) - 1);
            addattr_l(n, MAX_MSG, TCA_U32_OUTDEV, outd, strlen(outd) + 1);
...
static int u32_print_opt(struct filter_util *qu, FILE *f, struct rtattr *opt, __u32 handle)
{
	...
	/* added by shidongxue for ‘¥IP∆•≈‰*/

	if (tb[TCA_U32_CNSRC]) {

		struct tc_u32_cnsrc *cnsrc = RTA_DATA(tb[TCA_U32_CNSRC]);
		
		if (RTA_PAYLOAD(tb[TCA_U32_CNSRC]) < sizeof(*cnsrc)) {
			fprintf(f, "\n  Invalid cnsrc payload len (kernel&iproute2 mismatch)\n");
		} 
		else 
		{
    	    char ip[32];
		    memset(ip,0,sizeof(ip));
            if(NULL!=inet_ntop(AF_INET,&cnsrc->val,ip,sizeof(ip)))
        	{        		
        	
			    fprintf(f, "\n  original source ip : %s, mask: 0x%04x (success %d)",
				    ip, cnsrc->mask, cnsrc->success);
            }
            else
            {
                fprintf(f, "\n  Invalid cnsrc val:%d\n",cnsrc->val);
	...
	if (tb[TCA_U32_CNSPORT]) {

		struct tc_u32_cnsport *cnsrcport = RTA_DATA(tb[TCA_U32_CNSPORT]);
		
		if (RTA_PAYLOAD(tb[TCA_U32_CNSPORT]) < sizeof(*cnsrcport)) {
			fprintf(f, "\n  Invalid cnsrcport payload len:%d(<%d) (kernel&iproute2 mismatch)\n",
			    RTA_PAYLOAD(tb[TCA_U32_CNSPORT]),
			    sizeof(*cnsrcport));
		} 
		else 
		{
        	
			fprintf(f, "\n  original source port : %d, mask :0x%04x (success %d)",
				    ntohs(cnsrcport->val), cnsrcport->mask, cnsrcport->success);
		}
	}
    ...
    if (tb[TCA_U32_OUTDEV]) {
        struct rtattr *odev = tb[TCA_U32_OUTDEV];
        fprintf(f, "\n  output dev %s\n", (char *) RTA_DATA(odev));
    }
	if (tb[TCA_U32_ACT]) {
