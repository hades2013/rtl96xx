...
static int esfq_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	opt.hash_kind= TCA_ESFQ_HASH_CLASSIC;
	...
	while (argc > 0) {
		if (strcmp(*argv, "quantum") == 0) {
			...
			if (strcmp(*argv, "classic") == 0) {
				...
				opt.hash_kind = TCA_ESFQ_HASH_DST;
			} else if (strcmp(*argv, "src") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_SRC;
			} else if (strcmp(*argv, "fwmark") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_FWMARK;
			} else if (strcmp(*argv, "ctorigsrc") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_CTORIGSRC;
			} else if (strcmp(*argv, "ctorigdst") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_CTORIGDST;
			} else if (strcmp(*argv, "ctreplsrc") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_CTREPLSRC;
			} else if (strcmp(*argv, "ctrepldst") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_CTREPLDST;
			} else if (strcmp(*argv, "ctnatchg") == 0) {
				opt.hash_kind = TCA_ESFQ_HASH_CTNATCHG;
			} else {
