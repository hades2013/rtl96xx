...
static int xfrm_selector_iszero(struct xfrm_selector *s)
{
	...
	memset(&s0, 0, sizeof(s0));
      /* BEGIN: Modified by jinliming 05548, 2008/5/29   PN:SHD06351 */
	return (memcmp(&s0.daddr, &s->daddr, sizeof(s0)-sizeof(s0.name)) == 0);
	//return (memcmp(&s0, s, sizeof(s0)) == 0);
	/* END:   Modified by jinliming 05548, 2008/5/29 */
}
			    ...
			    const char *title)
{
	...
	memset(buf, '\0', sizeof(buf));
	/* BEGIN: Added by jinliming 05548, 2008/5/29   PN:SHD06351 */
	fprintf(fp, "%s ", xsinfo->sel.name);
	/* END:   Added by jinliming 05548, 2008/5/29 */
