...
static int ipneigh_modify(int cmd, int flags, int argc, char **argv)
{
	...
	if (rtnl_talk(&rth, &req.n, 0, 0, NULL, NULL, NULL) < 0) {
        /* Modified by liujiaquan, if there is an error here we should not exit
         * silently. We should return with a error, so "ip -force -batch" command
         * can process consequently, otherwise parameter "-force" has no effect */
//		exit(2);
        return -1;
    }
