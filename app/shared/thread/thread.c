...
#if 1 /*Implement by mw_time.c*/
static long get_sysup_time(struct timeval *tv )
{
	tv->tv_sec  = uptime();		
	tv->tv_usec = 0;
//	MW_DBG_INFO("uptime %ld\n",uptime());	
  ...
  if (thread_time > 200000L)
    {
       ...
       * to fix.
       */
      syslog(LOG_ERR,"CPU HOG task %lx ran for %ldms",
...
{
  struct thread dummy; 
  m = m;  /* avoid the compiler warning */
