...
void pool_create( UINT8 *base, INT32 size)
{
    arenabase = base;
    arenasize = size;

    ULONG front_misalign;
    INT32 correction;
    ...
    front_misalign = (ULONG)chunk2mem(base) & MALLOC_ALIGN_MASK;
...
UINT8 *
pool_resize_alloc( UINT8 *oldmem,    INT32 bytes,  INT32 *poldsize )
{
...
#define CYG_MEMPOOL_STAT_FREEBLOCKS 1
#define CYG_MEMPOOL_STAT_TOTALFREE 2
#define CYG_MEMPOOL_STAT_TOTALALLOCATED 4
#define CYG_MEMPOOL_STAT_MAXFREE 8
...
void get_status(UINT16 flags, Cyg_Mempool_Status *status)
{
                     ...
                     CYG_MEMPOOL_STAT_TOTALALLOCATED|CYG_MEMPOOL_STAT_MAXFREE)))
    {
        ...
        //ASSERT( ((avail + SIZE_SZ + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK) >= MINSIZE, "free mem negative!" );
        ...
        //ASSERT( ((maxfree + SIZE_SZ + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK) >= MINSIZE, "max free block size negative!" );
    } // if
...
//INT32 pool_status(CFG_MEMPOOL_STATUS_S *stStatus)
INT32 pool_status(Cyg_Mempool_Status *stStatus)
{
    Cyg_Mempool_Status memStatus;

    memset(&memStatus, 0x00, sizeof(Cyg_Mempool_Status));
    get_status(0xffff, &memStatus);

    stStatus->arenabase = (UINT32)(ULONG)memStatus.arenabase;
    stStatus->arenasize = memStatus.arenasize;
    stStatus->freeblocks = memStatus.freeblocks;
    stStatus->totalallocated = memStatus.totalallocated;
    stStatus->totalfree = memStatus.totalfree;
    stStatus->maxfree = memStatus.maxfree;
    return 0;
}
#endif
//----------------------------------------------------------------------------
